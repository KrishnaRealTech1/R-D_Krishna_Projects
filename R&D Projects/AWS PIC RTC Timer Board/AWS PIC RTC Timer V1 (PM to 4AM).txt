...
// -----------------------------------------------------------------------------
// Keep this as 1 only for testing a custom RTC time.
// Example:
...
// Production fixed schedules run at HH:00.
#define PRODUCTION_TRIGGER_MINUTE       0

// DIP 1 OFF + DIP 2 OFF night relay mode.
// Relay ON from 9 PM to 4 AM.
#define NIGHT_MODE_ON_HOUR              22
#define NIGHT_MODE_OFF_HOUR             4
...
static bool Is_Night_Mode_Relay_On_Time(const RTC_Time *now);
...
{
    // DIP 1 OFF + DIP 2 OFF:
    // Relay ON from 9:00 PM to 3:59 AM.
    // Relay OFF from 4:00 AM to 8:59 PM.
    if (now->hour >= NIGHT_MODE_ON_HOUR)
    {
        return true;
    }

    if (now->hour < NIGHT_MODE_OFF_HOUR)
    {
        return true;
    }
...
static bool Is_Scheduled_Restart_Time(Reset_Mode mode, const RTC_Time *now)
{
    ...
    switch (mode)
    {
        ...
        case RESET_MODE_TEST:
            // DIP 1 OFF + DIP 2 OFF is now handled separately in main().
            // It is no longer a 3-second scheduled reset pulse mode.
            return false;
...
void main(void)
{
    ...
    // Load last triggered time from EEPROM.
    // This prevents relay re-triggering if PIC resets during same RTC minute.
    ...
    // Wait until RTC is readable.
    // Relay stays OFF while RTC is unstable.
    ...
    while (1)
    {
        ...
        if (DS3231_ReadTime_With_Retry(&now))
        {
            ...
            if (Is_Scheduled_Restart_Time(selected_mode, &now))
            {
                ...
                {
                    // Save trigger time BEFORE turning relay ON.
                    // If relay switching resets the PIC, EEPROM already remembers
                    // that this exact minute was triggered.
                    ...
                    // Stay inside this loop until RTC minute/date changes.
                    // This avoids repeated schedule checking during the same minute.
                    while (1)
                    {
                        if (DS3231_ReadTime_With_Retry(&after_trigger_time))
                        {
                            if (!Already_Triggered_This_Minute(&after_trigger_time,
                                                               last_trigger_hour,
                                                               last_trigger_minute,
                                                               last_trigger_date,
                                                               last_trigger_month,
...
#else
            if (selected_mode == RESET_MODE_TEST)
            {
                // DIP 1 OFF + DIP 2 OFF:
                // Relay ON from 9 PM to 4 AM.
                // Relay OFF at 4 AM.
                if (Is_Night_Mode_Relay_On_Time(&now))
                {
                    Relay_On();
                }
                else
        ...
        {
            // Runtime RTC read failure:
            // Do not blink relay.
            // Keep relay OFF and retry.
            Relay_Off();
            I2C_Init();
            Delay_Approx_1s();
        }
