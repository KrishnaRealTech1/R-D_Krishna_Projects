...
# Garbage AI model is supplied locally because the original working installer uses a custom ONNX file.
if [[ ! -f "$PI_HOME/garbage_yolo.onnx" ]]; then
  echo "ERROR: Missing $PI_HOME/garbage_yolo.onnx" >&2
  echo "Copy garbage_yolo.onnx to the Pi user home before running this merged installer." >&2
  exit 1
fi
