...
echo "=================================================="
echo " G-Cam Full Clean Reinstall - V7.1 PV ONLY V8"
...
cat > "$APP_DIR/data/config.json" <<JSON
{
  ...
  "software_features": "Tiruppur Requirement Only + New PEM Server Interface Update",
...
SOFTWARE_VERSION = str(CONFIG.get("software_version", "Version 7.1")).strip()
SOFTWARE_FEATURES = str(
    CONFIG.get(
        "software_features",
                "Tiruppur Requirement Only + New PEM Server Interface Update"
    )
).strip()
...
def build_high_quality_event_image(
    ...
    When refine_fn is supplied, detections are re-run on the exact frame being
    saved. This prevents stale/sub-stream coordinates from being drawn in a
    different place on the high-quality image.
    """
    ...
    while time.time() < deadline and not stop_event.is_set():
        ...
        if main_frame is not None and main_frame.size > 0:
            if callable(refine_fn):
                ...
                if refined:
                    return draw_fn(main_frame, refined), refined, "main_redetect"

                if strict_refine:
                    # Never draw stale sub-stream coordinates on a different MAIN frame.
                    # If MAIN re-detect fails, save the exact detection frame instead.
                    # This keeps the marked box aligned and still allows the event image
                    # to show on the dashboard and upload to SFTP.
                    if detections:
                        fallback = detection_frame.copy()
                        return draw_fn(fallback, detections), detections, "sub_exact_after_main_redetect_none"
                    return main_frame.copy(), [], "main_redetect_none"

            scaled = scale_detections_to_frame(detections, detection_frame, main_frame)
            return draw_fn(main_frame, scaled), scaled, "main_scaled"
