...
DEFAULT_REBOOT_TIME_1="03:30:00"
DEFAULT_REBOOT_TIME_2="15:30:00"

DEFAULT_ROUTER_REBOOT_TIME_1="03:25"
DEFAULT_ROUTER_REBOOT_TIME_2="15:25"
...
read -rp "Enter RTSP URL [${DEFAULT_RTSP_URL}]: " RTSP_URL
...
echo "Battery interface disabled."
echo
...
echo "[7/17] Configure ALSA default to USB speaker hw:2,0..."
...
EOF
amixer -c 2 set Speaker 100% unmute || true
amixer -c 2 set PCM 100% unmute || true
amixer -c 2 set Master 100% unmute || true
sudo pkill -9 aplay 2>/dev/null || true
sudo pkill -9 ffplay 2>/dev/null || true
...
echo "[10/17] Download person model..."
cd "$APP_DIR/models"
...
MIN_FILL_RATIO = 0.06
MAX_FILL_RATIO = 0.96
MAX_ASPECT_RATIO = 6.8
MIN_EDGE_DENSITY = 0.010
MAX_SOLIDITY = 0.98
BOTTOM_STRIP_IGNORE_RATIO = 0.985
...
def default_state() -> Dict[str, Any]:
    return {
        ...
        "last_garbage": {
            ...
            "blob_count": 0,
            "triggered_by": None,
            "reference_items": GARBAGE_REFERENCE_ITEMS
        },
        "last_audio": {
            "file": None,
            "source_path": None,
...
def load_state() -> Dict[str, Any]:
    ...
    with state_lock:
        ...
        state_cache["system"]["garbage_mode"] = "hybrid_reference_filtered"

        if "last_garbage" not in state_cache:
            state_cache["last_garbage"] = default_state()["last_garbage"]
        state_cache["last_garbage"].setdefault("reference_items", GARBAGE_REFERENCE_ITEMS)
...
def init_models() -> None:
    global person_net, hog_detector
...
def record_person_video_clip(output_path: Path, duration_sec: int = PERSON_VIDEO_DURATION_SEC) -> None:
    ...
    if proc.returncode != 0 or not output_path.exists() or output_path.stat().st_size <= 0:
        raise RuntimeError(f"FFmpeg video record failed: {proc.stderr.strip()[:500]}")
...
def async_upload_person_video(
    ...
    def worker():
        try:
            ...
            time.sleep(1)  # wait 1 second after SFTP upload before MQTT response
            ...
            try:
                time.sleep(1)  # wait 1 second before MQTT failure response also
                mqtt_publish(MQTT_RESPONSE_TOPIC, {
                    ...
                    "camera_name": CAMERA_NAME,
                    "status": "failed",
                    "message": f"Video upload failed: {err}",
                    "video_name": remote_name,
                    "duration_sec": PERSON_VIDEO_DURATION_SEC,
                    "person_count": person_count,
                    "best_confidence": best_confidence,
...
def handle_person_video_event(
    ...
    try:
        record_person_video_clip(local_video_path, PERSON_VIDEO_DURATION_SEC)
        update_state(lambda s: s.update({
            "last_video": {
                "file": video_name,
                "time": event_time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        try:
            mqtt_publish(MQTT_RESPONSE_TOPIC, {
                ...
                "camera_name": CAMERA_NAME,
                "status": "failed",
                "message": f"Video record failed: {video_exc}",
...
def person_box_allowed(frame: np.ndarray, box: Tuple[int, int, int, int], confidence: float) -> bool:
    ...
    if bh / max(bw, 1) < PERSON_MIN_ASPECT_RATIO:
        return False

    if bh < int(h * PERSON_MIN_BOX_HEIGHT_RATIO):
        return False

    rx1 = int(w * PERSON_ALLOWED_ROI[0])
    ry1 = int(h * PERSON_ALLOWED_ROI[1])
    rx2 = int(w * PERSON_ALLOWED_ROI[2])
    ry2 = int(h * PERSON_ALLOWED_ROI[3])
...
def detect_persons(frame: np.ndarray) -> List[Dict[str, Any]]:
    ...
    for d in merged:
        ...
        if not person_box_allowed(frame, d["box"], float(d["confidence"])):
            continue
...
def build_foreground_mask(curr_roi: np.ndarray, ref_roi: np.ndarray) -> Tuple[np.ndarray, float, float]:
    ...
    _, m1 = cv2.threshold(gray_diff, 18, 255, cv2.THRESH_BINARY)
    _, m2 = cv2.threshold(sat_diff, 22, 255, cv2.THRESH_BINARY)
    _, m3 = cv2.threshold(val_diff, 20, 255, cv2.THRESH_BINARY)
    ...
    _, edge_change_bin = cv2.threshold(edge_change, 25, 255, cv2.THRESH_BINARY)
    mask = cv2.bitwise_or(mask, edge_change_bin)

    valid_light = cv2.inRange(curr_v, 26, 255)
...
def count_candidate_objects(curr_roi: np.ndarray, ref_roi: np.ndarray) -> Tuple[int, float, float, float, List[Tuple[int, int, int, int]]]:
    ...
    curr_edges = cv2.Canny(curr_gray, 45, 140)
    ref_edges = cv2.Canny(ref_gray, 45, 140)
    ...
    for contour in contours:
        ...
        if solidity > MAX_SOLIDITY and fill_ratio > 0.80:
            continue
        ...
        component_edges = cv2.bitwise_and(edges[y:y + h, x:x + w], edges[y:y + h, x:x + w], mask=component_mask)
...
def draw_garbage_boxes(frame: np.ndarray, roi_box: Tuple[int, int, int, int], boxes: List[Tuple[int, int, int, int]], severity: str) -> np.ndarray:
    out = frame.copy()
    ...
    cv2.putText(out, "Garbage ROI", (x1_roi + 6, max(20, y1_roi - 8)),
    ...
    for idx, (x1, y1, x2, y2) in enumerate(boxes, start=1):
        ...
        cv2.putText(out, f"G {idx}", (gx1, max(24, gy1 - 8)),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.55, color, 2, cv2.LINE_AA)

    cv2.putText(out, f"Severity: {severity}", (20, 30),
...
def analyze_garbage_once(frame: np.ndarray) -> Dict[str, Any]:
    ...
    ref_roi = load_reference_roi_shape(roi.shape[:2])
    ...
    if ref_roi is None:
        return {
            ...
            "blob_count": 0,
            "boxes": [],
            "marked_frame": draw_garbage_boxes(frame, roi_box, [], "NONE"),
            "roi_color": roi.copy(),
            "ref_missing": True
        }

    blob_count, diff_ratio, edge_ratio, blob_area_ratio, boxes = count_candidate_objects(roi, ref_roi)
    severity = garbage_count_to_severity(blob_count)
    marked = draw_garbage_boxes(frame, roi_box, boxes, severity)
...
def analyze_garbage_stable() -> Optional[Dict[str, Any]]:
    ...
    for idx in range(GARBAGE_CAPTURE_SAMPLES):
        ...
        try:
            samples.append(analyze_garbage_once(frame))
        except Exception:
            pass
    ...
    samples_sorted = sorted(samples, key=lambda x: (x["blob_count"], x["blob_area_ratio"], x["edge_ratio"]))
        ...
        if BATTERY_MODE == "sysfs" and BATTERY_SYSFS_PATH:
            ...
            if value > 1000:
                if value > 100000:
                    ...
                    value = value / 1000.0
            return round(value * BATTERY_DIVIDER_RATIO, 3)

        if BATTERY_MODE == "command" and BATTERY_COMMAND:
            out = subprocess.check_output(shlex.split(BATTERY_COMMAND), text=True, timeout=5).strip()
            value = parse_first_float(out)
            if value is None:
                return None
            return round(value * BATTERY_DIVIDER_RATIO, 3)
    except Exception:
...
def detector_loop() -> None:
    ...
    while not stop_event.is_set():
        ...
        if PERSON_DETECT_EVERY_N_FRAMES > 1 and counter % PERSON_DETECT_EVERY_N_FRAMES != 0:
            time.sleep(0.001)
            continue
        ...
        try:
            ...
            if persons:
                ...
                if should_fire_event:
                    ...
                    async_send(path, SFTP_PERSON_DIR, filename, MQTT_CAMERA_EVENT_TOPIC, mqtt_payload)
                    ...
                    threading.Thread(
                        ...
                        args=(now, len(persons), best_conf),
                        daemon=True
                    ).start()
            ...
            update_state(lambda s: s["system"].update({"last_error": f"Person detect error: {exc}"}))
            time.sleep(0.1)
...
def run_garbage_detection(triggered_by: str = "manual", create_live_link: bool = True) -> Optional[Dict[str, Any]]:
    ...
    if result["ref_missing"]:
        try:
            save_reference_roi(result["roi_color"])
        except Exception:
            pass
        ...
        return {
            ...
            "blob_count": 0,
            "garbage_count": 0,
            "garbage_labels": GARBAGE_REFERENCE_ITEMS,
            "image_file": None,
            "camera_name": CAMERA_NAME,
            "message": "Reference image initialized"
        }
    ...
    if severity == "NONE":
        ...
        if garbage_clean_streak >= REFERENCE_UPDATE_STABLE_COUNT:
            try:
                save_reference_roi(result["roi_color"])
            except Exception:
                pass
...
def normalize_command(raw: str) -> str:
    ...
    mapping = {
        ...
        "scheduled warning disabled": WARNING_AUDIO_SCHEDULE_DISABLE_COMMAND,
		 "trigger router reboot": "trigger router reboot",
    }
...
def handle_command(payload: Dict[str, Any]) -> None:
    ...
    if cmd == DATA_STATUS_COMMAND:
        ...
        add_event("data_status_requested", {"network_status": status["network_status"], "ram_percent": status["ram_percent"]})
        return
