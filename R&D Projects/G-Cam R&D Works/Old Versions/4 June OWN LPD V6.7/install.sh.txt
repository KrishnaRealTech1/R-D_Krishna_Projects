...
DEFAULT_SFTP_LICENSE_PLATE_DIR="/home/ftpuser/ftp/files/${DEFAULT_DEVICE_ID}/LPD/Picture/License Plate Recognition"
...
# Optional direct download URL for license plate model.
# Keep empty if you want to copy license_plate_yolo.onnx manually.
# Example:
# DEFAULT_LICENSE_PLATE_MODEL_URL="https://your-domain.com/models/license_plate_yolo.onnx"
DEFAULT_LICENSE_PLATE_MODEL_URL=""

echo "=================================================="
echo " G-Cam Full Clean Reinstall - V6.7 FIXED"
...
DEFAULT_MQTT_LICENSE_PLATE_EVENT_TOPIC="${DEVICE_ID}/license_plate_event"
read -rp "Enter MQTT License Plate Event Topic [${DEFAULT_MQTT_LICENSE_PLATE_EVENT_TOPIC}]: " MQTT_LICENSE_PLATE_EVENT_TOPIC
MQTT_LICENSE_PLATE_EVENT_TOPIC="${MQTT_LICENSE_PLATE_EVENT_TOPIC:-$DEFAULT_MQTT_LICENSE_PLATE_EVENT_TOPIC}"
...
read -rp "Paste License Plate SFTP Path [${DEFAULT_SFTP_LICENSE_PLATE_DIR}]: " SFTP_LICENSE_PLATE_DIR
SFTP_LICENSE_PLATE_DIR="${SFTP_LICENSE_PLATE_DIR:-$DEFAULT_SFTP_LICENSE_PLATE_DIR}"
...
if [[ "${SFTP_LICENSE_PLATE_DIR}" != /* ]]; then
  SFTP_LICENSE_PLATE_DIR="/${SFTP_LICENSE_PLATE_DIR}"
fi
SFTP_LICENSE_PLATE_DIR="${SFTP_LICENSE_PLATE_DIR%/}"
...
echo
echo "License Plate Detection model"
echo "Model filename must be: license_plate_yolo.onnx"
echo "You can place it in one of these locations before running installer:"
echo "  1) Same folder as this installer"
echo "  2) $PI_HOME/license_plate_yolo.onnx"
echo "  3) /boot/firmware/license_plate_yolo.onnx"
echo
read -rp "Enter License Plate Model Download URL, optional [${DEFAULT_LICENSE_PLATE_MODEL_URL}]: " LICENSE_PLATE_MODEL_URL
LICENSE_PLATE_MODEL_URL="${LICENSE_PLATE_MODEL_URL:-$DEFAULT_LICENSE_PLATE_MODEL_URL}"
...
if [[ ! -f "$PI_HOME/garbage_yolo.onnx" ]]; then
  echo "ERROR: Missing $PI_HOME/garbage_yolo.onnx"
  echo "Copy garbage_yolo.onnx to the Pi user home before running this installer."
  exit 1
fi

install -m 644 "$PI_HOME/garbage_yolo.onnx" "$APP_DIR/models/garbage_yolo.onnx"

# License Plate Detection YOLO ONNX model.
# The installer will try these sources in order:
#   1. Same folder as this installer
#   2. Pi user home
#   3. /boot/firmware
#   4. Optional download URL

LPD_MODEL_NAME="license_plate_yolo.onnx"
LPD_MODEL_DEST="$APP_DIR/models/$LPD_MODEL_NAME"

INSTALLER_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LPD_MODEL_SOURCE_1="$INSTALLER_DIR/$LPD_MODEL_NAME"
LPD_MODEL_SOURCE_2="$PI_HOME/$LPD_MODEL_NAME"
LPD_MODEL_SOURCE_3="/boot/firmware/$LPD_MODEL_NAME"
LPD_MODEL_SOURCE_4="/boot/$LPD_MODEL_NAME"

echo
echo "[LPD MODEL] Searching for $LPD_MODEL_NAME..."

if [[ -f "$LPD_MODEL_SOURCE_1" ]]; then
  install -m 644 "$LPD_MODEL_SOURCE_1" "$LPD_MODEL_DEST"
  echo "[LPD MODEL] Installed from installer folder: $LPD_MODEL_SOURCE_1"

elif [[ -f "$LPD_MODEL_SOURCE_2" ]]; then
  install -m 644 "$LPD_MODEL_SOURCE_2" "$LPD_MODEL_DEST"
  echo "[LPD MODEL] Installed from Pi home: $LPD_MODEL_SOURCE_2"

elif [[ -f "$LPD_MODEL_SOURCE_3" ]]; then
  install -m 644 "$LPD_MODEL_SOURCE_3" "$LPD_MODEL_DEST"
  echo "[LPD MODEL] Installed from boot firmware: $LPD_MODEL_SOURCE_3"

elif [[ -f "$LPD_MODEL_SOURCE_4" ]]; then
  install -m 644 "$LPD_MODEL_SOURCE_4" "$LPD_MODEL_DEST"
  echo "[LPD MODEL] Installed from boot: $LPD_MODEL_SOURCE_4"

elif [[ -n "${LICENSE_PLATE_MODEL_URL:-}" ]]; then
  echo "[LPD MODEL] Downloading from URL..."
  if wget -q -O "$LPD_MODEL_DEST.tmp" "$LICENSE_PLATE_MODEL_URL"; then
    if [[ -s "$LPD_MODEL_DEST.tmp" ]]; then
      mv "$LPD_MODEL_DEST.tmp" "$LPD_MODEL_DEST"
      chmod 644 "$LPD_MODEL_DEST"
      echo "[LPD MODEL] Downloaded and installed: $LPD_MODEL_DEST"
    else
      rm -f "$LPD_MODEL_DEST.tmp"
      echo "[LPD MODEL] ERROR: downloaded file is empty"
      echo "[LPD MODEL] LPD YOLO model unavailable. OCR fallback can still run if enabled in app.py."
    fi
  else
    rm -f "$LPD_MODEL_DEST.tmp"
    echo "[LPD MODEL] ERROR: download failed"
    echo "[LPD MODEL] LPD YOLO model unavailable. OCR fallback can still run if enabled in app.py."
  fi

else
  echo "[LPD MODEL] WARNING: $LPD_MODEL_NAME not found."
  echo "[LPD MODEL] Put the model in one of these locations and rerun installer:"
  echo "  - $LPD_MODEL_SOURCE_1"
  echo "  - $LPD_MODEL_SOURCE_2"
  echo "  - $LPD_MODEL_SOURCE_3"
  echo "  - $LPD_MODEL_SOURCE_4"
  echo "[LPD MODEL] YOLO LPD will not run without this model."
  echo "[LPD MODEL] OCR fallback can still run only if your app.py has OCR fallback enabled."
fi

if [[ -f "$LPD_MODEL_DEST" ]]; then
  echo "[LPD MODEL] Final model ready: $LPD_MODEL_DEST"
else
  echo "[LPD MODEL] Final model missing: $LPD_MODEL_DEST"
fi
...
cat > "$APP_DIR/data/config.json" <<JSON
{
  ...
  "software_version": "Version 6.7",
  "software_features": "Impact LPD + Prama Colour Night Vision Interface in PI 5 4GB",
  ...
  "feature_flags": {
    ...
    "license_plate_detection_enabled": true,
    "license_plate_ocr_enabled": true,
  ...
  "mqtt": {
    ...
    "license_plate_event_topic": "$MQTT_LICENSE_PLATE_EVENT_TOPIC",
  ...
  "sftp": {
    ...
    "license_plate_dir": "$SFTP_LICENSE_PLATE_DIR",
...
<div class="wrap">
  ...
  <div class="tabs">
    ...
    <button class="tab-btn" onclick="showTab('license_plate', this)">Last License Plate</button>
  ...
  <div id="tab-person" class="panel">
    ...
    <div id="tab-license_plate" class="panel">
    <div class="panel-grid">
      <div class="media-card">
        <h3>Last License Plate Detection Image</h3>
        <div class="media-body">
          {% if state.last_license_plate.image %}
            <img id="licensePlateImg" src="/media/{{ state.last_license_plate.image }}?t={{ ts }}" alt="License Plate">
          {% else %}
            <img id="licensePlateImg" src="" alt="No License Plate">
          {% endif %}
        </div>
      </div>
      ...
      <div class="info">
        <div class="info-box">
          <h4>Time</h4>
          <div id="licensePlateTime">{{ state.last_license_plate.time or "Waiting..." }}</div>
        </div>

        <div class="info-box">
          <h4>Vehicle Number</h4>
          <div class="big" id="licensePlateNumber">{{ state.last_license_plate.vehicle_number or "UNKNOWN" }}</div>
        </div>

        <div class="info-box">
          <h4>Status</h4>
          <div id="licensePlateStatus">
            {% if state.last_license_plate.detected %}
              Detected
            {% elif state.last_license_plate.status %}
              {{ state.last_license_plate.status }}
            {% else %}
              No recent detection
            {% endif %}
          </div>
        ...
        <div class="info-box">
          <h4>Plate Confidence</h4>
          <div id="licensePlateConfidence">{{ state.last_license_plate.plate_confidence or 0 }}</div>
        </div>

        <div class="info-box">
          <h4>OCR Confidence</h4>
          <div id="licensePlateOcrConfidence">{{ state.last_license_plate.ocr_confidence or 0 }}</div>
        </div>

        <div class="info-box">
          <h4>Plate Count</h4>
          <div id="licensePlateCount">{{ state.last_license_plate.plate_count or 0 }}</div>
        </div>

        <div class="info-box">
          <h4>Image File</h4>
          <div class="link-box" id="licensePlateImageFile">{{ state.last_license_plate.image_file or "None" }}</div>
        </div>

        <div class="info-box">
          <h4>SFTP Uploaded Path</h4>
          <div class="link-box" id="licensePlateImagePath">{{ state.last_license_plate.image_uploaded_path or "None" }}</div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div id="tab-controls" class="panel">
    ...
    <div class="controls-grid">
      ...
      <div class="control-card">
        <h4>Detection</h4>
        <div class="control-title">🔢 License Plate Detection</div>
        <div id="ctrlLicensePlateDetectionStatus" class="control-status">Loading...</div>
        <div class="control-actions">
          <button class="control-btn ok" onclick="controlSetFeature('license_plate_detection_enabled', true)">Enable</button>
          <button class="control-btn danger" onclick="controlSetFeature('license_plate_detection_enabled', false)">Disable</button>
        </div>
      </div>

      <div class="control-card">
        <h4>OCR</h4>
        <div class="control-title">🔤 License Plate OCR</div>
        <div id="ctrlLicensePlateOcrStatus" class="control-status">Loading...</div>
        <div class="control-actions">
          <button class="control-btn ok" onclick="controlSetFeature('license_plate_ocr_enabled', true)">Enable</button>
          <button class="control-btn danger" onclick="controlSetFeature('license_plate_ocr_enabled', false)">Disable</button>
        </div>
      </div>
	  
	  <div class="control-card">
        <h4>LPD Action</h4>
        <div class="control-title">🔢 Manual Plate Detect</div>
        <div class="control-status">On Demand</div>
        <div class="control-actions">
          <button class="control-btn primary" onclick="controlManualLicensePlate()">🔢 Run LPD</button>
        </div>
        <div style="color:var(--muted);font-size:12px;margin-top:8px;line-height:1.5">
          Runs independent full-frame LPD. It does not wait for vehicle detection.
        </div>
      </div>
  ...
  <div id="tab-links" class="panel">
    <div class="panel-grid">
      ...
      <div class="info">
        ...
        <div class="info-box"><h4>SFTP License Plate Path</h4><div class="link-box" id="sftpLicensePlate">{{ sftp_license_plate_dir }}</div></div>
...
const CONTROL_STATUS_IDS = {
  ...
  license_plate_detection_enabled: 'ctrlLicensePlateDetectionStatus',
  license_plate_ocr_enabled: 'ctrlLicensePlateOcrStatus',
...
async function controlManualLicensePlate() {
  try {
    controlStatus('Starting manual license plate detection...');

    const data = await controlFetchJson('/api/license_plate_detect', {
      method: 'POST'
    });

    controlStatus(data.message || 'Manual LPD started. Check Last License Plate tab.');

    setTimeout(refreshState, 1500);
  } catch (err) {
    controlStatus('Manual LPD failed: ' + err.message, false);
  }
}
...
async function refreshState(){
  try{
    ...
    const lp = s.last_license_plate || {};

    document.getElementById('licensePlateTime').textContent = lp.time || 'Waiting...';
    document.getElementById('licensePlateNumber').textContent = lp.vehicle_number || lp.plate_text || 'UNKNOWN';
    document.getElementById('licensePlateStatus').textContent =
      lp.detected ? 'Detected' : (lp.status || 'No recent detection');
    document.getElementById('licensePlateConfidence').textContent = lp.plate_confidence ?? 0;
    document.getElementById('licensePlateOcrConfidence').textContent = lp.ocr_confidence ?? 0;
    document.getElementById('licensePlateCount').textContent = lp.plate_count ?? 0;
    document.getElementById('licensePlateImageFile').textContent = lp.image_file || 'None';
    document.getElementById('licensePlateImagePath').textContent = lp.image_uploaded_path || 'None';

    if (lp.image) {
      document.getElementById('licensePlateImg').src = '/media/' + lp.image + '?t=' + Date.now();
    }
    ...
    document.getElementById('sftpLicensePlate').textContent = s.sftp_license_plate_dir || '';
...
#
# LIVE_FRAME_WIDTH / EVENT_FRAME_WIDTH can be increased for better
# high-resolution viewing and event video without changing detection logic.
#
...
# Event video/image buffer.
EVENT_FRAME_WIDTH = 960

# Public Cloudflare live quality.
# Use 960 for smooth HD-like public live.
# Use 1280 only if CPU/network is stable.
CLOUDFLARE_LIVE_FRAME_WIDTH = 1280

# JPEG quality.
# 65 gives noticeably better public live than 55 without becoming too heavy.
MJPEG_QUALITY = 60
CLOUDFLARE_MJPEG_QUALITY = 68

# Stream FPS.
LIVE_STREAM_TARGET_FPS = 10.0
CLOUDFLARE_STREAM_TARGET_FPS = 8.0
...
# Lowered slightly because high-angle CCTV vehicles often score 0.55–0.75.
VEHICLE_CONF = 0.55
...
VEHICLE_MIN_AREA = 1800
VEHICLE_MIN_BOX_HEIGHT_RATIO = 0.045
VEHICLE_MIN_BOX_WIDTH_RATIO = 0.045
VEHICLE_MAX_BOX_WIDTH_RATIO = 0.90
VEHICLE_MAX_BOX_HEIGHT_RATIO = 0.90

# Prevent duplicate image/video events for same vehicle.
...
# Allows missed detections without resetting the stay timer.
VEHICLE_STABLE_GRACE_SEC = 1.20

# Minimum good detections during static stay window.
VEHICLE_MIN_STABLE_HITS = 2

# Event capture needs this confidence or higher.
VEHICLE_EVENT_MIN_CONF = 0.58
...
# ─────────────────────────────────────────────────────────────
# Model path:
#   /opt/gcam/models/license_plate_yolo.onnx
#
# LPD is independent. It does NOT wait for vehicle event.
# It uses:
#   1. YOLO plate detector when model detects plate box.
#   2. OCR contour fallback when YOLO misses.
#   3. Full-frame and tiled search for long distance.

# 512 improves long-distance plates more than 416.
# If Pi CPU becomes high, change this back to 416.
LICENSE_PLATE_INPUT_SIZE = 512

# Lower confidence allows difficult plate boxes.
# OCR fallback and shape filters reduce false positives.
LICENSE_PLATE_CONF = 0.12
LICENSE_PLATE_NMS = 0.35

# Independent scan interval.
LICENSE_PLATE_SCAN_INTERVAL_SEC = 3.0          # Pi can't do 1.0s ONNX inference
LICENSE_PLATE_REPEAT_UPLOAD_COOLDOWN_SEC = 15.0
LICENSE_PLATE_NO_PLATE_STATE_INTERVAL_SEC = 5.0

# Long-distance full-frame tiled search.
LICENSE_PLATE_ENABLE_LONG_DISTANCE_TILES = False   # Disable on Pi — too heavy
LICENSE_PLATE_MAX_SEARCH_ROIS = 4                  # Full frame + 3 ROIs max

# Plate box filters.
LICENSE_PLATE_MIN_AREA_RATIO = 0.00004
LICENSE_PLATE_MAX_AREA_RATIO = 0.22
LICENSE_PLATE_MIN_WIDTH_RATIO = 0.012
LICENSE_PLATE_MIN_HEIGHT_RATIO = 0.004
LICENSE_PLATE_MIN_ASPECT_RATIO = 1.10
LICENSE_PLATE_MAX_ASPECT_RATIO = 10.50
LICENSE_PLATE_MAX_DETECTIONS = 8

# Vehicle crop support remains available, but LPD no longer depends on it.
LICENSE_PLATE_VEHICLE_PAD_RATIO = 0.25
LICENSE_PLATE_ENABLE_FULL_FRAME_FALLBACK = True
LICENSE_PLATE_ENABLE_LOWER_VEHICLE_ROIS = True

# OCR fallback.
LICENSE_PLATE_ENABLE_OCR_FALLBACK = True
LICENSE_PLATE_OCR_FALLBACK_MAX_CANDIDATES = 18
LICENSE_PLATE_OCR_FALLBACK_MIN_SCORE = 22.0

# OCR text cleaning limits.
LICENSE_PLATE_OCR_MIN_CHARS = 4
LICENSE_PLATE_OCR_MAX_CHARS = 14
...
LICENSE_PLATE_DETECTION_ENABLE_COMMAND = "license plate detection enable"
LICENSE_PLATE_DETECTION_DISABLE_COMMAND = "license plate detection disable"
LICENSE_PLATE_OCR_ENABLE_COMMAND = "license plate ocr enable"
LICENSE_PLATE_OCR_DISABLE_COMMAND = "license plate ocr disable"
...
FEATURE_TOGGLE_COMMANDS = {
    ...
    LICENSE_PLATE_DETECTION_ENABLE_COMMAND,
    LICENSE_PLATE_DETECTION_DISABLE_COMMAND,
    LICENSE_PLATE_OCR_ENABLE_COMMAND,
    LICENSE_PLATE_OCR_DISABLE_COMMAND,
}
...
lpd_net = None
lpd_output_names: List[str] = []
lpd_dnn_lock = threading.Lock()

# Independent LPD runtime guard.
# This prevents multiple heavy LPD scans from running at the same time.
lpd_event_lock = threading.Lock()
last_lpd_upload_ts = 0.0
last_lpd_upload_plate_text = ""
...
def set_feature_flag(flag_name: str, enabled: bool, source: str = "mqtt command") -> Dict[str, bool]:
    ...
    def mutate(state):
        ...
        if flag_name == "license_plate_detection_enabled" and not enabled and "last_license_plate" in state:
            state["last_license_plate"]["detected"] = False
            state["last_license_plate"]["status"] = "disabled"
...
SOFTWARE_VERSION = str(CONFIG.get("software_version", "Version 6.7")).strip()
SOFTWARE_FEATURES = str(
    CONFIG.get(
        "software_features",
                "Impact LPD + Prama Colour Night Vision Interface in PI 5 4GB"
    )
).strip()
...
MQTT_LICENSE_PLATE_EVENT_TOPIC = CONFIG["mqtt"].get(
    "license_plate_event_topic",
    f"{DEVICE_ID}/license_plate_event"
)
...
SFTP_LICENSE_PLATE_DIR = CONFIG["sftp"].get(
    "license_plate_dir",
    f"{SFTP_BASE_DIR.rstrip('/')}/LicensePlate"
)
...
def load_state() -> Dict[str, Any]:
    ...
    with state_lock:
        ...
        if "last_license_plate" not in state_cache:
            state_cache["last_license_plate"] = default_state()["last_license_plate"]

        state_cache["last_license_plate"].setdefault("detected", False)
        state_cache["last_license_plate"].setdefault("time", None)
        state_cache["last_license_plate"].setdefault("vehicle_number", None)
        state_cache["last_license_plate"].setdefault("plate_text", None)
        state_cache["last_license_plate"].setdefault("plate_confidence", 0.0)
        state_cache["last_license_plate"].setdefault("ocr_confidence", 0.0)
        state_cache["last_license_plate"].setdefault("image", None)
        state_cache["last_license_plate"].setdefault("image_file", None)
        state_cache["last_license_plate"].setdefault("image_uploaded_path", None)
        state_cache["last_license_plate"].setdefault("status", None)
        state_cache["last_license_plate"].setdefault("plate_count", 0)
...
def init_models() -> None:
    ...
    global lpd_net, lpd_output_names
    ...
    # License Plate Detection model is optional.
    # If the model is missing, old person/vehicle/garbage logic continues.
    lpd_model = MODEL_DIR / "license_plate_yolo.onnx"

    if lpd_model.exists():
        try:
            lpd_net = cv2.dnn.readNetFromONNX(str(lpd_model))
            lpd_net.setPreferableBackend(cv2.dnn.DNN_BACKEND_OPENCV)
            lpd_net.setPreferableTarget(cv2.dnn.DNN_TARGET_CPU)

            try:
                layer_names = lpd_net.getLayerNames()
                out_layers = lpd_net.getUnconnectedOutLayers()
                lpd_output_names = [layer_names[i - 1] for i in out_layers.flatten()]
            except Exception:
                lpd_output_names = []

            print(f"[LPD OK] Loaded license plate model: {lpd_model}", flush=True)

        except Exception as exc:
            lpd_net = None
            lpd_output_names = []
            print(f"[LPD ERROR] Failed to load {lpd_model}: {exc}", flush=True)
    else:
        lpd_net = None
        lpd_output_names = []
        print(
            f"[LPD WARN] Missing model: {lpd_model}. "
            "License plate detection will stay inactive.",
            flush=True
        )
...
def get_live_frame_copy() -> Optional[np.ndarray]:
    """
    Higher-resolution frame for dashboard live view and LPD fallback.
...
def get_latest_lpd_frame_copy() -> Optional[np.ndarray]:
    """
    Best available frame for LPD.

    Preference:
      1. Rolling event buffer frame, usually raw/high-resolution.
      2. Dashboard live frame.
      3. Process/detection frame.
    """
    try:
        with rolling_video_lock:
            if rolling_video_buffer:
                _, jpg_bytes = rolling_video_buffer[-1]
            else:
                jpg_bytes = None
    ...
    return get_live_frame_copy()


def scale_vehicle_boxes_for_frame(
    vehicles: List[Dict[str, Any]],
    src_shape: Tuple[int, int, int],
    dst_shape: Tuple[int, int, int]
) -> List[Dict[str, Any]]:
    """
    Vehicle boxes are usually from PROCESS_FRAME_WIDTH.
    LPD may run on higher-resolution frame, so boxes must be rescaled.
    """
    if not vehicles:
        return []
...
def sanitize_plate_text(raw_text: Any) -> str:
    """
    Clean OCR result for safe filename/MQTT usage.
    ...
    if len(text) < LICENSE_PLATE_OCR_MIN_CHARS:
        return "UNKNOWN"

    if len(text) > LICENSE_PLATE_OCR_MAX_CHARS:
        text = text[:LICENSE_PLATE_OCR_MAX_CHARS]

    return text


def build_license_plate_image_name(event_time: dt.datetime, plate_text: str) -> str:
    """
    SFTP filename format:
      DEVICEID_VEHICLENUMBER_YYYYMMDD_HHMMSS.jpg

    Example:
      RTGCAMx_TN09AB1234_20260603_154522.jpg
    """
    device_part = sanitize_filename_part(DEVICE_ID)
    plate_part = sanitize_plate_text(plate_text)
    time_part = event_time.strftime("%Y%m%d_%H%M%S")

    return f"{device_part}_{plate_part}_{time_part}.jpg"
...
def preprocess_plate_for_ocr(plate_crop: np.ndarray) -> np.ndarray:
    """
    Backward-compatible single OCR image.
    Main OCR uses multiple variants below.
    """
    variants = preprocess_plate_for_ocr_variants(plate_crop)
...
def build_lpd_detector_variants(image: np.ndarray) -> List[Tuple[str, np.ndarray]]:
    if image is None or image.size == 0:
        return []
    # Pi 4/5: limit to 2 variants max to avoid CPU overload
    ...
    try:
        ...
        y = clahe.apply(y)
        variants.append(("clahe", cv2.cvtColor(cv2.merge((y, cr, cb)), cv2.COLOR_YCrCb2BGR)))
    except Exception:
        pass
    return _dedupe_named_images(variants)


def preprocess_plate_for_ocr_variants(plate_crop: np.ndarray) -> List[Tuple[str, np.ndarray]]:
    """
    OCR variants for day/night/rain/shadow/angled plates.
    """
    ...
    try:
        ...
        # Upscale small/far plate crops before OCR.
        if w < 260:
            scale = min(
                5.0,
                max(2.0, 280.0 / max(1.0, float(w)))
            )
        ...
        for base_name, base in (("eq", eq), ("sharp", sharp)):
            try:
                _, th = cv2.threshold(
                    base,
                    0,
                ...
                variants.append((base_name + "_otsu", th))
                variants.append((base_name + "_otsu_inv", cv2.bitwise_not(th)))
            ...
            try:
                ...
                variants.append((base_name + "_adaptive", ad))
                variants.append((base_name + "_adaptive_inv", cv2.bitwise_not(ad)))

            except Exception:
                pass

        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))
        morphed: List[Tuple[str, np.ndarray]] = []
        ...
        variants.extend(morphed)

        return _dedupe_named_images(variants)

    except Exception as exc:
        print(f"[LPD OCR PREPROCESS ERROR] {exc}", flush=True)
        return []


def _score_plate_text(text: str, confidence: float) -> float:
    if text == "UNKNOWN":
        return -1.0
    ...
    length_score = min(10, length) * 3.0
    mix_score = 8.0 if alpha >= 1 and digit >= 1 else 0.0
    typical_score = 6.0 if 6 <= length <= 12 else 0.0

    return float(confidence) + length_score + mix_score + typical_score


def read_license_plate_text(plate_crop: np.ndarray) -> Tuple[str, float]:
    """
    OCR plate crop using multiple preprocessing variants and PSM modes.
    Returns best text and confidence.
    ...
    psm_modes = (7, 6)   # Only 2 modes on Pi — 7 is best for single-line plates
    ...
    try:
        ...
        for variant_name, ocr_img in variants:
            for psm in psm_modes:
                ...
                plate_text = sanitize_plate_text(raw_text)
                confidence = 0.0
                ...
                score = _score_plate_text(plate_text, confidence)

                # Bonus for Indian-style mixed letters and digits.
                has_alpha = any(ch.isalpha() for ch in plate_text)
                has_digit = any(ch.isdigit() for ch in plate_text)

                if has_alpha and has_digit:
                    score += 10.0

                if 6 <= len(plate_text) <= 12:
                    score += 8.0
        ...
        print(f"[LPD OCR ERROR] {exc}", flush=True)
        return "UNKNOWN", 0.0


def _run_lpd_yolo_once(
    image: np.ndarray,
    variant_name: str = "original"
) -> Tuple[List[List[int]], List[float], List[str]]:
    if lpd_net is None or image is None or image.size == 0:
        return [], [], []
    ...
    blob = cv2.dnn.blobFromImage(
        image,
        scalefactor=1.0 / 255.0,
        size=(LICENSE_PLATE_INPUT_SIZE, LICENSE_PLATE_INPUT_SIZE),
        swapRB=True,
        crop=False
    )
    ...
    try:
        with lpd_dnn_lock:
            ...
            if lpd_output_names:
                raw_outputs = lpd_net.forward(lpd_output_names)
            else:
                raw_outputs = lpd_net.forward()

    except Exception as exc:
        print(f"[LPD ERROR] YOLO forward failed: {exc}", flush=True)
        return [], [], []
    ...
    try:
        ...
        print(f"[LPD ERROR] YOLO output normalize failed: {exc}", flush=True)
        return [], [], []
    ...
    min_area = image_area * LICENSE_PLATE_MIN_AREA_RATIO
    max_area = image_area * LICENSE_PLATE_MAX_AREA_RATIO
    min_w = img_w * LICENSE_PLATE_MIN_WIDTH_RATIO
    min_h = img_h * LICENSE_PLATE_MIN_HEIGHT_RATIO
    ...
    for row in data:
        ...
        # YOLOv8 single-class often returns [x, y, w, h, score].
        # YOLOv5-style often returns [x, y, w, h, obj, classes...].
        if row.shape[0] == 5:
            conf = float(row[4])
        else:
            ...
            if class_scores.size == 0:
                conf = object_conf
            elif class_scores.size == 1:
                single_score = float(class_scores[0])

                if single_score <= 0.01:
                    conf = object_conf
                else:
                    conf = object_conf * single_score if object_conf <= 1.0 else single_score
            else:
                class_conf = float(np.max(class_scores))
                conf = object_conf * class_conf if object_conf <= 1.0 else class_conf

        if conf < LICENSE_PLATE_CONF:
            continue
        ...
        # Model-input coords.
            ...
            sx = img_w / float(LICENSE_PLATE_INPUT_SIZE)
            sy = img_h / float(LICENSE_PLATE_INPUT_SIZE)
        ...
        if aspect < LICENSE_PLATE_MIN_ASPECT_RATIO or aspect > LICENSE_PLATE_MAX_ASPECT_RATIO:
            continue
    ...
    return boxes_xywh, confidences, variant_names


def detect_license_plates_in_image(image: np.ndarray) -> List[Dict[str, Any]]:
    """
    Detect plates in one ROI/image using multi-light detector variants.
    Returns boxes local to the given image.
    """
    if lpd_net is None:
        return []
    ...
    for variant_name, variant_img in build_lpd_detector_variants(image):
        b, c, v = _run_lpd_yolo_once(
            variant_img,
            variant_name
        )
    ...
    try:
        ...
        print(f"[LPD ERROR] NMS failed: {exc}", flush=True)
        return []
    ...
    for idx in np.array(idxs).reshape(-1)[:LICENSE_PLATE_MAX_DETECTIONS]:
        idx = int(idx)
        x, y, w, h = boxes_xywh[idx]
        ...
        x1 = max(0, x - margin_x)
        y1 = max(0, y - margin_y)
        x2 = min(image.shape[1] - 1, x + w + margin_x)
        y2 = min(image.shape[0] - 1, y + h + margin_y)
        ...
        if is_feature_enabled("license_plate_ocr_enabled"):
            plate_text, ocr_confidence = read_license_plate_text(crop)
        ...
        detections.append({
            ...
            "crop": crop,
            "variant": variants[idx] if idx < len(variants) else "unknown",
        })

    detections.sort(
        key=lambda d: (
            1 if d.get("plate_text") != "UNKNOWN" else 0,
            _score_plate_text(
                d.get("plate_text", "UNKNOWN"),
                float(d.get("ocr_confidence", 0.0))
            ),
...
def _vehicle_lpd_rois(vehicle_crop: np.ndarray) -> List[Tuple[str, np.ndarray, int, int]]:
    """
    Search full vehicle crop and lower plate zones.
    """
    ...
    rois: List[Tuple[str, np.ndarray, int, int]] = [
        ("vehicle_full", vehicle_crop, 0, 0)
    ]

    if not LICENSE_PLATE_ENABLE_LOWER_VEHICLE_ROIS or h < 80 or w < 100:
        return rois

    candidates = [
        ("vehicle_lower_70", 0, int(h * 0.30), w, h),
        ("vehicle_lower_55", 0, int(h * 0.45), w, h),
        ("vehicle_center_lower", int(w * 0.08), int(h * 0.32), int(w * 0.92), h),
    ]
    ...
    for name, x1, y1, x2, y2 in candidates:
        ...
        roi = vehicle_crop[y1:y2, x1:x2].copy()

        if roi is not None and roi.size > 0:
            rois.append((name, roi, x1, y1))

    return rois


def _add_grid_lpd_rois(
    rois: List[Tuple[str, np.ndarray, int, int]],
    frame: np.ndarray,
    cols: int,
    rows: int,
    ...
    """
    Adds overlapping grid tiles for long-distance plates.
    Tile detection prevents far plates from becoming too small after
    full-frame resize into model input.
    """
    h, w = frame.shape[:2]

    if cols <= 0 or rows <= 0 or w <= 0 or h <= 0:
        return

    tile_w = int(min(w, max(260, (w / float(cols)) * 1.35)))
    tile_h = int(min(h, max(220, (h / float(rows)) * 1.35)))
    ...
    if cols == 1:
        ...
        x_starts = [
            int(round(v)) for v in np.linspace(
                0,
                max(0, w - tile_w),
                cols
            )
    ...
    for row_idx, y1 in enumerate(y_starts):
        for col_idx, x1 in enumerate(x_starts):
            x2 = min(w, x1 + tile_w)
            y2 = min(h, y1 + tile_h)
            ...
            rois.append((
                f"{name_prefix}_{rows}x{cols}_r{row_idx}_c{col_idx}",
                roi,
                x1,
                y1
            ))

            if len(rois) >= LICENSE_PLATE_MAX_SEARCH_ROIS:
                return


def build_full_frame_lpd_rois(frame: np.ndarray) -> List[Tuple[str, np.ndarray, int, int]]:
    """
    Full-frame + long-distance tiled LPD search.
    """
    if frame is None or frame.size == 0:
        return []
    ...
    rois: List[Tuple[str, np.ndarray, int, int]] = [
        ("full_frame", frame, 0, 0)
    ]

    # License plates are usually lower/middle part of camera view.
    base_candidates = [
        ("lower_75", 0, int(h * 0.25), w, h),
        ("lower_60", 0, int(h * 0.40), w, h),
        ("center_80", int(w * 0.10), int(h * 0.15), int(w * 0.90), int(h * 0.92)),
    ]

    for name, x1, y1, x2, y2 in base_candidates:
        x1 = max(0, min(w - 1, x1))
        y1 = max(0, min(h - 1, y1))
        x2 = max(x1 + 1, min(w, x2))
        y2 = max(y1 + 1, min(h, y2))
    ...
    if LICENSE_PLATE_ENABLE_LONG_DISTANCE_TILES:
        _add_grid_lpd_rois(rois, frame, cols=2, rows=2, name_prefix="tile")
        _add_grid_lpd_rois(rois, frame, cols=3, rows=2, name_prefix="tile")

        if w >= 1100:
            _add_grid_lpd_rois(rois, frame, cols=3, rows=3, name_prefix="tile")

    return rois[:LICENSE_PLATE_MAX_SEARCH_ROIS]


def _dedupe_lpd_detections(detections: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    if not detections:
        return []
    ...
    for det in ordered:
        ...
        for prev in kept:
            if box_iou_xyxy(tuple(box), tuple(prev["box"])) >= 0.45:
                duplicate = True
                break
        ...
        if len(kept) >= LICENSE_PLATE_MAX_DETECTIONS:
            break

    return kept

def _expand_lpd_candidate_box(
    box: Tuple[int, int, int, int],
    img_w: int,
    img_h: int,
    ...
    px = max(2, int(round(bw * pad_x_ratio)))
    py = max(2, int(round(bh * pad_y_ratio)))
...
def _is_reasonable_lpd_candidate_box(
    box: Tuple[int, int, int, int],
    img_w: int,
    img_h: int
) -> bool:
    ...
    if aspect < 1.20 or aspect > 8.80:
        return False

    if bw < max(24, int(img_w * 0.025)):
        return False

    if bh < max(9, int(img_h * 0.010)):
        return False

    if area < frame_area * 0.00018:
        return False

    if area > frame_area * 0.22:
        return False

    return True


def _dedupe_lpd_candidate_boxes(
    ...
    return kept[:LICENSE_PLATE_OCR_FALLBACK_MAX_CANDIDATES]


def find_plate_candidates_by_contour(image: np.ndarray) -> List[Tuple[int, int, int, int]]:
    """
    OCR fallback candidate finder.

    This catches cases where YOLO misses:
      - hand-held plate shown to camera
      - far plate
      - odd angle
      - indoor light
      - weak/missing ONNX model
    """
    if image is None or image.size == 0:
        return []
    ...
    # 1) Dark letters on bright plate.
    try:
        ...
        blackhat = cv2.morphologyEx(eq, cv2.MORPH_BLACKHAT, rect_kernel)
        ...
        if max_val > min_val:
            grad_x = ((grad_x - min_val) / (max_val - min_val) * 255.0).astype("uint8")
        else:
            grad_x = np.zeros_like(eq, dtype=np.uint8)
        ...
        thresh = cv2.dilate(thresh, small_kernel, iterations=1)
        ...
        for contour in contours:
            ...
            if _is_reasonable_lpd_candidate_box(box, img_w, img_h):
                candidate_boxes.append(
                    _expand_lpd_candidate_box(box, img_w, img_h, 0.18, 0.30)
                )

    except Exception as exc:
        print(f"[LPD OCR FALLBACK] blackhat candidate error: {exc}", flush=True)

    # 2) Bright rectangular plate finder.
    try:
        for source in (gray, eq):
            blur = cv2.GaussianBlur(source, (5, 5), 0)

            for mode in ("bright", "adaptive"):
                if mode == "bright":
                    thresh = cv2.threshold(
                        blur,
                        0,
                ...
                for contour in contours:
                    ...
                    if not _is_reasonable_lpd_candidate_box(box, img_w, img_h):
                        continue

                    rect_area = float(w * h)
                    contour_area = float(cv2.contourArea(contour))
                    fill_ratio = contour_area / float(max(1.0, rect_area))

                    if fill_ratio < 0.12:
                        continue

                    candidate_boxes.append(
                        _expand_lpd_candidate_box(box, img_w, img_h, 0.14, 0.22)
                    )

    except Exception as exc:
        print(f"[LPD OCR FALLBACK] bright candidate error: {exc}", flush=True)

    # 3) Text region grouping.
    try:
        if hasattr(cv2, "MSER_create"):
            mser = cv2.MSER_create()
            regions, _ = mser.detectRegions(eq)
            ...
            for pts in regions:
                ...
                if h > img_h * 0.30 or w > img_w * 0.60:
                    continue

                text_boxes.append((x, y, x + w, y + h))

            for i, box_a in enumerate(text_boxes):
                ax1, ay1, ax2, ay2 = box_a
                group = [box_a]

                for box_b in text_boxes[i + 1:]:
                    ...
                    horizontal_close = abs(bx1 - ax2) <= max(
                        45,
                        (ax2 - ax1) * 5
                    )
                ...
                if len(group) >= 3:
                    ...
                    if _is_reasonable_lpd_candidate_box(box, img_w, img_h):
                        candidate_boxes.append(
                            _expand_lpd_candidate_box(box, img_w, img_h, 0.22, 0.42)
                        )

    except Exception as exc:
        print(f"[LPD OCR FALLBACK] MSER candidate error: {exc}", flush=True)

    return _dedupe_lpd_candidate_boxes(candidate_boxes)


def detect_license_plates_by_ocr_fallback(frame: np.ndarray) -> List[Dict[str, Any]]:
    """
    OCR-only fallback.

    Runs after YOLO gives no plate. This is the important part for the
    test case where a plate is shown by hand and the plate YOLO model
    does not fire.
    """
    if not LICENSE_PLATE_ENABLE_OCR_FALLBACK:
        return []
    ...
    search_rois = build_full_frame_lpd_rois(frame)

    # Hand-held plates can appear above normal vehicle zone.
    search_rois.extend([
        ("upper_middle_ocr", frame[0:int(frame_h * 0.65), 0:frame_w].copy(), 0, 0),
        ("middle_ocr", frame[int(frame_h * 0.12):int(frame_h * 0.78), 0:frame_w].copy(), 0, int(frame_h * 0.12)),
        ("full_ocr", frame.copy(), 0, 0),
    ])

    seen_roi_names = set()
    unique_search_rois: List[Tuple[str, np.ndarray, int, int]] = []

    for name, roi_img, roi_x, roi_y in search_rois:
        if roi_img is None or roi_img.size == 0:
            continue

        key = (name, int(roi_x), int(roi_y), roi_img.shape[1], roi_img.shape[0])

        if key in seen_roi_names:
            continue

        seen_roi_names.add(key)
        unique_search_rois.append((name, roi_img, roi_x, roi_y))

    for roi_name, roi_img, roi_x, roi_y in unique_search_rois[:max(10, LICENSE_PLATE_MAX_SEARCH_ROIS)]:
        local_boxes = find_plate_candidates_by_contour(roi_img)

        for local_box in local_boxes:
            lx1, ly1, lx2, ly2 = local_box

            lx1 = max(0, min(roi_img.shape[1] - 1, int(lx1)))
            ly1 = max(0, min(roi_img.shape[0] - 1, int(ly1)))
            lx2 = max(lx1 + 1, min(roi_img.shape[1], int(lx2)))
            ly2 = max(ly1 + 1, min(roi_img.shape[0], int(ly2)))

            crop = roi_img[ly1:ly2, lx1:lx2].copy()
            ...
            score = _score_plate_text(plate_text, float(ocr_confidence))

            # Tesseract confidence can be 0 on some Pi builds,
            # so allow strong plate-like mixed text.
            if score < LICENSE_PLATE_OCR_FALLBACK_MIN_SCORE and len(plate_text) < 6:
                continue

            gx1 = max(0, min(frame_w - 1, roi_x + lx1))
            gy1 = max(0, min(frame_h - 1, roi_y + ly1))
            gx2 = max(gx1 + 1, min(frame_w - 1, roi_x + lx2))
            gy2 = max(gy1 + 1, min(frame_h - 1, roi_y + ly2))

            detections.append({
                "box": (gx1, gy1, gx2, gy2),
                "confidence": round(max(0.22, min(0.89, float(ocr_confidence) / 100.0)), 4),
                "plate_text": plate_text,
                "vehicle_number": plate_text,
                "ocr_confidence": round(float(ocr_confidence), 2),
                "crop": crop,
                "variant": "ocr_contour_fallback",
                "search_roi": roi_name,
                "vehicle_index": 0,
                "vehicle_label": "OCRFallback",
                "source": "ocr_fallback",
                "text_score": round(score, 2),
            })

    return _dedupe_lpd_detections(detections)
...
def detect_license_plates_for_vehicles(
    ...
    Detection order:
      1. YOLO on vehicle crops when vehicle boxes exist.
      2. YOLO on full-frame / tiled ROIs.
      3. OCR contour fallback when YOLO misses.

    The OCR fallback is required for real-world testing where a plate is
    shown directly to the camera or where the ONNX model is weak/missing.
    """
    if frame is None or frame.size == 0:
        return []
    ...
    # 1) YOLO on vehicle crops if vehicle boxes are available.
    if lpd_net is not None and vehicles:
        for vehicle_index, vehicle in enumerate(vehicles, start=1):
            ...
            vx1, vy1, vx2, vy2 = expand_pixel_box(
                tuple(box),
                frame_w,
                frame_h,
                LICENSE_PLATE_VEHICLE_PAD_RATIO
            )
            ...
            for roi_name, roi_img, roi_x, roi_y in _vehicle_lpd_rois(vehicle_crop):
                detections = detect_license_plates_in_image(roi_img)
                ...
                for det in detections:
                    ...
                    det["vehicle_index"] = vehicle_index
                    det["vehicle_label"] = vehicle.get(
                        "display_label",
                        vehicle.get("label", "Vehicle")
                    )
                    det["search_roi"] = roi_name
                    det["source"] = det.get("source", "yolo_vehicle_crop")

                    all_detections.append(det)

    # 2) YOLO full-frame / tiled fallback.
    if lpd_net is not None and (not all_detections) and LICENSE_PLATE_ENABLE_FULL_FRAME_FALLBACK:
        for roi_name, roi_img, roi_x, roi_y in build_full_frame_lpd_rois(frame):
            detections = detect_license_plates_in_image(roi_img)
            ...
            for det in detections:
                ...
                det["vehicle_index"] = 0
                det["vehicle_label"] = "FullFrame"
                det["search_roi"] = roi_name
                det["source"] = det.get("source", "yolo_full_frame")

                all_detections.append(det)

    all_detections = _dedupe_lpd_detections(all_detections)

    # 3) OCR contour fallback.
    # Run when YOLO misses completely OR all YOLO detections have UNKNOWN OCR.
    should_run_ocr_fallback = (
        not all_detections or
        all(str(det.get("plate_text", "UNKNOWN")) == "UNKNOWN" for det in all_detections)
    )

    if should_run_ocr_fallback:
        fallback_detections = detect_license_plates_by_ocr_fallback(frame)
...
def draw_license_plate_annotations(
    frame: np.ndarray,
    plates: List[Dict[str, Any]]
) -> np.ndarray:
    """
    Draw LPD boxes on image before saving/uploading to SFTP.
    """
    out = frame.copy()
    ...
    for idx, plate in enumerate(plates, start=1):
        ...
        plate_text = sanitize_plate_text(
            plate.get("plate_text") or "UNKNOWN"
        )

        conf = float(plate.get("confidence", 0.0))

        label = f"LPD {idx}: {plate_text} {conf:.2f}"
...
def async_upload_license_plate_image(
    local_path: Path,
    remote_name: str,
    event_time: dt.datetime,
    plate_text: str,
    ...
    """
    Upload LPD image to SFTP and publish MQTT event.
    ...
    def worker() -> None:
        remote_dir = SFTP_LICENSE_PLATE_DIR
        remote_path = f"{remote_dir.rstrip('/')}/{remote_name}"
        ...
        try:
            ...
            try:
                mqtt_publish(
                    MQTT_LICENSE_PLATE_EVENT_TOPIC,
                    payload
                )
            except Exception as mqtt_exc:
                update_state(
            ...
            add_event(
                "license_plate_uploaded",
                payload
            )

            print(
                "[LPD OK] Uploaded "
                f"vehicle_number={plate_text} "
                f"file={remote_path}",
                flush=True
            )
            ...
            print(
                "[LPD ERROR] Upload failed "
                f"file={remote_name} error={error_text}",
                flush=True
            )
...
def handle_license_plate_event(
    ...
    """
    Independent LPD handler.

    Works with:
      - YOLO license_plate_yolo.onnx when available
      - OCR contour fallback when YOLO misses or model is missing
      - vehicles=[] for fully separate LPD
    """
    global last_lpd_upload_ts
    global last_lpd_upload_plate_text

    if not is_feature_enabled("license_plate_detection_enabled"):
        return

    if frame is None or frame.size == 0:
        return

    _lpd_lock_acquired = False
    if not lpd_event_lock.acquire(blocking=False):
        print("[LPD SKIP] Previous LPD scan still running", flush=True)
        return
    _lpd_lock_acquired = True

    try:
        # Prefer high-resolution frame for long-distance LPD.
        lpd_frame = get_latest_lpd_frame_copy()

        if lpd_frame is None or lpd_frame.size == 0:
            lpd_frame = frame

        lpd_vehicles = list(vehicles or [])

        # If vehicle boxes came from smaller process frame, rescale them.
        if lpd_vehicles and frame is not None and lpd_frame is not None:
            try:
                if lpd_frame.shape[:2] != frame.shape[:2]:
                    lpd_vehicles = scale_vehicle_boxes_for_frame(
                        lpd_vehicles,
                        frame.shape,
                        lpd_frame.shape
                    )
        ...
        if lpd_net is None:
            print(
                "[LPD WARN] license_plate_yolo.onnx missing. "
                "Trying OCR contour fallback.",
                flush=True
            )

            add_event(
                "license_plate_model_missing_ocr_fallback",
                {
                    "time": event_time.strftime("%Y-%m-%d %H:%M:%S"),
                    "model": str(MODEL_DIR / "license_plate_yolo.onnx"),
                    "fallback": "ocr_contour",
                }
            )

        plates = detect_license_plates_for_vehicles(
            lpd_frame,
            lpd_vehicles
        )
        ...
        if not plates:
            update_state(
                lambda s: s.setdefault("last_license_plate", {}).update({
                    "detected": False,
                    "time": event_time.strftime("%Y-%m-%d %H:%M:%S"),
                    ...
                    "plate_confidence": 0.0,
                    "ocr_confidence": 0.0,
                    "model_available": bool(lpd_net is not None),
                    "ocr_available": bool(pytesseract is not None),
                    "frame_width": int(lpd_frame.shape[1]) if lpd_frame is not None else 0,
                    "frame_height": int(lpd_frame.shape[0]) if lpd_frame is not None else 0,
                }),
                force_flush=True
            )

            add_event(
                "license_plate_not_detected",
                {
                    "time": event_time.strftime("%Y-%m-%d %H:%M:%S"),
                    "vehicle_count": len(lpd_vehicles),
                    "model_available": bool(lpd_net is not None),
                    "ocr_available": bool(pytesseract is not None),
                    "frame_width": int(lpd_frame.shape[1]) if lpd_frame is not None else 0,
                    "frame_height": int(lpd_frame.shape[0]) if lpd_frame is not None else 0,
            ...
            print(
                "[LPD INFO] No plate detected "
                f"vehicles={len(lpd_vehicles)} "
                f"model_available={bool(lpd_net is not None)} "
                f"ocr_available={bool(pytesseract is not None)} "
                f"frame={lpd_frame.shape[1]}x{lpd_frame.shape[0]}",
                flush=True
            )
        ...
        plate_text = sanitize_plate_text(
            best_plate.get("plate_text") or "UNKNOWN"
        )

        plate_confidence = round(
            float(best_plate.get("confidence", 0.0)),
            4
        )

        ocr_confidence = round(
            float(best_plate.get("ocr_confidence", 0.0)),
            2
        )

        source = str(best_plate.get("source", "unknown"))
        search_roi = best_plate.get("search_roi")
        detector_variant = best_plate.get("variant")

        now_mono = time.time()

        same_plate_recently_uploaded = (
            plate_text == last_lpd_upload_plate_text and
            (now_mono - last_lpd_upload_ts) < LICENSE_PLATE_REPEAT_UPLOAD_COOLDOWN_SEC
        )

        if same_plate_recently_uploaded:
            update_state(
                lambda s: s.setdefault("last_license_plate", {}).update({
                    "detected": True,
                    "time": event_time.strftime("%Y-%m-%d %H:%M:%S"),
                    ...
                    "plate_confidence": plate_confidence,
                    "ocr_confidence": ocr_confidence,
                    "status": "detected_duplicate_cooldown",
                    "plate_count": len(plates),
                    "frame_width": int(lpd_frame.shape[1]),
                    "frame_height": int(lpd_frame.shape[0]),
            ...
            print(
                "[LPD SKIP] Duplicate cooldown "
                f"plate={plate_text} source={source}",
                flush=True
            )

            return

        marked = draw_license_plate_annotations(
            lpd_frame,
            plates
        )

        image_name = build_license_plate_image_name(
            event_time,
            plate_text
        )

        rel_img, path = save_capture(
            marked,
            image_name,
            garbage=False
        )

        remote_path = f"{SFTP_LICENSE_PLATE_DIR.rstrip('/')}/{image_name}"

        last_lpd_upload_ts = now_mono
        last_lpd_upload_plate_text = plate_text

        status_prefix = "detected"
        if source == "ocr_fallback":
            status_prefix = "detected_ocr_fallback"
        ...
        update_state(
            lambda s: s.update({
                "last_license_plate": {
                    ...
                    "image_uploaded_path": remote_path,
                    "status": f"{status_prefix}_uploading",
                    "plate_count": len(plates),
                    "frame_width": int(lpd_frame.shape[1]),
                    "frame_height": int(lpd_frame.shape[0]),
                    "search_roi": search_roi,
        ...
        async_upload_license_plate_image(
            local_path=path,
        ...
        print(
            "[LPD DETECTED] "
            f"vehicle_number={plate_text} "
            f"plate_conf={plate_confidence} "
            f"ocr_conf={ocr_confidence} "
            f"plates={len(plates)} "
            f"frame={lpd_frame.shape[1]}x{lpd_frame.shape[0]} "
            f"roi={search_roi} "
            f"variant={detector_variant} "
            f"source={source}",
            flush=True
        )
    finally:
        if _lpd_lock_acquired:
            try:
                lpd_event_lock.release()
            except Exception:
                pass
...
def reader_loop() -> None:
    ...
    while not stop_event.is_set():
        ...
        if not cap.isOpened():
            update_state(lambda s: s["system"].update({
                "camera_connected": False,
                "last_error": "Unable to open RTSP stream"
            }))
            time.sleep(3)
            continue
        ...
        while not stop_event.is_set():
            ...
            # Event buffer is throttled by ROLLING_BUFFER_TARGET_FPS internally.
...
def detector_loop() -> None:
    ...
    # Independent LPD scheduler.
    # LPD no longer waits for vehicle event.
    last_lpd_scan_ts = 0.0
    ...
    while not stop_event.is_set():
        ...
        license_plate_detection_enabled = bool(flags.get("license_plate_detection_enabled", True))
        ...
        if (
            not person_detection_enabled and
            not vehicle_detection_enabled and
            not license_plate_detection_enabled
        ):
            time.sleep(0.05)
            continue
        ...
        try:
            ...
            # ---------------- INDEPENDENT LPD FLOW ----------------
            # This is separate from vehicle detection.
            # It scans full frame + long-distance tiles.
            if license_plate_detection_enabled:
                now_lpd_ts = time.time()

                should_scan_lpd = (
                    (now_lpd_ts - last_lpd_scan_ts) >= LICENSE_PLATE_SCAN_INTERVAL_SEC
                )

                if should_scan_lpd and not lpd_event_lock.locked():
                    last_lpd_scan_ts = now_lpd_ts

                    # Use process frame directly — avoids heavy rolling buffer decode
                    lpd_frame = frame.copy()

                    threading.Thread(
                        target=handle_license_plate_event,
                        args=(
                            lpd_frame,
                            [],
                            now_dt(),
            ...
            if vehicle_detection_enabled:
                ...
                if vehicle_detection_mode == "moving":
                    # Moving vehicle event should trigger quickly.
                    # It should not wait for static stay time.
                    confirmed_vehicle = (
                ...
                update_state(lambda s: s.update({
                    "last_vehicle": {
                        ...
                        "stable_required_sec": vehicle_static_stay_sec if vehicle_detection_mode == "static" else 0.0,
                ...
                if should_fire_vehicle_event:
                    ...
                    # LPD is now independent.
                    # Do NOT trigger LPD from vehicle event here.
                    # Independent LPD runs in detector_loop using full-frame/high-resolution search.
...
def normalize_command(raw: str) -> str:
    ...
    mapping = {
        ...
        "license plate detection enable": LICENSE_PLATE_DETECTION_ENABLE_COMMAND,
        "enable license plate detection": LICENSE_PLATE_DETECTION_ENABLE_COMMAND,
        "lpd enable": LICENSE_PLATE_DETECTION_ENABLE_COMMAND,
        "enable lpd": LICENSE_PLATE_DETECTION_ENABLE_COMMAND,

        "license plate detection disable": LICENSE_PLATE_DETECTION_DISABLE_COMMAND,
        "disable license plate detection": LICENSE_PLATE_DETECTION_DISABLE_COMMAND,
        "lpd disable": LICENSE_PLATE_DETECTION_DISABLE_COMMAND,
        "disable lpd": LICENSE_PLATE_DETECTION_DISABLE_COMMAND,

        "license plate ocr enable": LICENSE_PLATE_OCR_ENABLE_COMMAND,
        "enable license plate ocr": LICENSE_PLATE_OCR_ENABLE_COMMAND,
        "lpd ocr enable": LICENSE_PLATE_OCR_ENABLE_COMMAND,
        "enable lpd ocr": LICENSE_PLATE_OCR_ENABLE_COMMAND,

        "license plate ocr disable": LICENSE_PLATE_OCR_DISABLE_COMMAND,
        "disable license plate ocr": LICENSE_PLATE_OCR_DISABLE_COMMAND,
        "lpd ocr disable": LICENSE_PLATE_OCR_DISABLE_COMMAND,
        "disable lpd ocr": LICENSE_PLATE_OCR_DISABLE_COMMAND,
    }
...
def handle_command(payload: Dict[str, Any]) -> None:
    ...
    feature_command_map = {
        ...
        LICENSE_PLATE_DETECTION_DISABLE_COMMAND: (
            "license_plate_detection_enabled",
            False,
            "license_plate_detection_disabled"
        ),
        LICENSE_PLATE_OCR_ENABLE_COMMAND: (
            "license_plate_ocr_enabled",
            True,
            "license_plate_ocr_enabled"
        ),
        LICENSE_PLATE_OCR_DISABLE_COMMAND: (
...
def mjpeg_stream():
    min_gap = 1.0 / max(float(LIVE_STREAM_TARGET_FPS), 1.0)
    ...
    while not stop_event.is_set():
        ...
        frame = resize_frame_to_width(frame, LIVE_FRAME_WIDTH)
...
def render_dashboard(initial_tab: str = "live", geofence_page_only: bool = False):
    ...
    # Safety for old state.json files created before LPD was added.
    state.setdefault("last_license_plate", {
        "detected": False,
        "time": None,
        "vehicle_number": None,
        "plate_text": None,
...
def video_feed_cloudflare():
    ...
    return Response(
        mjpeg_stream_from_rtsp(
            CLOUDFLARE_LIVE_RTSP_URL,
...
@app.route("/api/license_plate_detect", methods=["POST"])
def api_license_plate_detect_manual():
    """
    Manual dashboard/API LPD test.
    Uses independent high-resolution full-frame LPD.
    """
    if not is_feature_enabled("license_plate_detection_enabled"):
        return jsonify({
            "ok": False,
            "error": "license_plate_detection_disabled"
        }), 400

    frame = get_latest_lpd_frame_copy()
    ...
    return jsonify({
        "ok": True,
        "message": "Manual independent LPD started. Check Last License Plate tab.",
        "time": event_time.strftime("%Y-%m-%d %H:%M:%S")
    })
...
def api_state():
    ...
    payload.setdefault("last_license_plate", {
        "detected": False,
        "time": None,
        "vehicle_number": None,
        "plate_text": None,
    ...
    payload["sftp_license_plate_dir"] = SFTP_LICENSE_PLATE_DIR
    ...
    payload["last_license_plate"] = payload.get("last_license_plate", {})
