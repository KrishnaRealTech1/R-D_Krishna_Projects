...
DEFAULT_RTSP_URL="rtsp://admin:Realtech@1@192.168.22.225:554/Streaming/Channels/101"
DEFAULT_CLOUDFLARE_LIVE_RTSP_URL="rtsp://admin:Realtech@1@192.168.22.225:554/Streaming/Channels/102"
DEFAULT_PI_IP="192.168.22.101"
DEFAULT_ROUTER_IP="192.168.22.1"
DEFAULT_CAMERA_IP="192.168.22.225"
DEFAULT_DEVICE_ID="RTGCAMx"
DEFAULT_CAMERA_NAME="$DEFAULT_DEVICE_ID"
DEFAULT_CAMERA_CONFIG_URL="http://192.168.22.225"

DEFAULT_SUBDOMAIN_NAME="$(echo "$DEFAULT_DEVICE_ID" | tr '[:upper:]' '[:lower:]')"
...
DEFAULT_MQTT_GARBAGE_TOPIC="${DEFAULT_DEVICE_ID}/garbage_level"
DEFAULT_MQTT_CAMERA_EVENT_TOPIC="${DEFAULT_DEVICE_ID}/camera_event"
DEFAULT_MQTT_COMMAND_TOPIC="${DEFAULT_DEVICE_ID}/command"
DEFAULT_MQTT_RESPONSE_TOPIC="${DEFAULT_DEVICE_ID}/device_response"
...
DEFAULT_SFTP_DEFAULT_AUDIO_DIR="/ftp/files/Default_Audio"
...
echo "=================================================="
echo " G-Cam Full Clean Reinstall - V8.0 MERGED / Pi 5 1GB"
echo " Version 8.0 DVR + Storage Playback + Permanent Cloudflare"
echo " + Person + Vehicle + Garbage AI/Normal Detection"
echo " + MQTT/Default Audio + Live Talk + Device Status"
echo " + White / Red Web Interface"
echo "=================================================="
echo
...
# Derive all device-specific defaults from the Device ID entered above.
# Example: RTGCAM43 -> camera RTGCAM43, subdomain rtgcam43,
# MQTT RTGCAM43/... and SFTP /ftp/files/RTGCAM43/...
DEFAULT_CAMERA_NAME="$DEVICE_ID"
DEFAULT_SUBDOMAIN_NAME="$(echo "$DEVICE_ID" | tr '[:upper:]' '[:lower:]')"
DEFAULT_MQTT_GARBAGE_TOPIC="${DEVICE_ID}/garbage_level"
DEFAULT_MQTT_CAMERA_EVENT_TOPIC="${DEVICE_ID}/camera_event"
DEFAULT_MQTT_COMMAND_TOPIC="${DEVICE_ID}/command"
DEFAULT_MQTT_RESPONSE_TOPIC="${DEVICE_ID}/device_response"
DEFAULT_SFTP_BASE_DIR="/ftp/files/${DEVICE_ID}"
DEFAULT_SFTP_PERSON_DIR="/ftp/files/${DEVICE_ID}/Person"
DEFAULT_SFTP_PERSON_VIDEO_DIR="/ftp/files/${DEVICE_ID}/Video/Person_Video"
DEFAULT_SFTP_VEHICLE_DIR="/ftp/files/${DEVICE_ID}/Vehicle"
DEFAULT_SFTP_VEHICLE_VIDEO_DIR="/ftp/files/${DEVICE_ID}/Video/Vehicle_Video"
DEFAULT_SFTP_GARBAGE_DIR="/ftp/files/${DEVICE_ID}/Garbage"
DEFAULT_SFTP_AUDIO_DIR="/ftp/files/${DEVICE_ID}/Audio"
...
echo "IMPORTANT: Enter REAL LINUX PATH like /ftp/files/${DEVICE_ID}"
...
read -rp "Paste Default Audio SFTP Path [${DEFAULT_SFTP_DEFAULT_AUDIO_DIR}]: " SFTP_DEFAULT_AUDIO_DIR
SFTP_DEFAULT_AUDIO_DIR="${SFTP_DEFAULT_AUDIO_DIR:-$DEFAULT_SFTP_DEFAULT_AUDIO_DIR}"
...
if [[ "${SFTP_GARBAGE_DIR}" != /* ]]; then
  SFTP_GARBAGE_DIR="/${SFTP_GARBAGE_DIR}"
fi
SFTP_GARBAGE_DIR="${SFTP_GARBAGE_DIR%/}"

if [[ "${SFTP_AUDIO_DIR}" != /* ]]; then
  SFTP_AUDIO_DIR="/${SFTP_AUDIO_DIR}"
fi
SFTP_AUDIO_DIR="${SFTP_AUDIO_DIR%/}"

if [[ "${SFTP_DEFAULT_AUDIO_DIR}" != /* ]]; then
  SFTP_DEFAULT_AUDIO_DIR="/${SFTP_DEFAULT_AUDIO_DIR}"
fi
SFTP_DEFAULT_AUDIO_DIR="${SFTP_DEFAULT_AUDIO_DIR%/}"
...
sudo mkdir -p "$APP_DIR"/{assets,data,logs,models,files,files/garbage,files/audio,files/default_audio,files/video,files/recordings,tmp,templates}
...
echo "[10/17] Download person/vehicle model + install garbage model..."
cd "$APP_DIR/models"
...
# Garbage AI model can be supplied by the self-contained GCAM wrapper.
# Fallback to the legacy Pi-home path for direct script execution.
GARBAGE_MODEL_SOURCE="${GCAM_EMBEDDED_GARBAGE_MODEL:-$PI_HOME/garbage_yolo.onnx}"
if [[ ! -f "$GARBAGE_MODEL_SOURCE" ]]; then
  echo "ERROR: Garbage AI model is unavailable: $GARBAGE_MODEL_SOURCE" >&2
  echo "Run the self-contained GCAM binary, or place garbage_yolo.onnx in the Pi user home." >&2
  exit 1
fi
install -m 644 "$GARBAGE_MODEL_SOURCE" "$APP_DIR/models/garbage_yolo.onnx"
...
cat > "$APP_DIR/data/config.json" <<JSON
{
  ...
  "software_version": "Version 8.0-Edge-AI-Upgrade",
  "software_features": "Version 8.0 DVR/Playback/Cloudflare + Person/Vehicle + Garbage AI/Normal + MQTT Audio + Live Talk + White/Red UI",
  "router_reboot_schedule": {
    "time_1": "$ROUTER_REBOOT_TIME_1",
    "time_2": "$ROUTER_REBOOT_TIME_2"
  },
  "garbage_interval_sec": 3600,
  "garbage_detection_mode": "object_ai",
  "garbage_object_detection_enabled": true,
  ...
  "feature_flags": {
    ...
    "sd_playback_recording_enabled": true
  },
  "mqtt": {
    "host": "$MQTT_HOST",
    "port": $MQTT_PORT,
  ...
  "sftp": {
    ...
    "default_audio_dir": "$SFTP_DEFAULT_AUDIO_DIR"
  },
  "battery": {
    "mode": "$BATTERY_MODE",
...
async function startTalk(event) {
  ...
  try {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      throw new Error('Microphone access requires HTTPS or localhost. Open Live Talk using the secure Cloudflare URL.');
    }
    ...
    setStatus('Start failed: ' + (err.message || err), 'bad');
    if (mediaStream) {
...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <style>
    :root{--bg:#ffffff;--card:#ffffff;--line:#fecaca;--text:#1f1f1f;--muted:#6b7280;--accent:#dc2626;--ok:#dc2626;--bad:#ef4444;--warn:#f59e0b}
    *{box-sizing:border-box}
    body{margin:0;background:#ffffff;color:var(--text);font-family:Arial,Helvetica,sans-serif;min-height:100vh}
    ...
    input{background:#fff5f5;color:var(--text)}
    button{background:var(--accent);color:#000000;font-weight:800;cursor:pointer;min-width:140px}
    video{display:block;width:100%;max-height:70vh;background:#ffffff;border:1px solid var(--line);border-radius:12px}
    .ok{color:#8bffb2}.bad{color:#ff9b9b}.warn{color:#ffd07f}.mono{font-family:Consolas,monospace;word-break:break-all}
    .grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}
    .stat{background:#fff5f5;border:1px solid var(--line);border-radius:12px;padding:12px}
    .stat b{display:block;font-size:20px;margin-top:5px}
    .dates{display:flex;gap:7px;flex-wrap:wrap;margin-top:8px}.date-pill{padding:6px 9px;border-radius:999px;background:#fff5f5;border:1px solid var(--line);font-size:12px}
    .range-row{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:10px}
    .range-box{background:#fff5f5;border:1px solid var(--line);border-radius:10px;padding:10px 12px}
...
<head>
  ...
  <style>
    *{box-sizing:border-box}
    html,body{margin:0;width:100%;height:100%;background:#ffffff;overflow:hidden;font-family:Arial,Helvetica,sans-serif}
    #stage{position:fixed;inset:0;background:#ffffff;touch-action:manipulation}
    .playback-video{position:absolute;inset:0;display:block;width:100%;height:100%;object-fit:contain;background:#ffffff;visibility:hidden}
    .playback-video.active{visibility:visible}
    #shade{position:absolute;left:0;right:0;bottom:0;height:150px;pointer-events:none;background:linear-gradient(transparent,rgba(255,255,255,.96))}
    #controls{position:absolute;left:0;right:0;bottom:0;padding:12px 16px 14px;color:#b91c1c;z-index:20}
    #seekRow{display:grid;grid-template-columns:auto 1fr auto;gap:10px;align-items:center;margin-bottom:9px}
    #seekBar{width:100%;accent-color:#dc2626;cursor:pointer}
    .time{font-size:12px;white-space:nowrap;color:#b91c1c;font-weight:700}
    #buttonRow{display:flex;align-items:center;gap:8px;flex-wrap:wrap}
    .ctl{appearance:none;border:1px solid #dc2626;background:#ffffff;color:#b91c1c;border-radius:7px;min-height:38px;padding:7px 11px;font-size:14px;font-weight:700;cursor:pointer}
    .ctl:hover{background:#fff5f5}
    .ctl.active{background:#dc2626;color:#ffffff;border-color:#dc2626}
    #playPause{min-width:54px;font-size:18px}
    #speedSelect{min-width:76px}
    #status{margin-left:auto;font-size:12px;color:#b91c1c;max-width:42vw;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-weight:700}
    #playbackError{position:fixed;inset:0;display:none;align-items:center;justify-content:center;padding:24px;background:#ffffff;color:#b91c1c;font:16px Arial,sans-serif;text-align:center}
...
<head>
  <meta charset="utf-8">
  <title>G-Cam Login</title>
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <style>
    *{box-sizing:border-box}
    body{
      margin:0;min-height:100vh;display:flex;align-items:center;justify-content:center;
      font-family:Arial,Helvetica,sans-serif;background:#ffffff;color:#000000;padding:18px
    }
    .login-card{
      width:min(420px,100%);background:#fff5f5;border:1px solid #fecaca;border-radius:18px;
      padding:24px;box-shadow:0 18px 45px rgba(0,0,0,.18)
    }
    h1{margin:0 0 8px;font-size:26px}
    .muted{font-size:13px;color:#555;margin-bottom:18px}
    label{display:block;font-size:13px;font-weight:700;margin-bottom:7px}
    input{
      width:100%;padding:12px;border:1px solid #fca5a5;border-radius:10px;
      background:#ffffff;color:#000000;font-size:16px;outline:none
    }
    button{
      width:100%;margin-top:14px;padding:12px;border:0;border-radius:10px;
      background:#dc2626;color:#ffffff;font-weight:800;font-size:15px;cursor:pointer
    }
    .error{
      margin-top:12px;padding:10px;border:1px solid #fca5a5;border-radius:9px;
      background:#fff1f2;color:#b91c1c;font-size:13px
    }
  </style>
</head>
<body>
  <form class="login-card" method="post" action="/login">
    <h1>G-Cam Dashboard Login</h1>
    <div class="muted">Enter dashboard password to continue.</div>
...
<head>
  ...
  <style>
    :root{
      --bg:#ffffff;--card:#ffffff;--card2:#fff5f5;--line:#fecaca;--text:#1f1f1f;--muted:#6b7280;
      --accent:#dc2626;--high:#ef4444;--med:#f59e0b;--low:#22c55e;--shadow:0 20px 45px rgba(0,0,0,.25);
    }
    *{box-sizing:border-box}
    ...
    .header{display:flex;justify-content:space-between;gap:20px;align-items:center;background:#ffffff;border:1px solid var(--line);border-radius:18px;box-shadow:var(--shadow);padding:20px 24px;margin-bottom:18px}
...
<div class="wrap">
  ...
  <div class="tabs">
    ...
    <a href="/logout" class="tab-btn" style="text-decoration:none;display:inline-flex;align-items:center;justify-content:center">
      Logout
    </a>
    <button class="tab-btn" onclick="showTab('device', this)">Device Health</button>
    <a class="tab-btn" href="/live_talk" target="_blank" rel="noopener" style="text-decoration:none">Live Talk</a>
  </div>
      ...
      <div class="info">
        ...
        <div class="info-box">
          ...
          <button id="vehicleModeToggle" onclick="toggleVehicleDetectionMode()"
            style="margin-top:10px;background:var(--accent);color:#ffffff;border:none;border-radius:10px;padding:10px 14px;font-weight:700;cursor:pointer;font-size:14px">
            🚗 Switch Mode
          </button>
  ...
  <div id="tab-controls" class="panel">
    <div style="margin-bottom:14px">
      <strong>Dashboard Controls</strong> — control detection, recordings, SD-card live playback recording,
    ...
    <div class="controls-grid">
      ...
      <div class="control-card">
        <h4>SD Card Playback</h4>
        <div class="control-title">💾 Live Playback Recording</div>
        <div id="ctrlSdPlaybackStatus" class="control-status">Loading...</div>
        <div class="control-actions">
          <button class="control-btn ok" onclick="controlSetFeature('sd_playback_recording_enabled', true)">Enable</button>
          <button class="control-btn danger" onclick="controlSetFeature('sd_playback_recording_enabled', false)">Disable</button>
        </div>
        <div style="font-size:11px;margin-top:8px;line-height:1.4;color:#000000">
          Controls continuous SD-card recording used by Playback.
        </div>
      </div>
  ...
  <div id="tab-device" class="panel">
    <div class="panel-grid">
      <div class="info">
        ...
        <div class="info-box"><h4>Battery Voltage</h4><div class="big" id="batteryVoltage">{{ state.system.battery_voltage or "Unavailable" }} V</div></div>
        <div class="info-box"><h4>Battery Current</h4><div class="big" id="batteryCurrent">{{ state.system.battery_current_a or "Unavailable" }} A</div></div>
        <div class="info-box"><h4>Battery Power</h4><div class="big" id="batteryPower">{{ state.system.battery_power_w or "Unavailable" }} W</div></div>
      </div>
      <div class="info">
    ...
    <div style="display:flex;gap:14px;align-items:flex-start;flex-wrap:wrap">
      ...
      <div style="display:grid;gap:12px;min-width:230px">
        ...
        <button onclick="geoSave()"
          style="background:var(--accent);color:#ffffff;border:none;border-radius:10px;padding:12px 18px;font-weight:700;cursor:pointer;font-size:15px">
          ✅ Save Polygon
        </button>
...
const CONTROL_STATUS_IDS = {
  ...
  sd_playback_recording_enabled: 'ctrlSdPlaybackStatus'
};
...
async function refreshState(){
  try{
    ...
    // Safe dashboard updates: guard optional DOM elements during refresh.
...
from flask import Flask, Response, jsonify, make_response, render_template, send_from_directory, request, stream_with_context, session, redirect, url_for
...
# V8.0 object-entry filtering:
# Reference subtraction is symmetric, so an object being REMOVED can otherwise
# look exactly like an object being ADDED.  These checks keep only changes where
# the current frame has stronger object/foreground evidence than the reference.
GARBAGE_ADDITION_MIN_CHANGED_RATIO = 0.055
GARBAGE_ADDITION_MIN_CONTRAST_GAIN = 5.0
GARBAGE_ADDITION_MIN_EDGE_GAIN = 0.004
GARBAGE_ADDITION_TEXTURE_RATIO = 0.90
...
def set_feature_flag(flag_name: str, enabled: bool, source: str = "mqtt command") -> Dict[str, bool]:
    ...
    if flag_name == "sd_playback_recording_enabled" and not enabled:
        try:
...
DASHBOARD_PASSWORD = "7799"
app.secret_key = hashlib.sha256(
    f"gcam-dashboard-session:{DEVICE_ID}:v8".encode("utf-8")
).hexdigest()
app.config.update(
...
SOFTWARE_VERSION = str(CONFIG.get("software_version", "Version 8.0")).strip()
...
SFTP_DEFAULT_AUDIO_DIR = CONFIG["sftp"].get("default_audio_dir", "/ftp/files/Default_Audio")
...
def init_models() -> None:
    ...
    # Garbage object detector (merged from the older working build)
...
def garbage_change_is_added_object(
    curr_roi: np.ndarray,
    ref_roi: np.ndarray,
    box: Tuple[int, int, int, int]
) -> bool:
    """
    Return True only when a changed region looks like an object ADDED to the
    current image.  Pure disappearance/removal should return False.

    Normal absdiff cannot distinguish add vs remove.  We therefore compare:
      * amount of local change
      * current-vs-reference edge density
      * current-vs-reference texture
      * foreground contrast against the local surrounding ring
    """
    try:
        roi_h, roi_w = curr_roi.shape[:2]
        clipped = _clip_box(box, roi_w, roi_h)
        if clipped is None:
            return False
        ...
        curr_patch = curr_roi[y1:y2, x1:x2]
        ref_patch = ref_roi[y1:y2, x1:x2]
        if curr_patch.size == 0 or ref_patch.size == 0:
            return False

        curr_gray = cv2.cvtColor(curr_patch, cv2.COLOR_BGR2GRAY)
        ref_gray = cv2.cvtColor(ref_patch, cv2.COLOR_BGR2GRAY)

        # Region must actually differ from the saved clean/reference image.
        local_delta = cv2.absdiff(curr_gray, ref_gray)
        _, local_changed = cv2.threshold(local_delta, 22, 255, cv2.THRESH_BINARY)
        changed_ratio = float(np.count_nonzero(local_changed)) / float(max(local_changed.size, 1))
        if changed_ratio < GARBAGE_ADDITION_MIN_CHANGED_RATIO:
            return False

        curr_edges = cv2.Canny(curr_gray, 45, 135)
        ref_edges = cv2.Canny(ref_gray, 45, 135)
        curr_edge_density = float(np.count_nonzero(curr_edges)) / float(max(curr_edges.size, 1))
        ref_edge_density = float(np.count_nonzero(ref_edges)) / float(max(ref_edges.size, 1))
        edge_gain = curr_edge_density - ref_edge_density

        curr_var = float(curr_gray.var())
        ref_var = float(ref_gray.var())

        # Compare the candidate centre with a surrounding ring.  When an object
        # is removed, the current patch normally becomes MORE like its local
        # background; when an object is added it normally becomes LESS like it.
        bw = max(2, x2 - x1)
        bh = max(2, y2 - y1)
        pad_x = max(6, int(bw * 0.35))
        pad_y = max(6, int(bh * 0.35))

        ex1 = max(0, x1 - pad_x)
        ey1 = max(0, y1 - pad_y)
        ex2 = min(roi_w, x2 + pad_x)
        ey2 = min(roi_h, y2 + pad_y)

        curr_outer = curr_roi[ey1:ey2, ex1:ex2]
        ref_outer = ref_roi[ey1:ey2, ex1:ex2]

        def centre_ring_contrast(outer: np.ndarray, inner_box: Tuple[int, int, int, int]) -> float:
            if outer.size == 0:
                return 0.0

            ox1, oy1, ox2, oy2 = inner_box
            gray = cv2.cvtColor(outer, cv2.COLOR_BGR2GRAY)
            mask = np.ones(gray.shape[:2], dtype=np.uint8) * 255

            rx1 = max(0, ox1 - ex1)
            ry1 = max(0, oy1 - ey1)
            rx2 = min(gray.shape[1], ox2 - ex1)
            ry2 = min(gray.shape[0], oy2 - ey1)
            if rx2 > rx1 and ry2 > ry1:
                mask[ry1:ry2, rx1:rx2] = 0

            ring_values = gray[mask > 0]
            if ring_values.size < 20:
                return 0.0

            ring_median = float(np.median(ring_values))
            centre = gray[ry1:ry2, rx1:rx2]
            if centre.size == 0:
                return 0.0
            return float(np.mean(np.abs(centre.astype(np.float32) - ring_median)))

        curr_contrast = centre_ring_contrast(curr_outer, (x1, y1, x2, y2))
        ref_contrast = centre_ring_contrast(ref_outer, (x1, y1, x2, y2))
        contrast_gain = curr_contrast - ref_contrast

        # Strong evidence that the CURRENT frame gained an object.
        if contrast_gain >= GARBAGE_ADDITION_MIN_CONTRAST_GAIN:
            return True

        if edge_gain >= GARBAGE_ADDITION_MIN_EDGE_GAIN and curr_var >= (ref_var * GARBAGE_ADDITION_TEXTURE_RATIO):
            return True

        # Conservative fallback for dark/low-edge objects: accept only when the
        # current patch did not become simpler than the reference patch.
        if changed_ratio >= 0.18 and curr_var >= (ref_var * 1.05):
            return True

        return False

    except Exception:
        # Fail closed: a doubtful region must not create a garbage event.
        return False


def filter_added_change_boxes(
    curr_roi: np.ndarray,
    ref_roi: np.ndarray,
    boxes: List[Tuple[int, int, int, int]]
) -> List[Tuple[int, int, int, int]]:
    return [
        box for box in boxes
        if garbage_change_is_added_object(curr_roi, ref_roi, box)
    ]
...
def analyze_garbage_once(frame: np.ndarray) -> Dict[str, Any]:
    ...
    for box in merged_change_boxes:
        ...
        # Directional filtering: only an ADDED object is garbage.
        # If the reference contained an object and it has now been removed,
        # absdiff also produces a box; reject that removal here.
        if not garbage_change_is_added_object(roi, ref_roi, box):
            continue
    ...
    # Object AI detections must also represent something NEW relative to the
    # saved reference.  This prevents unchanged reference objects from becoming
    # garbage events and keeps the system entry-triggered.
    detections = [
        det for det in detections
        if garbage_change_is_added_object(roi, ref_roi, det["box"])
    ]
    ...
    # IMPORTANT: absdiff itself is symmetric (add and remove look the same).
    # Use the area of directionally-filtered ADDED regions for fallback severity.
    added_diff_ratio = round(change_area_sum / roi_area, 4)
    diff_fallback_severity = garbage_diff_to_severity(
        diff_ratio=added_diff_ratio,
    ...
    # Keep only foreground additions.  Removal/disappearance regions are not
    # garbage and must not generate boxes or MQTT/SFTP events.
    particular_change_boxes = filter_added_change_boxes(
        roi, ref_roi, particular_change_boxes
    )
    ...
    static_corner_boxes = filter_added_change_boxes(
        roi, ref_roi, static_corner_boxes
    )
...
def parse_battery_command_output(text: str) -> Dict[str, Optional[float]]:
    ...
    try:
        ...
        return {
            ...
            "battery_power_w": round(float(voltage) * float(current_a), 3) if voltage is not None and current_a is not None else None
        }
    except Exception:
        pass
...
def collect_storage_status() -> Dict[str, Any]:
    ...
    return {
        ...
        "recording_enabled": bool(RECORDING_ENABLED and is_feature_enabled("sd_playback_recording_enabled")),
        "sd_playback_recording_enabled": is_feature_enabled("sd_playback_recording_enabled"),
...
def start_recording_process() -> Tuple[bool, Optional[str]]:
    ...
    if not is_feature_enabled("sd_playback_recording_enabled"):
        return False, "SD playback recording disabled from dashboard controls"
        ...
        try:
            ...
            if not is_feature_enabled("sd_playback_recording_enabled"):
                ...
                update_state(lambda s: s["system"].update({
                    ...
                    "recording_last_error": "SD playback recording disabled from dashboard controls"
                }))
                time.sleep(2)
                continue
    ...
    status = {
        ...
        **garbage_detection_mode_payload(),
        ...
        "battery_power_w": round(
            (battery_voltage or 0) * (battery.get("battery_current_a") or 0),
            3
        ),
...
def refresh_device_metrics_in_state() -> None:
    ...
    def mutate(state):
        ...
        state["system"].update({
            ...
            "battery_power_w": status.get("battery_power_w"),
        ...
        # Keep battery values visible in Device Health dashboard.
        # Voltage was previously removed from the API state after calculation.
        state["system"]["battery_voltage"] = status["battery_voltage"]
        state["system"]["battery_current_a"] = status["battery_current_a"]
        state["system"]["battery_current_ma"] = status["battery_current_ma"]
        state["system"]["battery_status"] = status["battery_status"]
...
def dashboard_logged_in() -> bool:
    return bool(session.get("gcam_dashboard_authenticated", False))


def _safe_dashboard_next_url(value: str) -> str:
...
def dashboard_login():
    if dashboard_logged_in():
        return redirect(_safe_dashboard_next_url(request.args.get("next", "/")))

    error = None
    next_url = _safe_dashboard_next_url(
    ...
    if request.method == "POST":
        ...
        if hmac.compare_digest(supplied, DASHBOARD_PASSWORD):
            session.clear()
            session["gcam_dashboard_authenticated"] = True
...
def require_dashboard_login():
    ...
    # Keep health/MQTT-facing APIs and raw camera stream operational.
    ...
    protected_page = (
        ...
        or path.startswith("/api/controls/")
        or path.startswith("/api/playback/")
        or path.startswith("/playback/video/")
    )

    if not protected_page:
        return None
    ...
    return redirect(url_for("dashboard_login", next=request.full_path if request.query_string else path))
...
def startup() -> None:
    ...
    threading.Thread(target=garbage_loop, daemon=True, name="gcam-garbage").start()
    ...
    threading.Thread(target=audio_worker_loop, daemon=True, name="gcam-audio").start()
    threading.Thread(target=live_talk_worker_loop, daemon=True, name="gcam-live-talk").start()
    threading.Thread(target=network_audio_monitor_loop, daemon=True, name="gcam-network-audio").start()
...
if __name__ == "__main__":
    startup()
    # Pi 5 1GB: eight Waitress threads are enough for dashboard/API/live-talk traffic
# without the extra per-thread memory pressure of the 16-thread build.
serve(app, host=HOST, port=PORT, threads=8, connection_limit=100, channel_timeout=120)
PY
...
echo "[INFO] Live Talk URL: ${PERMANENT_LIVE_BASE_URL}/live_talk"
echo "[INFO] Garbage AI/Normal detection and MQTT/default audio are enabled."
echo "[INFO] Pi 5 1GB profile: conservative stream sizes + 8 Waitress threads."
...
echo "[V8.0 FIX PATCH] Applying UI text visibility and garbage detect compatibility fixes..."

# Force all important UI controls/status text to black after templates are installed.
if [ -d "$APP_DIR/templates" ]; then
  cat >> "$APP_DIR/templates/index.html" <<'CSSPATCH'
...
# Fix accidental JSON boolean literals inside Python generated code.
if [ -d "$APP_DIR" ]; then
  find "$APP_DIR" -name "*.py" -type f -exec sed -i \
    -e 's/: true\([,}]\)/: True\1/g' \
    -e 's/: false\([,}]\)/: False\1/g' \
    -e 's/= true$/= True/g' \
    -e 's/= false$/= False/g' {} \; 2>/dev/null || true
fi

echo "[V8.0 FIX PATCH] Completed."


# V8.0 FINAL TEXT OVERRIDE
if [ -f "$APP_DIR/templates/index.html" ]; then
cat >> "$APP_DIR/templates/index.html" <<'CSS'
<style>
.control-btn,
.control-btn.primary,
.control-btn.ok,
...
echo "[V8.0 GARBAGE MANUAL DETECT FIX] Applying runtime patch..."

# Fix JSON boolean literals if any generated Python/config helper contains them.
if [ -d "$APP_DIR" ]; then
    grep -rl --include="*.py" "\btrue\b\|\bfalse\b" "$APP_DIR" 2>/dev/null | while read -r f; do
        sed -i \
          -e 's/\btrue\b/True/g' \
          -e 's/\bfalse\b/False/g' "$f" || true
    done
fi

# Ensure garbage manual API never crashes without returning a useful JSON response.
...
s=s.replace("run_garbage_detection_guarded(", "run_garbage_detection_guarded(")
p.write_text(s)
PY
fi

systemctl daemon-reload 2>/dev/null || true
systemctl restart gcam.service 2>/dev/null || true

echo "[V8.0 GARBAGE MANUAL DETECT FIX] Completed."


echo "[V8.0 VOLTAGE CURRENT HEALTH] Adding INA226 voltage/current dashboard support..."
# INA226 helper already exists in this installer. This patch exposes the values in Device Health.
# app.py should read battery command JSON and publish:
# battery_voltage, battery_current_a, battery_power_w
echo "[V8.0 VOLTAGE CURRENT HEALTH] Completed."
