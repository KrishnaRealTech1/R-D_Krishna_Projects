...
echo "=================================================="
echo " G-Cam Full Clean Reinstall - V6.18 Permanent Cloudflare Self-Healing"
...
<div class="wrap">
  <div class="card">
    <h1>G-Cam 7-Day Playback</h1>
    <div class="muted">Select a start date/time and end date/time. Playback starts at the exact requested start position, continues through saved recording segments, and stops at the selected end time.</div>
  </div>
  ...
  <div class="card">
    ...
    <div class="grid">
      <div class="stat"><span class="muted">Recording data</span><b>{{ storage.recordings_used_gb }} GB</b></div>
      <div class="stat"><span class="muted">SD free</span><b>{{ storage.sd_free_gb }} GB</b></div>
      <div class="stat"><span class="muted">Recorder</span><b>{{ 'ACTIVE' if storage.recording_active else 'IDLE' }}</b></div>
    </div>
    <div class="muted" style="margin-top:12px">
      Target retention: {{ storage.retention_days }} days · Protected free-space reserve: {{ storage.reserve_gb }} GB
...
SOFTWARE_VERSION = str(CONFIG.get("software_version", "Version 6.18")).strip()
...
RECORDING_RETENTION_DAYS = max(1, int(RECORDING_CONFIG.get("retention_days", 7)))
RECORDING_SEGMENT_SEC = max(60, int(RECORDING_CONFIG.get("segment_sec", 300)))
RECORDING_RESERVE_GB = max(0.5, float(RECORDING_CONFIG.get("reserve_gb", 3.0)))
...
def collect_storage_status() -> Dict[str, Any]:
    ...
    estimated_7day_bytes: Optional[float] = None
    estimated_capacity_days: Optional[float] = None

    if oldest is not None and recording_bytes > 0:
        elapsed_days = max((now - oldest).total_seconds() / 86400.0, 1.0 / 24.0)
        bytes_per_day = recording_bytes / elapsed_days
        estimated_7day_bytes = bytes_per_day * RECORDING_RETENTION_DAYS
        usable_for_recordings = max(0, usage.free + recording_bytes - RECORDING_RESERVE_BYTES)
        if bytes_per_day > 0:
            estimated_capacity_days = usable_for_recordings / bytes_per_day

    seven_day_history_complete = bool(
        oldest is not None
        and oldest <= (now - dt.timedelta(days=RECORDING_RETENTION_DAYS) + dt.timedelta(minutes=15))
    )

    if estimated_capacity_days is None:
        seven_day_capacity_estimate: Optional[bool] = None
    else:
        seven_day_capacity_estimate = estimated_capacity_days >= RECORDING_RETENTION_DAYS
    ...
    return {
        ...
        "recording_file_count": len(valid_files),
        ...
        "seven_day_history_complete": seven_day_history_complete,
        "estimated_daily_recording_gb": round(bytes_per_day / (1024 ** 3), 3) if bytes_per_day is not None else None,
        "estimated_7day_recording_gb": round(estimated_7day_bytes / (1024 ** 3), 3) if estimated_7day_bytes is not None else None,
        "estimated_capacity_days": round(estimated_capacity_days, 2) if estimated_capacity_days is not None else None,
        "seven_day_capacity_estimate": seven_day_capacity_estimate,
...
def enforce_recording_retention() -> Dict[str, Any]:
    RECORDING_DIR.mkdir(parents=True, exist_ok=True)
    cutoff = now_dt() - dt.timedelta(days=RECORDING_RETENTION_DAYS)
    deleted_files = 0
    deleted_bytes = 0
    ...
    # Never allow continuous recording to consume the protected OS reserve.
    # If the configured bitrate cannot fit seven full days on this card, oldest
    # recording segments are removed first rather than risking the existing app.
    usage = shutil.disk_usage(str(RECORDING_DIR))
    free_estimate = usage.free
    if free_estimate < RECORDING_RESERVE_BYTES:
        for stamp, path in _recording_files(include_recent=True):
            if free_estimate >= RECORDING_RESERVE_BYTES:
                break
            try:
                stat = path.stat()
                # Avoid deleting the MP4 FFmpeg is currently writing.
...
def stop_recording_process() -> None:
    global recording_process, recording_process_date
    with recording_process_lock:
        ...
        try:
            ...
            if (now_ts - last_state_ts) >= 30.0:
                update_state(lambda s: s["system"].update({
                    ...
                    "recording_segment_sec": RECORDING_SEGMENT_SEC,
                    "recording_playback_url": PERMANENT_PLAYBACK_URL,
                    "recording_last_error": None if is_recording_process_active() else s["system"].get("recording_last_error")
                }))
...
def handle_command(payload: Dict[str, Any]) -> None:
    ...
    if cmd == STORAGE_STATUS_COMMAND:
        ...
        add_event("storage_status_requested", {
            ...
            "seven_day_capacity_estimate": status.get("seven_day_capacity_estimate")
        })
        return
...
def startup() -> None:
    ...
    threading.Thread(target=recording_manager_loop, daemon=True, name="gcam-7day-recorder").start()
...
echo "[INFO] 7-day playback URL: ${PERMANENT_LIVE_BASE_URL}/playback"
