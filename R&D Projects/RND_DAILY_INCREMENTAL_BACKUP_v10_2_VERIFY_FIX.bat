...
title R&D Daily Exact Verified GitHub Backup v10.2

set "SELF=%~f0"
set "PS1=%TEMP%\rnd_daily_v10_2_%RANDOM%_%RANDOM%.ps1"
...
function RunRoboMirror([string]$Source,[string]$Dest,[string]$LogFile) {
  ...
  & robocopy.exe $Source $Dest /MIR /COPY:DAT /DCOPY:DAT /R:3 /W:2 /XJ /XD .git /MT:16 /NP /TEE "/LOG:$LogFile"
  $Code = $LASTEXITCODE
...
Write-Host ''
Write-Host '======================================================================'
Write-Host ' R&D DAILY EXACT VERIFIED GITHUB BACKUP v10.2'
Write-Host '======================================================================'
...
Write-Host 'v10.2 fixes the false verification failure caused by Robocopy log output.'
Write-Host ''
...
foreach ($S in $Sources) {
  ...
  [void](RunRoboMirror $S.Source $D $CopyLog)
}
...
foreach ($S in $Sources) {
  ...
  if ($V -ne 0) {
    Write-Host ("      First verification found differences for {0}; retrying one sync..." -f $S.Dest)
    $RetryLog = Join-Path $LogDir ("{0}_{1}_retry.log" -f $RunStamp,$SafeName)
    [void](RunRoboMirror $S.Source $D $RetryLog)
    $V = VerifyRoboMirror $S.Source $D $VerifyLog
  }
  if ($V -ne 0) {
...
if ($All.Count -le $MaxBatchFiles -and $Total -le $MaxBatchBytes) {
  ...
  [long]$Bytes = 0
  $N = 0
  $PathSpecFile = Join-Path $env:TEMP "rnd_v10_2_paths_$PID.bin"
