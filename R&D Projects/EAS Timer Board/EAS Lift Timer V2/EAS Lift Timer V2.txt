...
// Edit these two values only for testing.
...
// -----------------------------------------------------------------------------
// Important:
// Do NOT blink relay forever on RTC/I2C fault.
// Blinking the relay forever can look like the same continuous-trigger bug.
static void Safe_Fault_Idle_Forever(void)
{
    while (1)
    {
...
void main(void)
{
    ...
    // This is the most important fix for relay re-trigger after PIC reset.
    ...
    while (1)
    {
        ...
        if (DS3231_ReadTime_With_Retry(&now))
        {
            if (Is_Scheduled_Restart_Time(selected_mode, &now))
            {
                ...
                {
                    // Critical fix:
                    // Save trigger time BEFORE turning ON the relay.
                    // If relay switching resets the PIC, EEPROM will still remember
                    // that this exact minute already triggered.
                    ...
                    // Stay safely inside this loop until RTC minute changes.
