...
public sealed class ImageUploadService(AppOptions options) : IDisposable
{
        ...
        CancellationToken cancellationToken)
    {
            ...
            !cancellationToken.IsCancellationRequested)
        {
            ...
            throw new ImageTransferException(
                ...
                diagnostics,
                $"SFTP connection/upload failed. {diagnostics}",
                ex);
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            !cancellationToken.IsCancellationRequested)
        {
            // Session.Open may surface certificate/connection failures through more
            // than one WinSCP exception type. Always collect the WinSCP session log
            // before disposing it so the application SERVER LOG contains the actual
            // FTP/FTPS reply and TLS diagnostics.
            ...
            throw new ImageTransferException(
                ...
                $"Server reply: {serverReply} WinSCP diagnostics: {diagnostics} " +
                $"WinSCP response: {exceptionText}",
                ex);
        }
    ...
    private static string TryScanFtpsSha256Fingerprint(ImageUploadOptions upload)
    {
        try
        {
    ...
    {
        var rawResponse = CollapseExceptionChain(exception);
        var fingerprint = NormalizeSha256CertificateFingerprint(
            upload.TlsCertificateSha256Fingerprint);
        ...
        return new ImageTransferException(
            ...
            serverReply,
            diagnostics,
            $"{guidance} FTPS server reply: {serverReply} " +
            $"WinSCP/TLS diagnostic: {diagnostics} " +
            $"WinSCP response: {rawResponse}",
            exception);
    }
    ...
    private static string ReadWinScpDiagnostics(string sessionLogPath)
    {
        try
        {
            ...
            var diagnostics = File.ReadLines(sessionLogPath)
                .Select(line => line.Trim())
                .Where(line => line.StartsWith(". ", StringComparison.Ordinal))
