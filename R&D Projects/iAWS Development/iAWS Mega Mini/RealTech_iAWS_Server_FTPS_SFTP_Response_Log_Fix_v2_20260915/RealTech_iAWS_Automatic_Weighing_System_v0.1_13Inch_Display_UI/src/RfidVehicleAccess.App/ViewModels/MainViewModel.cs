...
public sealed class MainViewModel : ViewModelBase, IDisposable
{
    ...
    private string _systemDate = DateTime.Now.ToString("dd-MM-yyyy");
    ...
    private string _camera3Status = "Connecting...";
    private string _camera4Status = "Connecting...";
    private string _liveWeight = "0 KG";
    private string _weightDisplayValue = "0";
    private string _weightVehicleNumber = string.Empty;
    private string _targetWeight = "TARGET: 0 KG";
    private string _weightProcessStatus = "WAITING FOR TARGET WEIGHT";
    private string _weighbridgeStatus = "Weight bridge starting...";
    private bool _isWeighbridgeConnected;
    private bool _isInRfidConnected;
    private bool _isOutRfidConnected;
    private string _simulatedWeightInput = "0";
        ...
        AppConfigurationService configurationService)
    {
        ...
        _isInRfidConnected = _hardware.IsInRfidConnected;
        _isOutRfidConnected = _hardware.IsOutRfidConnected;
        ...
        _inCameraStatus = _options.Cameras.Camera1.Enabled
            ? _cameraStreams.Camera1Status
            : "Camera disabled";
        _outCameraStatus = _options.Cameras.Camera2.Enabled
            ? _cameraStreams.Camera2Status
            : "Camera disabled";
        _camera3Status = _options.Cameras.Camera3.Enabled
            ? _cameraStreams.Camera3Status
            : "Camera disabled";
        _camera4Status = _options.Cameras.Camera4.Enabled
            ? _cameraStreams.Camera4Status
            : "Camera disabled";
        _liveWeight = $"{_weighbridge.CurrentWeightKg:0.###} KG";
        _weightDisplayValue = $"{_weighbridge.CurrentWeightKg:0.###}";
        _targetWeight = $"TARGET: {_options.Weighbridge.TargetWeightKg:0.###} KG";
        _weighbridgeStatus = _weighbridge.CurrentStatus;
        _isWeighbridgeConnected = _weighbridge.IsConnected;
        _weightProcessStatus = _weighbridge.CurrentWeightKg >= _options.Weighbridge.TargetWeightKg
            ? _options.Processing.SensorInterfaceEnabled
                ? "TARGET REACHED - WAITING FOR SENSOR + RFID"
                : "TARGET REACHED - WAITING FOR FIRST RFID"
            : "WAITING FOR TARGET WEIGHT";
        ...
        SimulateWeightCommand = new RelayCommand(
            SimulateWeight,
            () => decimal.TryParse(SimulatedWeightInput, out var value) && value >= 0m);
        ...
        _weighbridge.ConnectionChanged += OnWeighbridgeConnectionChanged;
    ...
    {
        get => _camera3Status;
        set => SetProperty(ref _camera3Status, value);
    }

    public string Camera4Status
    {
        get => _camera4Status;
        set => SetProperty(ref _camera4Status, value);
    }

    public string LiveWeight
    {
        get => _liveWeight;
        set => SetProperty(ref _liveWeight, value);
    }

    public string WeightDisplayValue
    {
        get => _weightDisplayValue;
        set => SetProperty(ref _weightDisplayValue, value);
    }

    public string WeightVehicleNumber
    {
        get => _weightVehicleNumber;
        set => SetProperty(ref _weightVehicleNumber, value);
    }
    ...
    public string WeightProcessStatus
    {
        get => _weightProcessStatus;
        set => SetProperty(ref _weightProcessStatus, value);
    }

    public string WeighbridgeStatus
    {
        get => _weighbridgeStatus;
        set => SetProperty(ref _weighbridgeStatus, value);
    }

    public bool IsWeighbridgeConnected
    {
        get => _isWeighbridgeConnected;
        set => SetProperty(ref _isWeighbridgeConnected, value);
    }

    public bool IsInRfidConnected
    {
        get => _isInRfidConnected;
        set => SetProperty(ref _isInRfidConnected, value);
    }

    public bool IsOutRfidConnected
    {
        get => _isOutRfidConnected;
        set => SetProperty(ref _isOutRfidConnected, value);
    }

    public string SimulatedWeightInput
    {
        get => _simulatedWeightInput;
        set
        {
            if (SetProperty(ref _simulatedWeightInput, value))
            {
                ((RelayCommand)SimulateWeightCommand).RaiseCanExecuteChanged();
            }
        }
    }
    ...
    private ICommand CreateControlCommand(string command) =>
        new AsyncRelayCommand(() => ExecuteManualControlAsync(command));

    private async Task ExecuteManualControlAsync(string command)
    {
        // Reflect the operator's action on the main dashboard immediately.
        // HardwareGateway also publishes ControlCommandSent after a successful
        // physical/simulated write, so ApplyDashboardControlState is intentionally
        // idempotent and safe to receive the same command again.
        RunOnUiThread(() => ApplyDashboardControlState(command));
        await _hardware.SendControlCommandAsync(command);
    }

    public void RefreshWeighbridgeSettings()
    {
        TargetWeight = $"TARGET: {_options.Weighbridge.TargetWeightKg:0.###} KG";
        LiveWeight = $"{_weighbridge.CurrentWeightKg:0.###} KG";
        WeightDisplayValue = $"{_weighbridge.CurrentWeightKg:0.###}";
        WeighbridgeStatus = _weighbridge.CurrentStatus;
        IsWeighbridgeConnected = _weighbridge.IsConnected;
        WeightProcessStatus = _weighbridge.CurrentWeightKg >= _options.Weighbridge.TargetWeightKg
            ? "TARGET REACHED - FIRST IN/OUT RFID WINS"
            : _weighbridge.CurrentWeightKg <= _options.Weighbridge.ResetWeightKg
                ? "READY - WAITING FOR TARGET WEIGHT"
                : "WEIGHT BELOW TARGET";
    }
    ...
    private async Task ImportVehiclesFromApiAsync()
    {
        ...
        var confirmation = MessageBox.Show(
            ...
            sourceNames +
            Environment.NewLine + Environment.NewLine +
            "API vehicle sync uses only Username, Vehicle Number, RFID and Empty Weight. " +
            "Paid/Free access type, balance and other unrelated fields are ignored.",
    ...
    {
        var logsFolder = PathResolver.EnsureLogsFolder();
        Process.Start(new ProcessStartInfo
        {
            FileName = logsFolder,
            UseShellExecute = true
    ...
    {
        var captureFolder = PathResolver.EnsureImagesFolder();
        Process.Start(new ProcessStartInfo
        {
    ...
    private void UpdateLaneVehicleCard(LaneDisplayState state)
    {
        ...
        if (!string.IsNullOrWhiteSpace(state.VehicleNumber))
        {
            WeightVehicleNumber = state.VehicleNumber.Trim();
        }
        ...
        RunOnUiThread(() =>
        {
            switch (eventArgs.CameraNumber)
            {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    Camera3Status = eventArgs.Status;
                    break;
                case 4:
                    Camera4Status = eventArgs.Status;
                    break;
            }
        });
    }
    ...
    private void OnWeightChanged(object? sender, WeightChangedEventArgs eventArgs)
    {
        RunOnUiThread(() =>
        {
            LiveWeight = $"{eventArgs.WeightKg:0.###} KG";
            WeightDisplayValue = $"{eventArgs.WeightKg:0.###}";
            TargetWeight = $"TARGET: {_options.Weighbridge.TargetWeightKg:0.###} KG";
            if (eventArgs.WeightKg <= _options.Weighbridge.ResetWeightKg)
            {
                WeightVehicleNumber = string.Empty;
            }

            WeightProcessStatus = eventArgs.WeightKg >= _options.Weighbridge.TargetWeightKg
                ? _options.Processing.SensorInterfaceEnabled
                    ? "TARGET REACHED - WAITING FOR SENSOR + RFID"
                    : "TARGET REACHED - FIRST IN/OUT RFID WINS"
                : eventArgs.WeightKg <= _options.Weighbridge.ResetWeightKg
                    ? "READY - WAITING FOR TARGET WEIGHT"
                    : "WEIGHT BELOW TARGET";
        });
    }

    private void OnWeighbridgeConnectionChanged(
        object? sender,
        WeighbridgeConnectionChangedEventArgs eventArgs)
    {
        RunOnUiThread(() =>
        {
            IsWeighbridgeConnected = eventArgs.IsConnected;
            WeighbridgeStatus = eventArgs.Status;
        });
    }

    private void SimulateWeight()
    {
        if (decimal.TryParse(SimulatedWeightInput, out var weight) && weight >= 0m)
        {
            _weighbridge.SimulateWeight(weight);
        }
    }
        ...
        HardwareConnectionStatusChangedEventArgs eventArgs)
    {
        RunOnUiThread(() =>
        {
            ...
            IsInRfidConnected = _hardware.IsInRfidConnected;
            IsOutRfidConnected = _hardware.IsOutRfidConnected;
    ...
    private void ApplyDashboardControlState(string command)
    {
    ...
    private void OnClockTimerTick(object? sender, EventArgs e)
    {
        ...
        SystemDate = now.ToString("dd-MM-yyyy");
