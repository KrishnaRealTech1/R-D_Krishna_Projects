...
public partial class HardwareSettingsWindow : Window
{
    ...
    private void LoadCurrentSettings()
    {
        ...
        WifiPortTextBox.Text = _options.Hardware.WifiControl.Port.ToString(CultureInfo.InvariantCulture);
    ...
    private HardwareOptions BuildHardwareSettings()
    {
        ...
        hardware.WifiControlEnabled = WifiEnabledCheckBox.IsChecked == true;
        var wifiHost = WifiHostTextBox.Text.Trim();
        if (hardware.WifiControlEnabled && string.IsNullOrWhiteSpace(wifiHost))
        {
            throw new InvalidOperationException("Enter the Wi-Fi terminal IP/host.");
        }
        if (hardware.WifiControlEnabled &&
            (!int.TryParse(WifiPortTextBox.Text.Trim(), NumberStyles.Integer, CultureInfo.InvariantCulture, out var wifiPort) ||
             wifiPort is < 1 or > 65535))
        {
            throw new InvalidOperationException("Enter a valid Wi-Fi terminal TCP port (1-65535).");
        }
        hardware.WifiControl.Host = wifiHost;
        if (hardware.WifiControlEnabled)
        {
            hardware.WifiControl.Port = int.Parse(WifiPortTextBox.Text.Trim(), CultureInfo.InvariantCulture);
        }
    ...
    private WeighbridgeOptions BuildWeighbridgeSettings()
    {
        ...
        {
            Enabled = true,
            UniversalFormatEnabled = _options.Weighbridge.UniversalFormatEnabled,
    ...
    {
        var hardwareEnabled = SimulationCheckBox.IsChecked != true;
        WeightPortComboBox.IsEnabled = hardwareEnabled;
        WeightBaudRateComboBox.IsEnabled = hardwareEnabled;
        InPortComboBox.IsEnabled = hardwareEnabled;
        OutPortComboBox.IsEnabled = hardwareEnabled;
        ControlPortComboBox.IsEnabled = hardwareEnabled;
        InBaudRateComboBox.IsEnabled = hardwareEnabled;
        OutBaudRateComboBox.IsEnabled = hardwareEnabled;
        ControlBaudRateComboBox.IsEnabled = hardwareEnabled;

        BluetoothEnabledCheckBox.IsEnabled = hardwareEnabled;
        var bluetoothEnabled = hardwareEnabled && BluetoothEnabledCheckBox.IsChecked == true;
        BluetoothPortComboBox.IsEnabled = bluetoothEnabled;
        BluetoothBaudRateComboBox.IsEnabled = bluetoothEnabled;

        WifiEnabledCheckBox.IsEnabled = hardwareEnabled;
        var wifiEnabled = hardwareEnabled && WifiEnabledCheckBox.IsChecked == true;
