...
public sealed class HardwareGateway : IHostedService, IDisposable
{
    ...
    public bool IsInRfidConnected =>
        _options.Hardware.SimulationEnabled || _inRfidPort?.IsOpen == true;
    public bool IsOutRfidConnected =>
        _options.Hardware.SimulationEnabled || _outRfidPort?.IsOpen == true;
    public bool IsControlConnected =>
        _options.Hardware.SimulationEnabled || IsActiveControlLinkReady();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // iAWS sends the lane-specific signal command and the common signal
        // command together in one locked write cycle. Examples:
        // "IN RED <vehicle>" -> "RED <vehicle>", "OUT ORG" -> "ORG", "IN GRN" -> "GRN".
        var parallelSignalCommand = GetParallelSignalCommand(command);
        ...
        try
        {
            ...
            {
                await PublishSimulatedCommandAsync(command, cancellationToken);
                if (parallelSignalCommand is not null)
                {
                    await PublishSimulatedCommandAsync(parallelSignalCommand, cancellationToken);
                }
                return;
            }
            ...
            try
            {
                await WriteControlCommandPairAsync(
                    command,
                    parallelSignalCommand,
                    cancellationToken);
            }
            ...
            await PublishSentCommandAsync(transport, command, cancellationToken);
            if (parallelSignalCommand is not null)
            {
                await PublishSentCommandAsync(
                    transport,
                    parallelSignalCommand,
                    cancellationToken,
                    isParallelSignal: true);
            }
            PublishConnectionSummary();
        }
        catch (Exception ex)
    ...
    private async Task WriteControlCommandPairAsync(
        string command,
        string? parallelSignalCommand,
        CancellationToken cancellationToken)
    {
        ...
        if (parallelSignalCommand is not null)
        {
            await WriteActiveControlAsync(parallelSignalCommand, cancellationToken);
        }
    }

    private async Task PublishSimulatedCommandAsync(
        string command,
        CancellationToken cancellationToken)
    {
    ...
    private async Task PublishSentCommandAsync(
        string transport,
        string command,
        CancellationToken cancellationToken,
        bool isParallelSignal = false)
    {
        var prefix = isParallelSignal ? "Parallel signal command" : "Command";
        await _logger.StatusAsync($"{prefix} sent via {transport}: {command}", cancellationToken);
        PublishControlTraffic($"[TX][{transport}] {command}");
        ControlCommandSent?.Invoke(this, command);
    }

    private static string? GetParallelSignalCommand(string command)
    {
        var trimmed = command.Trim();
        var normalized = trimmed.ToUpperInvariant();

        // RED carries the same optional display payload to the common signal
        // channel. Examples:
        //   IN RED TN 11 BY 9880 -> RED TN 11 BY 9880
        //   OUT RED             -> RED
        foreach (var prefix in new[] { "IN RED", "OUT RED" })
        {
            ...
            {
                var payload = trimmed[prefixWithSpace.Length..].Trim();
        ...
        if (normalized == "IN ORG" || normalized == "OUT ORG")
        {
            return "ORG";
        }

        if (normalized == "IN GRN" || normalized == "OUT GRN")
        {
            return "GRN";
        }
    ...
    private void OpenRfidPorts()
    {
        ...
        // Keep the active physical-to-logical lane mapping visible in STATUS LOG.
        // This is especially useful after a COM-port reconfiguration or reconnect.
        _ = _logger.StatusAsync(
            $"RFID PORT MAP | IN={_inRfidPort.PortName} | OUT={_outRfidPort.PortName}.");
    }
    ...
    private void OnInRfidDataReceived(object sender, SerialDataReceivedEventArgs e)
    {
        ...
        {
            ReadRfidFromSourcePort(port, LaneDirection.In);
        }
    }
    ...
    private void OnOutRfidDataReceived(object sender, SerialDataReceivedEventArgs e)
    {
        if (sender is SerialPort port)
        {
            ReadRfidFromSourcePort(port, LaneDirection.Out);
        }
    }

    private void ReadRfidFromSourcePort(SerialPort port, LaneDirection handlerDirection)
    {
        // Resolve the lane from the actual SerialPort instance/COM name, not only
        // from the event-handler name. This prevents an IN read from being published
        // as OUT if handlers are rebound during a reconnect/settings refresh.
        var direction = ResolveRfidDirection(port, handlerDirection);
        var settings = direction == LaneDirection.In
            ? _options.Hardware.InRfid
            : _options.Hardware.OutRfid;
        var decoder = direction == LaneDirection.In
            ? _inRfidDecoder
            : _outRfidDecoder;

        if (direction != handlerDirection)
        {
            _ = _logger.StatusAsync(
                $"RFID direction corrected from {handlerDirection.ToString().ToUpperInvariant()} to " +
                $"{direction.ToString().ToUpperInvariant()} using source port {port.PortName}.");
        }

        ReadRfid(port, direction, settings, decoder);
    }

    private LaneDirection ResolveRfidDirection(
        SerialPort port,
        LaneDirection fallbackDirection)
    {
        if (ReferenceEquals(port, _inRfidPort))
        {
            return LaneDirection.In;
        }

        if (ReferenceEquals(port, _outRfidPort))
        {
            return LaneDirection.Out;
        }

        if (port.PortName.Equals(
                _options.Hardware.InRfid.PortName,
        ...
        if (port.PortName.Equals(
                _options.Hardware.OutRfid.PortName,
                StringComparison.OrdinalIgnoreCase))
        {
            return LaneDirection.Out;
        }
        ...
        UhfCfFrameDecoder decoder)
    {
        while (port.BytesToRead > 0)
        {
            ...
            {
                PublishRfid(direction, epc, port.PortName);
            }
        }
    }
    ...
    private void ReadLineRfid(SerialPort port, LaneDirection direction)
    {
        while (port.BytesToRead > 0)
        {
            ...
            {
                PublishRfid(direction, line, port.PortName);
            }
        }
    }

    private void PublishRfid(
        LaneDirection direction,
        string rfid,
        string sourcePortName)
    {
        ...
        var lane = direction == LaneDirection.In ? "IN" : "OUT";
        _ = _logger.StatusAsync(
            $"RFID READ | lane={lane} | port={sourcePortName} | EPC={rfid}.");
        RfidReceived?.Invoke(this, (direction, rfid));
    }
