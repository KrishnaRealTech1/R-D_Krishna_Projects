    ...
    AppLogger logger)
{
    public Task<ImportResult> ImportAsync(
        ...
        ImportInternalAsync(filePath, apiMinimalFormat: false, cancellationToken: cancellationToken);

    public Task<ImportResult> ImportApiMinimalAsync(
        string filePath,
        CancellationToken cancellationToken = default) =>
        ImportInternalAsync(filePath, apiMinimalFormat: true, cancellationToken: cancellationToken);

    private async Task<ImportResult> ImportInternalAsync(
        string filePath,
        bool apiMinimalFormat,
        CancellationToken cancellationToken)
    {
        ...
        foreach (var row in rows)
        {
            ...
            try
            {
                ...
                var record = CreateVehicleRecord(
                    row,
                    rfid,
                    vehicleNumber,
                    existing,
                ...
                // Vehicle API rows are intentionally limited to username,
                // vehicle number, RFID and empty weight. Payment/access type,
                // balance, category and contractor fields are ignored.
                if (!apiMinimalFormat && !string.IsNullOrWhiteSpace(record.ContractorCode))
                {
                ...
                if (!apiMinimalFormat && record.CategoryDebitAmount.HasValue)
                {
        ...
        bool apiMinimalFormat)
    {
        ...
        if (apiMinimalFormat)
        {
            return new VehicleRecord
            {
                ...
                VehicleCategory = existing?.VehicleCategory ?? "Default",
                // Paid/Free is not part of the iAWS registered-vehicle API.
                // Preserve existing local data; for a new row use neutral BOTH.
                AccessType = existing?.AccessType ?? RfidAccessType.Both,
                Balance = existing?.Balance ?? 0m,
                EmptyWeight = string.IsNullOrWhiteSpace(row.EmptyWeight)
                ...
                ContractorCode = existing?.ContractorCode ?? string.Empty,
                ContractorName = existing?.ContractorName ?? string.Empty,
                CategoryDebitAmount = existing?.CategoryDebitAmount
            };
        }

        var vehicleCategory = string.IsNullOrWhiteSpace(row.VehicleCategory)
            ? existing?.VehicleCategory ?? "Default"
    ...
    private IEnumerable<VehicleImportRow> ReadCsv(string filePath)
    {
        ...
        while (csv.Read())
        {
            ...
            var row = new VehicleImportRow
            {
                ...
                VehicleNumber = GetCsvValue(csv, "vehicle_no", "vehicle_number", "vehicle_n", "vehicle_num", "vehicleno"),
             ...
             rowNumber++)
        {
            ...
            var row = new VehicleImportRow
            {
                ...
                VehicleNumber = GetExcelValue(worksheetRow, headers,
                    "vehicle_no", "vehicle_number", "vehicle_n", "vehicle_num", "vehicleno"),
