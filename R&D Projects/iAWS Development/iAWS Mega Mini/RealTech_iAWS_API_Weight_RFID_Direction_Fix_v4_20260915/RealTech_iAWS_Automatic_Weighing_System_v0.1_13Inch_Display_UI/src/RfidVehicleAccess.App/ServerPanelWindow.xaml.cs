...
public partial class ServerPanelWindow : Window
{
    ...
    private TabItem CreateDeviceAndProcessingTab()
    {
        ...
        var processing = CreateGroup("Trip Processing", grid =>
        {
            ...
            AddCheckField(
                grid,
                "Processing.SensorInterfaceEnabled",
                "Enable Sensor Interface",
                "Enabled: the matching lane sensor, target weight and RFID are all required. Disabled: iAWS starts from target weight + RFID only.");
            AddTextField(grid, "Processing.SensorValiditySeconds", "Sensor Validity Seconds", "Maximum age of a sensor HIGH event accepted for the RFID transaction.");
        ...
        var weighbridge = CreateGroup("iAWS Weighbridge Processing", grid =>
        {
            AddCheckField(grid, "Weighbridge.Enabled", "Enable Weighbridge");
            AddCheckField(
                grid,
                "Weighbridge.UniversalFormatEnabled",
                "Universal / Auto Detect Weight Format",
                "Recommended. Accepts common weighbridge outputs such as wn-00.120kg, 000020, comma/status-prefixed values and unit-tagged KG/G/T values without changing the application.");
            AddTextField(grid, "Weighbridge.TargetWeightKg", "Target Weight (KG)", "Weight required to arm the automatic cycle.");
            AddTextField(grid, "Weighbridge.ResetWeightKg", "Reset Weight (KG)", "Weight must fall below this value before the next vehicle cycle.");
            AddTextField(grid, "Weighbridge.StableReadCount", "Stable Read Count");
            AddTextField(grid, "Weighbridge.DataPrefix", "Legacy Weight Data Prefix", "Used only when Universal / Auto Detect is disabled. Example: wn.");
            AddTextField(grid, "Weighbridge.UnitText", "Default / Legacy Weight Unit", "Used as the default unit when the incoming value has no unit. Example: kg.");
        });

        var audio = CreateGroup("Audio Interface", grid =>
        {
            AddCheckField(
                grid,
                "Audio.Enabled",
                "Enable Audio Interface",
                "Master switch for the iAWS RFID-detected and process-completed announcements.");
            AddCheckField(grid, "Audio.RfidDetected.Enabled", "Enable RFID Detected Audio");
            AddAudioFileField(
                grid,
                "Audio.RfidDetected.FilePath",
                "RFID Detected Audio File",
                "Played when a valid RFID wins the weighbridge cycle. The imported file is copied into Documents\\Realtech_iAWS\\Audio.",
                "RFID_Detected");
            AddTextField(
                grid,
                "Audio.RfidDetected.RepeatCount",
                "RFID Detected Play Count",
                "Example: 3 plays the selected RFID audio three times.");
            AddCheckField(grid, "Audio.ProcessCompleted.Enabled", "Enable Completed / Barrier Open Audio");
            AddAudioFileField(
                grid,
                "Audio.ProcessCompleted.FilePath",
                "Completed / Barrier Open Audio File",
                "Played after the transaction is completed and the boom barrier OPEN command is sent.",
                "Process_Completed");
            AddTextField(
                grid,
                "Audio.ProcessCompleted.RepeatCount",
                "Completed Audio Play Count",
                "Example: 3 plays the selected completion audio three times.");
        });

        return CreateTab("Device & Processing", device, processing, weighbridge, audio);
    }
    ...
    private TabItem CreateHardwareTab()
    {
        ...
        var weighbridgeSerial = CreateSerialPortGroup("Weighbridge", "Weighbridge.Serial");
    ...
    private TabItem CreateCamerasTab()
    {
        ...
        var camera1 = CreateCameraGroup("Camera 1", "Cameras.Camera1");
        var camera2 = CreateCameraGroup("Camera 2", "Cameras.Camera2");
        var camera3 = CreateCameraGroup("Camera 3", "Cameras.Camera3");
        var camera4 = CreateCameraGroup("Camera 4", "Cameras.Camera4");
        return CreateTab("Cameras", general, camera1, camera2, camera3, camera4);
    }
    ...
    private TabItem CreateServerTab()
    {
        ...
        var mqtt = CreateGroup("MQTT", grid =>
        {
            AddCheckField(
                ...
                "Server.Mqtt.Enabled",
                "Enable MQTT Method",
                "When disabled, iAWS does not publish weighing transactions by MQTT.");
        ...
        var api = CreateGroup("HTTP API Method", grid =>
        {
            AddCheckField(
                grid,
                "Server.Api.Enabled",
                "Enable API Method",
                "Posts each completed weighing transaction as application/json to the configured endpoint.");
            AddTextField(
                grid,
                "Server.Api.Endpoint",
                "API Endpoint",
                "Editable API URL. Default: http://dev.igps.io/aws_raw_data/iAWS_universalAPi.php");
            AddComboField(grid, "Server.Api.Method", "HTTP Method", ["POST"]);
            AddTextField(grid, "Server.Api.RequestTimeoutSeconds", "Request Timeout Seconds");
            AddTextField(grid, "Server.Api.Action", "Action", "Payload field action. Default: WEIGH.");
            AddTextField(
                grid,
                "Server.Api.DefaultMaterialType",
                "Default Material Type",
                "Payload field material_type. Default: Municipal Solid Waste.");
        });
        ...
        var upload = CreateGroup("Image Upload", grid =>
        {
            AddComboField(
                grid,
                "Server.ImageUpload.Mode",
                "Mode",
                ["Disabled", "Sftp", "Ftp"],
                "Choose Sftp (SSH File Transfer Protocol) or Ftp. For FTP servers using TLS/SSL Explicit encryption, enable the FTPS checkbox below.");
            AddTextField(grid, "Server.ImageUpload.Host", "Upload Host");
            AddTextField(
                grid,
                "Server.ImageUpload.Port",
                "Upload Port",
                "Normally 22 for SFTP or 21 for FTP unless your server uses a custom port.");
            AddCheckField(
                grid,
                "Server.ImageUpload.UseTls",
                "Use Explicit TLS/SSL (FTPS)",
                "For FTP servers configured as TLS/SSL Explicit encryption (AUTH TLS), normally on port 21. Ignored for SFTP.");
            AddTextField(
                grid,
                "Server.ImageUpload.TlsCertificateSha256Fingerprint",
                "FTPS Certificate SHA-256 Fingerprint",
                "Recommended when the FTPS certificate is self-signed, has an incomplete chain, or its name does not match the FTP host. Verify the SHA-256 fingerprint in WinSCP, then paste it here. Leave blank for normal Windows certificate validation.");
            AddFtpsCertificateFingerprintDetector(grid);
            AddCheckField(
                grid,
                "Server.ImageUpload.AllowInvalidTlsCertificate",
                "Allow Invalid FTPS Certificate (Unsafe)",
                "Emergency compatibility only. Accepts any FTPS server certificate when TLS is enabled, which removes server identity verification. Prefer the SHA-256 fingerprint field above.");

            var uploadModeCombo = _comboFields["Server.ImageUpload.Mode"];
            uploadModeCombo.SelectionChanged += (_, _) =>
            {
                if (!_textFields.TryGetValue("Server.ImageUpload.Port", out var portField))
                {
                    return;
                }

                var selectedMode = uploadModeCombo.SelectedItem?.ToString() ?? uploadModeCombo.Text;
                if (ImageUploadService.IsFtpMode(selectedMode) &&
                    (string.IsNullOrWhiteSpace(portField.Text) || portField.Text.Trim() == "22"))
                {
                    portField.Text = "21";
                }
                else if (ImageUploadService.IsSftpMode(selectedMode) &&
                         (string.IsNullOrWhiteSpace(portField.Text) || portField.Text.Trim() == "21"))
                {
                    portField.Text = "22";
                }
            };
        ...
        return CreateTab("Server", general, mqtt, api, upload);
    }
    ...
    private TabItem CreateStorageConnectivityImportTab()
    {
        ...
        {
            AddComboField(grid, "Import.DefaultAccessType", "Default Access Type", ["Free", "Paid", "Both"]);
    ...
    private GroupBox CreateVehicleApiSourcesGroup()
    {
        ...
        {
            Text = "Each enabled source is requested directly and the response is read as raw CSV. " +
                   "No JSON request body is sent. Priority 1 is highest. Lower-priority sources are imported first, so the highest-priority source wins conflicting values.",
    ...
    private void AddAudioFileField(
        Grid grid,
        string key,
        string label,
        string? description,
        string destinationBaseName)
    {
        ...
        importButton.Click += (_, _) => ImportAudioFile(key, destinationBaseName);
        Grid.SetColumn(importButton, 1);
        editorGrid.Children.Add(importButton);

        AddEditorRow(grid, label, description, editorGrid);
    }

    private void ImportAudioFile(string fieldKey, string destinationBaseName)
    {
        var dialog = new OpenFileDialog
        {
            Title = "Import iAWS Audio File",
            Filter = "Audio Files (*.wav;*.mp3;*.wma;*.m4a;*.aac)|*.wav;*.mp3;*.wma;*.m4a;*.aac|All Files (*.*)|*.*",
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException("The selected audio file does not have a file extension.");
            }

            var audioFolder = PathResolver.EnsureAudioFolder();
            var destinationFileName = destinationBaseName + extension.ToLowerInvariant();
            var destinationPath = Path.Combine(audioFolder, destinationFileName);
            ...
            _textFields[fieldKey].Text = $"%REALTECH_SYSTEMS%\\Audio\\{destinationFileName}";
            ShowStatus(
                $"Audio imported to the dedicated iAWS folder: {destinationPath}. Select Save & Apply to store this selection.",
                true);
        }
        catch (Exception ex)
        {
    ...
    private void AddFtpsCertificateFingerprintDetector(Grid grid)
    {
        var button = new Button
        {
            Content = "Detect FTPS Certificate...",
            MinWidth = 190,
            MinHeight = 34,
            Margin = new Thickness(4),
            Padding = new Thickness(10, 5, 10, 5),
        ...
        button.Click += async (_, _) => await DetectFtpsCertificateFingerprintAsync(button);
        AddEditorRow(
            grid,
            "Detect FTPS Certificate",
            "Reads the SHA-256 certificate fingerprint currently presented by the configured FTPS host. You must confirm before it replaces the configured fingerprint.",
            button);
    }

    private async Task DetectFtpsCertificateFingerprintAsync(Button button)
    {
        var mode = ReadCombo("Server.ImageUpload.Mode");
        var useTls = ReadCheck("Server.ImageUpload.UseTls");
        var host = ReadText("Server.ImageUpload.Host").Trim();
        var portText = ReadText("Server.ImageUpload.Port").Trim();

        if (!ImageUploadService.IsFtpMode(mode) || !useTls)
        {
            MessageBox.Show(
                this,
                "Select Ftp mode and enable 'Use Explicit TLS/SSL (FTPS)' before detecting the certificate.",
                "FTPS Certificate Detection",
                MessageBoxButton.OK,
            ...
            port is < 1 or > 65535)
        {
            MessageBox.Show(
                this,
                "Enter a valid FTPS host and port first.",
                "FTPS Certificate Detection",
                MessageBoxButton.OK,
        ...
        if (_textFields.TryGetValue("Server.ImageUpload.ConnectionTimeoutSeconds", out var timeoutField) &&
            int.TryParse(timeoutField.Text, NumberStyles.Integer, CultureInfo.InvariantCulture, out var parsedTimeout) &&
            parsedTimeout > 0)
        {
            connectionTimeout = parsedTimeout;
        }

        var scanOptions = new ImageUploadOptions
        {
            Mode = "Ftp",
            UseTls = true,
            Host = host,
        ...
        try
        {
            var presentedFingerprint = await Task.Run(
                () => ImageUploadService.TryScanFtpsSha256Fingerprint(scanOptions));

            if (string.IsNullOrWhiteSpace(presentedFingerprint))
            {
                MessageBox.Show(
                    this,
                    "The server did not return a detectable FTPS SHA-256 certificate fingerprint. Check the host, port, firewall and Explicit FTPS setting, then try again.",
                    "FTPS Certificate Detection",
                    MessageBoxButton.OK,
                    MessageBoxImage.Warning);
                return;
            ...
            var currentFingerprint = ReadText(
                "Server.ImageUpload.TlsCertificateSha256Fingerprint").Trim();
            var currentText = string.IsNullOrWhiteSpace(currentFingerprint)
                ? "<not configured>"
                : currentFingerprint;
            ...
            var result = MessageBox.Show(
                ...
                $"Certificate currently presented by the server:\n{presentedFingerprint}\n\n" +
                "Confirm this fingerprint with the FTP/server administrator. Select Yes only if it belongs to the expected server.\n\n" +
                "Copy the presented fingerprint into the FTPS certificate field?",
                "FTPS Certificate Fingerprint",
            ...
            _textFields["Server.ImageUpload.TlsCertificateSha256Fingerprint"].Text =
                presentedFingerprint;
            _checkFields["Server.ImageUpload.AllowInvalidTlsCertificate"].IsChecked = false;
            ShowStatus(
                "Detected FTPS certificate fingerprint copied into the settings. Select Save & Apply to store it.",
                true);
        }
        catch (Exception ex)
        {
            ...
            MessageBox.Show(
                this,
                ex.Message,
                "FTPS Certificate Detection Failed",
                MessageBoxButton.OK,
                MessageBoxImage.Error);
        }
        finally
    ...
    private void LoadCurrentSettings()
    {
        ...
        SetCheck("Processing.SensorInterfaceEnabled", _options.Processing.SensorInterfaceEnabled);
        ...
        SetCheck("Weighbridge.Enabled", _options.Weighbridge.Enabled);
        SetCheck("Weighbridge.UniversalFormatEnabled", _options.Weighbridge.UniversalFormatEnabled);
        SetText("Weighbridge.TargetWeightKg", FormatDecimal(_options.Weighbridge.TargetWeightKg));
        SetText("Weighbridge.ResetWeightKg", FormatDecimal(_options.Weighbridge.ResetWeightKg));
        SetText("Weighbridge.StableReadCount", _options.Weighbridge.StableReadCount);
        SetText("Weighbridge.DataPrefix", _options.Weighbridge.DataPrefix);
        SetText("Weighbridge.UnitText", _options.Weighbridge.UnitText);
        LoadSerialPort("Weighbridge.Serial", _options.Weighbridge.Serial);

        SetCheck("Audio.Enabled", _options.Audio.Enabled);
        SetCheck("Audio.RfidDetected.Enabled", _options.Audio.RfidDetected.Enabled);
        SetText("Audio.RfidDetected.FilePath", _options.Audio.RfidDetected.FilePath);
        SetText("Audio.RfidDetected.RepeatCount", _options.Audio.RfidDetected.RepeatCount);
        SetCheck("Audio.ProcessCompleted.Enabled", _options.Audio.ProcessCompleted.Enabled);
        SetText("Audio.ProcessCompleted.FilePath", _options.Audio.ProcessCompleted.FilePath);
        SetText("Audio.ProcessCompleted.RepeatCount", _options.Audio.ProcessCompleted.RepeatCount);
        ...
        LoadCamera("Cameras.Camera1", _options.Cameras.Camera1);
        LoadCamera("Cameras.Camera2", _options.Cameras.Camera2);
        LoadCamera("Cameras.Camera3", _options.Cameras.Camera3);
        LoadCamera("Cameras.Camera4", _options.Cameras.Camera4);
        ...
        SetCheck("Server.Mqtt.Enabled", _options.Server.Mqtt.Enabled);
        ...
        SetCheck("Server.Api.Enabled", _options.Server.Api.Enabled);
        SetText("Server.Api.Endpoint", _options.Server.Api.Endpoint);
        SetCombo("Server.Api.Method", _options.Server.Api.Method);
        SetText("Server.Api.RequestTimeoutSeconds", _options.Server.Api.RequestTimeoutSeconds);
        SetText("Server.Api.Action", _options.Server.Api.Action);
        SetText("Server.Api.DefaultMaterialType", _options.Server.Api.DefaultMaterialType);

        SetCombo("Server.ImageUpload.Mode", _options.Server.ImageUpload.Mode);
        SetCheck("Server.ImageUpload.UseTls", _options.Server.ImageUpload.UseTls);
        SetText("Server.ImageUpload.TlsCertificateSha256Fingerprint", _options.Server.ImageUpload.TlsCertificateSha256Fingerprint);
        SetCheck("Server.ImageUpload.AllowInvalidTlsCertificate", _options.Server.ImageUpload.AllowInvalidTlsCertificate);
    ...
    private async void SaveButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            var weighbridgeResult = await _weighbridge.ApplySettingsAsync(updated.Weighbridge);
            ...
            var fullyApplied = hardwareResult.Success && weighbridgeResult.Success;
            var message = fullyApplied
                ? "Configuration saved and applied successfully."
                : $"Configuration saved with connection warning. Hardware: {hardwareResult.Message} | Weighbridge: {weighbridgeResult.Message}";
            ...
            ShowStatus(message, fullyApplied);
            MessageBox.Show(
                message,
                "Server Panel",
                MessageBoxButton.OK,
        ...
        catch (Exception ex)
        {
            ...
            try
            {
                ...
                await _weighbridge.ApplySettingsAsync(original.Weighbridge);
                _cameraStreams.ReloadConfiguration();
                _viewModel.RefreshExceptionalApprovalSettings();
            }
            catch
    ...
    private AppOptions BuildOptionsFromFields()
    {
        ...
        updated.Processing.SensorInterfaceEnabled = ReadCheck("Processing.SensorInterfaceEnabled");
        ...
        updated.Weighbridge.Enabled = ReadCheck("Weighbridge.Enabled");
        updated.Weighbridge.UniversalFormatEnabled = ReadCheck("Weighbridge.UniversalFormatEnabled");
        updated.Weighbridge.TargetWeightKg = ReadDecimal("Weighbridge.TargetWeightKg", "Target Weight", 0.001m);
        updated.Weighbridge.ResetWeightKg = ReadDecimal("Weighbridge.ResetWeightKg", "Reset Weight", 0m);
        updated.Weighbridge.StableReadCount = ReadInt("Weighbridge.StableReadCount", "Stable Read Count", 1);
        updated.Weighbridge.DataPrefix = ReadText("Weighbridge.DataPrefix");
        updated.Weighbridge.UnitText = ReadText("Weighbridge.UnitText");
        updated.Weighbridge.Serial = ReadSerialPort("Weighbridge.Serial", "Weighbridge");

        updated.Audio.Enabled = ReadCheck("Audio.Enabled");
        updated.Audio.RfidDetected.Enabled = ReadCheck("Audio.RfidDetected.Enabled");
        updated.Audio.RfidDetected.FilePath = ReadText("Audio.RfidDetected.FilePath");
        updated.Audio.RfidDetected.RepeatCount = ReadInt(
            "Audio.RfidDetected.RepeatCount",
            "RFID Detected Play Count",
            1,
            100);
        updated.Audio.ProcessCompleted.Enabled = ReadCheck("Audio.ProcessCompleted.Enabled");
        updated.Audio.ProcessCompleted.FilePath = ReadText("Audio.ProcessCompleted.FilePath");
        updated.Audio.ProcessCompleted.RepeatCount = ReadInt(
            "Audio.ProcessCompleted.RepeatCount",
            "Completed Audio Play Count",
            1,
            100);
        ...
        updated.Cameras.Camera1 = ReadCamera("Cameras.Camera1", "Camera 1");
        updated.Cameras.Camera2 = ReadCamera("Cameras.Camera2", "Camera 2");
        updated.Cameras.Camera3 = ReadCamera("Cameras.Camera3", "Camera 3");
        updated.Cameras.Camera4 = ReadCamera("Cameras.Camera4", "Camera 4");
        ...
        updated.Server.Mqtt.Enabled = ReadCheck("Server.Mqtt.Enabled");
        ...
        updated.Server.Api.Enabled = ReadCheck("Server.Api.Enabled");
        updated.Server.Api.Endpoint = ReadText("Server.Api.Endpoint");
        updated.Server.Api.Method = ReadCombo("Server.Api.Method");
        updated.Server.Api.RequestTimeoutSeconds = ReadInt(
            "Server.Api.RequestTimeoutSeconds",
            "API Request Timeout Seconds",
            1,
            300);
        updated.Server.Api.Action = ReadText("Server.Api.Action");
        updated.Server.Api.DefaultMaterialType = ReadText("Server.Api.DefaultMaterialType");

        updated.Server.ImageUpload.Mode = ReadCombo("Server.ImageUpload.Mode");
        updated.Server.ImageUpload.UseTls = ReadCheck("Server.ImageUpload.UseTls");
        updated.Server.ImageUpload.TlsCertificateSha256Fingerprint = ReadText("Server.ImageUpload.TlsCertificateSha256Fingerprint");
        updated.Server.ImageUpload.AllowInvalidTlsCertificate = ReadCheck("Server.ImageUpload.AllowInvalidTlsCertificate");
        updated.Server.ImageUpload.Host = ReadText("Server.ImageUpload.Host");
        updated.Server.ImageUpload.Port = ReadInt("Server.ImageUpload.Port", "Upload Port", 1, 65535);
        ...
        updated.Server.ImageUpload.ConnectionTimeoutSeconds = ReadInt("Server.ImageUpload.ConnectionTimeoutSeconds", "Upload Connection Timeout Seconds", 1);
        updated.Server.ImageUpload.OperationTimeoutSeconds = ReadInt("Server.ImageUpload.OperationTimeoutSeconds", "Upload Operation Timeout Seconds", 1);
        updated.Server.ImageUpload.TotalTimeoutSeconds = ReadInt(
            "Server.ImageUpload.TotalTimeoutSeconds",
            "Total Upload Timeout Seconds",
            1);
    ...
    private static void ValidateOptions(AppOptions options)
    {
        ...
        {
            var assignedPorts = new List<(string Name, string Port)>
            {
                ("Weighbridge", options.Weighbridge.Serial.PortName),
                ("IN RFID", options.Hardware.InRfid.PortName),
                ("OUT RFID", options.Hardware.OutRfid.PortName),
                ("Control Unit", options.Hardware.Control.PortName)
            };
            if (options.Hardware.BluetoothControlEnabled &&
                !string.IsNullOrWhiteSpace(options.Hardware.BluetoothControl.PortName))
            {
                assignedPorts.Add(("Bluetooth Backup", options.Hardware.BluetoothControl.PortName));
            }

            var duplicatePort = assignedPorts
                .GroupBy(item => item.Port.Trim(), StringComparer.OrdinalIgnoreCase)
                .FirstOrDefault(group => group.Count() > 1);
            if (duplicatePort is not null)
            {
                throw new InvalidOperationException(
                    $"COM port {duplicatePort.Key} is assigned more than once: {string.Join(", ", duplicatePort.Select(item => item.Name))}.");
            }
        }

        if (options.Weighbridge.ResetWeightKg >= options.Weighbridge.TargetWeightKg)
        {
            throw new InvalidOperationException("Reset weight must be lower than target weight.");
        }

        ValidateCamera(options.Cameras.Camera1, "Camera 1");
        ValidateCamera(options.Cameras.Camera2, "Camera 2");
        ValidateCamera(options.Cameras.Camera3, "Camera 3");
        ValidateCamera(options.Cameras.Camera4, "Camera 4");

        if (options.Server.Enabled)
        {
            if (!options.Server.Mqtt.Enabled && !options.Server.Api.Enabled)
            {
                throw new InvalidOperationException(
                    "Enable at least one server method: MQTT or HTTP API.");
            }
            ...
            if (options.Server.Mqtt.Enabled)
            {
                ...
                {
                    throw new InvalidOperationException(
                        "MQTT Broker Host is required when MQTT Method is enabled.");
                }
                ...
                {
                    throw new InvalidOperationException(
                        "MQTT Base Topic or Publish Topic is required when MQTT Method is enabled.");
                }
            ...
            {
                if (!Uri.TryCreate(options.Server.Api.Endpoint, UriKind.Absolute, out var apiUri) ||
                    (apiUri.Scheme != Uri.UriSchemeHttp && apiUri.Scheme != Uri.UriSchemeHttps))
                {
                    throw new InvalidOperationException(
                        "API Endpoint must be a valid HTTP or HTTPS URL when API Method is enabled.");
                }

                if (!string.Equals(options.Server.Api.Method, "POST", StringComparison.OrdinalIgnoreCase))
                {
                    throw new InvalidOperationException("HTTP API Method must be POST.");
                }

                if (string.IsNullOrWhiteSpace(options.Server.Api.Action))
                {
                    throw new InvalidOperationException("API Action is required when API Method is enabled.");
                }
            }
        }

        if (!ImageUploadService.IsSupportedMode(options.Server.ImageUpload.Mode))
        {
            throw new InvalidOperationException(
                "Image Upload Mode must be Disabled, Sftp, or Ftp.");
        }

        if (ImageUploadService.IsSftpMode(options.Server.ImageUpload.Mode) ||
            ImageUploadService.IsFtpMode(options.Server.ImageUpload.Mode))
        {
            var protocol = ImageUploadService.GetProtocolName(
            ...
            {
                throw new InvalidOperationException(
                    $"{protocol} Host and Username are required when image upload mode is {options.Server.ImageUpload.Mode}.");
            }
        }

        if (ImageUploadService.IsFtpMode(options.Server.ImageUpload.Mode) &&
            options.Server.ImageUpload.UseTls &&
            !string.IsNullOrWhiteSpace(options.Server.ImageUpload.TlsCertificateSha256Fingerprint) &&
            !ImageUploadService.IsValidSha256CertificateFingerprint(
                options.Server.ImageUpload.TlsCertificateSha256Fingerprint))
        {
            throw new InvalidOperationException(
                "FTPS Certificate SHA-256 Fingerprint must contain exactly 64 hexadecimal characters. " +
                "Colon, dash, and space separators are allowed.");
        }
    ...
    private static void ApplyNonHardwareOptions(AppOptions source, AppOptions destination)
    {
        ...
        destination.Weighbridge = source.Weighbridge;
        ...
        destination.Audio = source.Audio;
