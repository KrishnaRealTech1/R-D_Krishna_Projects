...
public sealed class LaneProcessor
{
    ...
    private bool _inSensorHigh;
    private bool _outSensorHigh;
    private DateTimeOffset? _inSensorDetectedAt;
    private DateTimeOffset? _outSensorDetectedAt;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (becameArmed)
        {
            var waitingFor = _options.Processing.SensorInterfaceEnabled
                ? "matching lane sensor + first RFID"
                : "first IN/OUT RFID";
            await _logger.StatusAsync(
                $"WEIGHBRIDGE TARGET REACHED: {weightKg:0.###} kg >= {target:0.###} kg. " +
                $"System ARMED; waiting for {waitingFor}.",
                cancellationToken);

            var laneWaitText = _options.Processing.SensorInterfaceEnabled
                ? $"Weight target reached ({weightKg:0.###} kg) - waiting for sensor + RFID"
                : $"Weight target reached ({weightKg:0.###} kg) - waiting for first RFID";
            PublishWaitingState(LaneDirection.In, laneWaitText);
            PublishWaitingState(LaneDirection.Out, laneWaitText);
        }
    ...
    /// <summary>
    /// Tracks the optional IN/OUT vehicle sensors. When SensorInterfaceEnabled is
    /// false these events do not gate processing. When enabled, the matching lane
    /// must have a recent HIGH state before its RFID can start the weighing cycle.
    /// </summary>
    public Task RegisterSensorStateAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        if (!_options.Processing.SensorInterfaceEnabled)
        {
        ...
        lock (_cycleSync)
        {
            var detectedAt = isHigh ? DateTimeOffset.Now : (DateTimeOffset?)null;
            if (direction == LaneDirection.In)
            {
                _inSensorHigh = isHigh;
                _inSensorDetectedAt = detectedAt;
            }
            else
            {
                _outSensorHigh = isHigh;
                _outSensorDetectedAt = detectedAt;
            }
        }

        return _logger.StatusAsync(
            $"{LaneText(direction)} sensor {(isHigh ? "DETECTED" : "RELEASED")}. " +
            "Sensor interface is enabled for iAWS processing.",
            cancellationToken);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        lock (_cycleSync)
        {
            ...
            else if (_options.Processing.SensorInterfaceEnabled &&
                     !IsSensorReadyNoLock(direction, DateTimeOffset.Now, out ignoreReason))
            {
                // Sensor gating is intentionally checked before the direction lock.
                // The RFID reader can retry continuously until all required inputs are present.
            }
    ...
    private bool IsSensorReadyNoLock(
        LaneDirection direction,
        DateTimeOffset now,
        out string reason)
    {
        var isHigh = direction == LaneDirection.In ? _inSensorHigh : _outSensorHigh;
        var detectedAt = direction == LaneDirection.In ? _inSensorDetectedAt : _outSensorDetectedAt;

        if (!isHigh || !detectedAt.HasValue)
        {
            reason = $"{LaneText(direction)} sensor has not detected a vehicle";
            return false;
        }

        var validitySeconds = Math.Max(1, _options.Processing.SensorValiditySeconds);
        if (now - detectedAt.Value > TimeSpan.FromSeconds(validitySeconds))
        {
            if (direction == LaneDirection.In)
            {
                _inSensorHigh = false;
                _inSensorDetectedAt = null;
            }
            else
            {
                _outSensorHigh = false;
                _outSensorDetectedAt = null;
            }

            reason = $"{LaneText(direction)} sensor detection expired after {validitySeconds} seconds";
            return false;
        }
    ...
    private async Task<decimal> WaitForStableProcessingWeightAsync(
        CancellationToken cancellationToken)
    {
        const decimal toleranceKg = 0.5m;
        var requiredStableTime = TimeSpan.FromSeconds(3);
        decimal candidate;

        lock (_cycleSync)
        {
            candidate = _currentWeightKg;
        }
        ...
        while (true)
        {
            ...
            lock (_cycleSync)
            {
                current = _currentWeightKg;
            }

            var now = DateTimeOffset.UtcNow;
            if (Math.Abs(current - candidate) <= toleranceKg)
            {
                if (now - stableSince >= requiredStableTime)
                {
                    return current;
                }
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Stage 1: immediately show the detected vehicle while the lane goes RED.
        // HardwareGateway sends the common display command in the same locked write:
        //   IN/OUT RED <vehicle>  +  RED <vehicle>
        ...
        PublishLane(
            ...
            rfid,
            vehicleNumber,
            $"{lane} RFID detected - waiting for weight to stay stable for 3 seconds");

        // Preserve the configured processing delay, but let the 3-second stability
        // confirmation run in parallel so the existing cycle is not needlessly slowed.
        var waitSeconds = Math.Max(0, _options.Processing.ProcessWaitSeconds);
        var processingDelayTask = waitSeconds > 0
            ? Task.Delay(TimeSpan.FromSeconds(waitSeconds), cancellationToken)
            : Task.CompletedTask;

        var stableWeight = await WaitForStableProcessingWeightAsync(cancellationToken);

        // Stage 2: after the live weight remains stable for 3 continuous seconds,
        // send the exact common display message requested by iAWS.
        await _hardware.SendControlCommandAsync(
            $"RED {vehicleNumber} {stableWeight:0.###}",
            cancellationToken);
        await _logger.StatusAsync(
            $"{lane} stable-weight display command sent: RED {vehicleNumber} {stableWeight:0.###}",
            cancellationToken);
        ...
        PublishLane(
            ...
            rfid,
            vehicleNumber,
            $"Capturing 4 cameras at stable weight {stableWeight:0.###} kg");

        await processingDelayTask;
        ...
        PublishLane(
            ...
            rfid,
            vehicleNumber,
            $"{lane} WEIGHT CAPTURED - {stableWeight:0.###} KG");
        await _logger.StatusAsync(
            $"{lane} iAWS transaction {trip.Id} saved locally. RFID={rfid}; " +
            $"weight={stableWeight:0.###} kg; target={trip.TargetWeightKg:0.###} kg; cameras=4. " +
