...
public partial class App : Application
{
    ...
    protected override async void OnStartup(StartupEventArgs e)
    {
        ...
        try
        {
            ...
            await ReportStartupProgressAsync(
                ...
                "Creating RFID, weighbridge, four-camera, hardware, MQTT/API, upload and user-interface services.",
                40);
            ...
            _host = Host.CreateDefaultBuilder()
                .ConfigureServices(services =>
                {
                    ...
                    services.AddSingleton<HttpApiPublishService>();
    ...
    private static AppOptions LoadOptions()
    {
        ...
        ApplyAudioCompatibilityDefaults(options);
    ...
    private static void ApplyWeighbridgeCompatibilityDefaults(AppOptions options)
    {
        ...
        options.Weighbridge.DataPrefix = options.Weighbridge.DataPrefix?.Trim() ?? string.Empty;
    ...
    private static void ApplyImportCompatibilityDefaults(AppOptions options)
    {
        ...
        const string currentVehicleApiEndpoint =
            "https://gov.igps.io/aws_madurai/api/csv_link.php";

        // One-time compatibility migration from the older iTOLL/iAWS vehicle CSV API.
        // Existing installations keep their configuration under Documents, so without this
        // migration they would continue calling the old 404 endpoint after an application update.
        var legacyEndpointConfigured =
            string.Equals(
                options.Import.ApiEndpoint?.Trim(),
                "https://gov.igps.io/aws_admin/Server/csv_link.php",
                StringComparison.OrdinalIgnoreCase) ||
            (options.Import.ApiSources?.Any(source =>
                source is not null &&
                string.Equals(
        ...
        // Earlier iAWS test builds allowed several auto-generated "Vehicle CSV API N"
        // rows. Those rows are not separate Madurai APIs and caused the six 404
        // requests seen in production. Collapse that legacy generated list to the one
        // confirmed vehicle-import endpoint. User-created named sources are left alone.
        var sources = options.Import.ApiSources ?? [];
        var legacyGeneratedMultiSourceList = sources.Count > 1 &&
            sources.All(source =>
            {
                var name = source?.Name?.Trim() ?? string.Empty;
                return name.Equals("Primary Vehicle CSV API", StringComparison.OrdinalIgnoreCase) ||
                       name.StartsWith("Vehicle CSV API ", StringComparison.OrdinalIgnoreCase);
            });

        if (legacyEndpointConfigured || legacyGeneratedMultiSourceList)
        {
            options.Import.ApiEndpoint = currentVehicleApiEndpoint;
            options.Import.ApiUsername = string.Empty;
            options.Import.ApiSources =
            [
                new VehicleApiSourceOptions
                {
                    Name = "Primary Vehicle CSV API",
                    Enabled = true,
                    Priority = 1,
                    Endpoint = currentVehicleApiEndpoint,
                    Username = string.Empty,
                    RequestTimeoutSeconds = Math.Clamp(
                        options.Import.ApiRequestTimeoutSeconds <= 0
        ...
        if (!Enum.TryParse<RfidVehicleAccess.Models.RfidAccessType>(
                options.Import.DefaultAccessType,
                true,
                out _))
        {
            options.Import.DefaultAccessType = defaults.DefaultAccessType;
        }
        ...
        options.Import.ApiSources = options.Import.ApiSources
            ...
            .GroupBy(source => source.Endpoint, StringComparer.OrdinalIgnoreCase)
            .Select(group => group
    ...
    private static void ApplyAudioCompatibilityDefaults(AppOptions options)
    {
        options.Audio ??= new AudioOptions();
        options.Audio.RfidDetected ??= new AudioCueOptions();
        options.Audio.ProcessCompleted ??= new AudioCueOptions();
        options.Audio.RfidDetected.RepeatCount = Math.Clamp(
            options.Audio.RfidDetected.RepeatCount,
            1,
            100);
        options.Audio.ProcessCompleted.RepeatCount = Math.Clamp(
            options.Audio.ProcessCompleted.RepeatCount,
            1,
            100);
        options.Audio.RfidDetected.FilePath =
            options.Audio.RfidDetected.FilePath?.Trim() ?? string.Empty;
        options.Audio.ProcessCompleted.FilePath =
            options.Audio.ProcessCompleted.FilePath?.Trim() ?? string.Empty;
    }
    ...
    private static void ApplyMqttTopicCompatibilityDefaults(AppOptions options)
    {
        ...
        options.Server.Api ??= new HttpApiOptions();
        ...
        {
            api.Endpoint = "http://dev.igps.io/aws_raw_data/iAWS_universalAPi.php";
        }

        api.Method = "POST";
        api.RequestTimeoutSeconds = Math.Clamp(api.RequestTimeoutSeconds, 1, 300);
        if (string.IsNullOrWhiteSpace(api.Action))
        {
            api.Action = "WEIGH";
        }

        if (string.IsNullOrWhiteSpace(api.DefaultMaterialType))
        {
            api.DefaultMaterialType = "Municipal Solid Waste";
        }
    }
