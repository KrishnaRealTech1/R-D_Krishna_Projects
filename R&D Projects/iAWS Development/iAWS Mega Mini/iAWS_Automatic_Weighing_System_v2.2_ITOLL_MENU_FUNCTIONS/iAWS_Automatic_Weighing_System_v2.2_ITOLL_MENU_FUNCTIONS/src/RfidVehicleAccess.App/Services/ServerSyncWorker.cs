    ...
    SystemEventHub eventHub) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        else
        {
            await logger.ServerAsync(
                ...
                $"Image upload: {(imageUploader.IsEnabled ? imageUploader.ProtocolName + " enabled" : "disabled")}.",
                stoppingToken);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            await logger.ServerAsync(
                $"{transactionPrefix} IMAGE SKIPPED | image upload is disabled.",
                cancellationToken);
            return null;
        }
        ...
        var result = await imageUploader.UploadAsync(trip, cancellationToken)
            ?? throw new IOException($"{imageUploader.ProtocolName} upload returned no result.");
        ...
        await logger.ServerAsync(
            $"{transactionPrefix} IMAGE UPLOAD COMPLETE | protocol={result.Protocol} | remote={result.RemotePath} | " +
