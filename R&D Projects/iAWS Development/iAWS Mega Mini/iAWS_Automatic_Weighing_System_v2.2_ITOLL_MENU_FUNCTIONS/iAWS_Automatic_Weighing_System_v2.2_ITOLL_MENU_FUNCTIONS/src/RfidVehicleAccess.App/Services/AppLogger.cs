...
public sealed class AppLogger
{
    ...
    {
        var configuredPath = channel switch
        {
            LogChannel.Status => _options.Storage.StatusLogFile,
            LogChannel.Api => _options.Storage.ApiLogFile,
            _ => _options.Storage.ServerLogFile
        };
        ...
        {
            configuredFileName = channel switch
            {
                LogChannel.Status => "status.log",
                LogChannel.Api => "api.log",
                _ => "server.log"
            };
        }

        var channelFolderName = channel switch
        {
            LogChannel.Status => "Status",
            LogChannel.Api => "Api",
            _ => "Server"
        };
