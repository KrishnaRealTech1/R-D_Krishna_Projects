...
public sealed class VehicleApiImportService : IDisposable
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        await _logger.StatusAsync(
            $"Requesting raw registered-vehicle CSV from source '{source.Name}' " +
            $"({endpoint.Host}).",
            cancellationToken);
        ...
        try
        {
            // The Madurai vehicle API returns the registered vehicles directly as raw CSV.
            // Do not POST JSON and do not expect a JSON object containing a CSV link.
            ...
            var importResult = await _importService.ImportApiMinimalAsync(
                tempFile,
                cancellationToken);
            await _logger.StatusAsync(
    ...
    private static Uri ResolveCsvUri(Uri endpoint, string responseText)
    {
        ...
        throw new InvalidOperationException(
            "The vehicle API responded successfully but did not return a supported raw vehicle CSV " +
            "and did not provide a usable CSV download link.");
    }
    ...
    private static bool LooksLikeCsv(string value)
    {
        ...
        var hasVehicle =
            ...
            header.Contains("vehicle_n", StringComparison.Ordinal) ||
            header.Contains("vehicle_num", StringComparison.Ordinal) ||
