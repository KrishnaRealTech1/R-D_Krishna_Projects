...
public sealed class ImageUploadService(AppOptions options) : IDisposable
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        return new ImageUploadResult(
            ...
            reusedConnection,
            reusedConnection
                ? $"Existing SFTP session reused; upload completed successfully; remote path={remotePath}."
                : $"SFTP session connected and authenticated; upload completed successfully; remote path={remotePath}.");
    }
        ...
        CancellationToken cancellationToken)
    {
            ...
            LooksLikeTlsCertificateFailure(ex.Message))
        {
            throw BuildFtpsCertificateException(
                upload,
                ex,
                ReadWinScpServerResponses(sessionLogPath));
        }
        catch (WinSCP.SessionRemoteException ex)
        {
            ...
            throw new IOException(
                ...
                $"Server reply: {ReadWinScpServerResponses(sessionLogPath)} " +
                $"WinSCP response: {CollapseForLog(ex.Message)}",
                ex);
        }
        finally
        {
    ...
    private static AuthenticationException BuildFtpsCertificateException(
        ImageUploadOptions upload,
        WinSCP.SessionRemoteException exception,
        string serverReply)
    {
        var rawResponse = CollapseForLog(exception.Message);
        var fingerprint = NormalizeSha256CertificateFingerprint(
            upload.TlsCertificateSha256Fingerprint);
        ...
        if (fingerprint.Length == 64)
        {
            guidance =
                ...
                "Verify that the fingerprint still matches the current server certificate and that the configured host/port are correct.";
        }
        else if (upload.AllowInvalidTlsCertificate)
        {
            guidance =
                "The FTPS TLS handshake failed even though invalid server certificates are currently allowed. " +
                "Verify the FTPS host, port, explicit-TLS mode, username/password, and the server TLS configuration.";
        }
        else
        {
            guidance =
                "FTPS server certificate validation failed. Verify the server certificate in WinSCP and paste its SHA-256 fingerprint into " +
                "Server Panel > Server > Image Upload > FTPS Certificate SHA-256 Fingerprint, or correct the server certificate/chain.";
        }

        return new AuthenticationException(
            $"{guidance} FTPS server reply: {serverReply} WinSCP response: {rawResponse}",
            exception);
    }
    ...
    private static string ReadWinScpServerResponses(string sessionLogPath)
    {
        try
        {
            ...
            var serverLines = File.ReadLines(sessionLogPath)
