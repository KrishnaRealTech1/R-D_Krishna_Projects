# MQTT and image-upload synchronization (SFTP or FTP)

## Processing order

1. Read pending trips from SQLite.
2. When the local image exists and image upload mode is `Sftp` or `Ftp`, upload the image.
...
If SFTP, FTP, or MQTT fails, the trip remains pending and is retried during the next cycle.
...
`Server.ImageUpload.Mode` accepts `Disabled`, `Sftp`, or `Ftp`. The same Host, Port, Username, Password, Remote Directory, and timeout fields are used for either transfer protocol. Typical ports are 22 for SFTP and 21 for FTP. The Server Panel automatically changes 22 to 21 when switching from SFTP to FTP, and 21 to 22 when switching back, while preserving custom port values.

FTP support is standard passive-mode FTP and is not FTPS. FTP credentials and image data are not encrypted in transit, so SFTP remains the preferred mode on untrusted networks.
...
SFTP uses a reusable authenticated SSH connection. FTP uses standard passive-mode FTP requests. Both modes prepare the configured remote directory when possible and apply `Server.ImageUpload.TotalTimeoutSeconds` as the wall-clock limit for one image attempt. After a transfer failure, a short in-memory cooldown prevents the rest of the same batch from repeatedly waiting for the identical unavailable server.
...
- A Manual or Auto approved exception continues through the same current-lane camera capture, configured SFTP/FTP image upload and `Device_Response` publish process as a normal trip.
