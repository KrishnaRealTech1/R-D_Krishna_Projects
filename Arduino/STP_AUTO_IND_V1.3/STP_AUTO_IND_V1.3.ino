 ...
 * - Sensor inputs A3 and A4 require external 10 kOhm pull-downs.
 ***************************************************************/
...
const unsigned long SENSOR_RELEASE_MINIMUM_HIGH_MS = 2000UL;
...
struct FilteredSensor {
    ...
    unsigned long detectedHighStartTime;
};
...
void initialiseSensor(FilteredSensor &sensor) {
    ...
    sensor.awaitingRelease = false;
    sensor.releaseEligible = false;
    sensor.detectedHighStartTime = 0;
}
...
void printSensorReleased(SensorLane lane) {
    if (lane == SENSOR_LANE_IN) {
        Serial.println(F("IN Realeased"));
    } else {
        Serial.println(F("OUT Realeased"));
    }
}

void updateReleaseEligibility(FilteredSensor &sensor,
                              unsigned long now) {
    if (!sensor.awaitingRelease ||
        sensor.releaseEligible ||
        !sensor.filteredState ||
        !sensor.candidateState) {
        return;
    }

    if (now - sensor.detectedHighStartTime >=
        SENSOR_RELEASE_MINIMUM_HIGH_MS) {
        sensor.releaseEligible = true;
    }
}
...
void updateSensor(FilteredSensor &sensor) {
    ...
    updateReleaseEligibility(sensor, now);
    ...
    if (!sensor.filteredState) {
        if (sensor.awaitingRelease && sensor.releaseEligible) {
            printSensorReleased(sensor.lane);
        }
    ...
    if (sensor.armed) {
        ...
        sensor.armed = false;
        sensor.awaitingRelease = true;
        sensor.releaseEligible = false;
        sensor.detectedHighStartTime = sensor.candidateStartTime;
    }
}
