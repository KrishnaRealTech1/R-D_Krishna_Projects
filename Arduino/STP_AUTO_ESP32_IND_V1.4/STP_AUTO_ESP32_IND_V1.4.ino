 ...
 * ESP32 DevKit V1 / ESP32-WROOM-32 version - V1.4 UNO SENSOR/SIGNAL LOGIC
 *
 * Preserves the Arduino Uno application logic and adds:
...
void printOutputState(const __FlashStringHelper *name, byte pin) {
    ...
    Serial.print(pin);
    Serial.print(F(":"));
    Serial.println(digitalRead(pin) == OUTPUT_ON_LEVEL ? F("ON") : F("OFF"));
}
...
void printSignalState(const __FlashStringHelper *name, byte pin) {
    ...
    Serial.print(pin);
    Serial.print(F(":"));
    Serial.print(digitalRead(pin) == SIGNAL_ON_LEVEL ? F("ON") : F("OFF"));
    Serial.print(F(" RAW="));
...
void printStatus() {
    ...
    Serial.print(F("IN PE ADC=")); Serial.println(analogRead(IN_PE_SENSOR_PIN));
    Serial.print(F("OUT PE ADC=")); Serial.println(analogRead(OUT_PE_SENSOR_PIN));
    Serial.print(F("IN RELEASE=")); Serial.println(digitalRead(IN_RELEASE_SENSOR_PIN));
    Serial.print(F("OUT RELEASE=")); Serial.println(digitalRead(OUT_RELEASE_SENSOR_PIN));
    Serial.println(F("--------------"));
}

void printHelp() {
    Serial.println(F("Commands:"));
    Serial.println(F("IN GRN | IN ORG | IN RED | IN RED <VEHICLE>"));
    Serial.println(F("OUT GRN | OUT ORG | OUT RED | OUT RED <VEHICLE>"));
    Serial.println(F("IN BUZZER | IN BUZZER OFF | IN ALL OFF"));
    Serial.println(F("OUT BUZZER | OUT BUZZER OFF | OUT ALL OFF"));
    Serial.println(F("OPEN IN BB | CLOSE IN BB | OPEN OUT BB | CLOSE OUT BB"));
    Serial.println(F("AUTO CLOSE BB | GRN | ORG | RED <VEHICLE>"));
...
void processSerialCommand(char *command) {
    ...
    if (commandStartsWith(command, PSTR("IN RED "))) {
        ...
        trimInPlace(vehicle);
        if (vehicle[0] == '\0') {
            Serial.println(F("ERR: vehicle number missing"));
            return;
        }
        setInRed();
...
void finishCurrentSerialCommand() {
    if (serialBufferOverflow) {
        Serial.println(F("ERR: serial command too long"));
...
void setup() {
    ...
    Serial.println();
    Serial.println(F("STP AUTO ESP32 V1.4 UNO SENSOR/SIGNAL LOGIC READY"));
    Serial.println(F("Serial: 9600 baud. Send HELP or STATUS."));
    printStatus();
}
