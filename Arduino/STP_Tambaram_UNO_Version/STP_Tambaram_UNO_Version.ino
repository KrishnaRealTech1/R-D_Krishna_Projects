...
void executeReleaseAction(SensorLane lane) {
    if (lane == SENSOR_LANE_IN) {
        // IN RELEASE: orange OFF + immediate IN barrier close pulse.
        digitalWrite(IN_ORANGE_PIN, OUTPUT_OFF_LEVEL);

        // Cancel a pending delayed manual close to prevent a second pulse.
        inBBPendingClose = false;
        startRelayPulse(IN_BB_CLOSE_PIN);

        Serial.println(F("IN RELEASE ACTION"));
    } else {
        // OUT RELEASE: orange OFF + immediate OUT barrier close pulse.
        digitalWrite(OUT_ORANGE_PIN, OUTPUT_OFF_LEVEL);

        outBBPendingClose = false;
        startRelayPulse(OUT_BB_CLOSE_PIN);

        Serial.println(F("OUT RELEASE ACTION"));
    }
}
...
void updateReleaseSensor(ReleaseSensor &sensor) {
    ...
    if (sensor.filteredState) {
        ...
        if (sensor.lane == SENSOR_LANE_IN) {
            Serial.println(F("IN RELEASE SENSOR HIGH"));
        } else {
            Serial.println(F("OUT RELEASE SENSOR HIGH"));
        }
        return;
    }
