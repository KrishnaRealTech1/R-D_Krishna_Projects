# 1 "AWS_Standard_Weight_Converter_V2.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 288 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "AWS_Standard_Weight_Converter_V2.c" 2
# 35 "AWS_Standard_Weight_Converter_V2.c"
...
# 35 "AWS_Standard_Weight_Converter_V2.c" 2


# 1 "C:\\Program Files\\Microchip\\xc8\\v2.31\\pic\\include\\c99\\stdbool.h" 1 3
# 37 "AWS_Standard_Weight_Converter_V2.c" 2
...
# 156 "AWS_Standard_Weight_Converter_V2.c"
...
static _Bool uart1_try_read(uint8_t *byte);
...
static _Bool is_digit_ascii(char c);
static _Bool is_alpha_ascii(char c);
static char upper_ascii(char c);
static _Bool contains_ci(const char *text, const char *pattern);
static _Bool contains_isolated_letter_ci(const char *text, char letter);
...
static _Bool parse_weight_line(const char *line, WeightData *weight);
...
void __attribute__((picinterrupt(("")))) isr(void)
{
    ...
    {
        TMR0H = ((uint8_t)(65036u >> 8));
        TMR0L = ((uint8_t)(65036u & 0xFF));
        INTCONbits.TMR0IF = 0;
        g_millis++;
    }
}
...
void main(void)
{
    ...
    timer0_init();
    startup_led_test();
# 266 "AWS_Standard_Weight_Converter_V2.c"
    while (1)
    {
        ...
        {
            uint32_t now = millis();
            if ((uint32_t)(now - last_rx_time) >= 80u)
            {
...
{
    uint32_t brg;
    brg = ((16000000UL / 4UL) + (baud / 2UL)) / baud;
    if (brg > 0UL)
    {
        brg--;
    }
...
# 1032 "AWS_Standard_Weight_Converter_V2.c"
