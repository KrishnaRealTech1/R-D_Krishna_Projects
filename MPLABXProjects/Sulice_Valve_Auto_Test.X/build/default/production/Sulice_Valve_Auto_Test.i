...
# 2 "<built-in>" 2
# 1 "Sulice_Valve_Auto_Test.c" 2
# 41 "Sulice_Valve_Auto_Test.c"
...
# 106 "Sulice_Valve_Auto_Test.c"
static __attribute__((inline)) void wdt_enable(void){ WDTCONbits.SWDTEN = 1; }
static __attribute__((inline)) void wdt_disable(void){ WDTCONbits.SWDTEN = 0; }
# 135 "Sulice_Valve_Auto_Test.c"
volatile uint32_t g_ms = 0;
volatile uint32_t g_last_pulse_ms = 0;


static __attribute__((inline)) uint32_t ms_now(void){
...
volatile _Bool g_counting_enabled = 0;
...
volatile _Bool g_offline_active = 0;


static volatile uint8_t g_pulses_since_checkpoint = 0;
static volatile _Bool g_checkpoint_pending = 0;
...
static _Bool g_install_pending = 0;


static uint8_t g_run_start_pos = 0;

static __attribute__((inline)) uint8_t status_code_from_outputs(void){
    if (LATBbits.LATB3 && !LATBbits.LATB4) return 1;
    if (LATBbits.LATB4 && !LATBbits.LATB3) return 2;
    return 0;
}
static __attribute__((inline)) _Bool seq_newer(uint8_t a, uint8_t b){
    return (uint8_t)(a - b) < 128U;
}
static _Bool ee_is_all_ff(void){
    for(uint16_t i=0;i<36*7;i++){
        if(ee_read((uint8_t)(0x00 + i)) != 0xFF) return 0;
    }
    return 1;
}

static _Bool ee_slot_valid_at(uint8_t slot, ee_slot_t *out) {
    uint8_t base = 0x00 + slot*7;
    ee_slot_t s;
    ...
    c ^= s.lastcnt;
    _Bool ok = (s.tag==0x5A) && (s.inv == (uint8_t)~s.count) && (s.crc == c) &&
              (s.count>=1 && s.count<=250) &&
...
static void ee_store_full(uint8_t count, uint8_t status, uint8_t lastcnt){
    ...
    uint8_t next = (uint8_t)((g_slot_index + 1U) % 36);
    ...
    uint8_t base = (uint8_t)(0x00 + next*7);
...
static void ee_store_setpoint(uint8_t count){
    if (count < 1) count = 1;
    if (count > 250) count = 250;
    if (g_position > count) g_position = count;
...
static void uart_init(void) {
    ...
    uint16_t spbrg = (16000000UL/(4UL*9600UL)) - 1;
...
volatile _Bool rx_line_ready = 0;
...
static uint16_t parse_uint(const char* p, _Bool* ok){
...
static void i2c_bus_recover(void){
    ...
    for(uint8_t i=0;i<9;i++){
        ...
        TRISCbits.TRISC3 = 0; LATCbits.LATC3 = 0; _delay((unsigned long)((5)*(16000000UL/4000000.0)));

        TRISCbits.TRISC3 = 1; _delay((unsigned long)((5)*(16000000UL/4000000.0)));
        __asm("clrwdt");
    }
...
static void i2c_init(void){
    ...
    SSP1CON1 = 0x28;
    SSP1CON2 = 0x00;
    SSP1ADD = (uint8_t)((16000000UL/(4UL*100000UL))-1);
    SSP1STAT = 0x80;
...
static void i2c_wait_idle(void){
    while ( (SSP1CON2 & 0x1F) || SSP1STATbits.R_W ) { __asm("clrwdt"); }
}
static _Bool i2c_start(void){
    i2c_wait_idle();
    SSP1CON2bits.SEN = 1;
    while (SSP1CON2bits.SEN){ __asm("clrwdt"); }
    _delay((unsigned long)((2)*(16000000UL/4000000.0)));
    return 1;
}
static void i2c_stop(void){
    i2c_wait_idle();
    SSP1CON2bits.PEN = 1;
    while (SSP1CON2bits.PEN){ __asm("clrwdt"); }
    _delay((unsigned long)((2)*(16000000UL/4000000.0)));
}
static _Bool i2c_write_byte(uint8_t b){
    i2c_wait_idle();
    PIR1bits.SSP1IF = 0;
    SSP1BUF = b;
    ...
    while(!PIR1bits.SSP1IF) { __asm("clrwdt"); }
    PIR1bits.SSP1IF = 0;
    return (SSP1CON2bits.ACKSTAT == 0);
}
static _Bool i2c_write_bytes(const uint8_t *data, uint8_t n){
    for (uint8_t i=0;i<n;i++){
        if (!i2c_write_byte(data[i])) return 0;
    }
    return 1;
}
static _Bool i2c_cmd(uint8_t addr7, const uint8_t *data, uint16_t n){
    ...
    for (uint8_t attempt=0; attempt<3; ++attempt){
        ...
        if (!i2c_write_byte(dev)){ i2c_stop(); _delay((unsigned long)((1)*(16000000UL/4000.0))); continue; }
...
static void oled_cmd1(uint8_t c){ uint8_t b[2]={0x00,c}; (void)i2c_cmd(0x3C,b,2); }
static void oled_cmd2(uint8_t c1,uint8_t c2){ uint8_t b[3]={0x00,c1,c2}; (void)i2c_cmd(0x3C,b,3); }
...
static void oled_clear(void){
    ...
    for(uint8_t p=0;p<8;p++){
        ...
        for (uint8_t blk=0; blk<8; ++blk){
            (void)i2c_cmd(0x3C, ctrl_and_zeros, sizeof(ctrl_and_zeros));
            __asm("clrwdt");
        }
    }
}

static _Bool oled_init_once(void){
...
static void oled_write_char(uint8_t page, uint8_t col, char c){
    ...
    (void)i2c_cmd(0x3C, cmds, 3);
    uint8_t hi[2] = {0x00, (uint8_t)(0x10 | ((col>>4)&0x0F))};
    uint8_t lo[2] = {0x00, (uint8_t)(0x00 | (col & 0x0F))};
    (void)i2c_cmd(0x3C, lo, 2);
    (void)i2c_cmd(0x3C, hi, 2);

    (void)i2c_cmd(0x3C, buf, 7);
}
...
static void tmr1_init(void){
    ...
    TMR1H = (uint8_t)((uint16_t)(65536u - 500u) >> 8);
    TMR1L = (uint8_t)((uint16_t)(65536u - 500u) & 0xFF);
    PIR1bits.TMR1IF = 0;
    PIE1bits.TMR1IE = 1;
    T1CONbits.TMR1ON = 1;
}
...
static void osc_init(void){
    ...
    uint16_t tmo = 30000;
    while(!OSCCONbits.HFIOFS && tmo--) { __nop(); }
}
...
static uint8_t g_display_last_offline = 0xFF;
static uint8_t g_display_last_offdir = 0xFF;


static __attribute__((inline)) void stop_all_relays(void){
    (LATBbits.LATB1 = !!(0));
    (LATBbits.LATB2 = !!(0));
}
static __attribute__((inline)) void set_open_status_only(void){
    (LATBbits.LATB3 = !!(1));
    (LATBbits.LATB4 = !!(0));
}
static __attribute__((inline)) void set_close_status_only(void){
    (LATBbits.LATB3 = !!(0));
    (LATBbits.LATB4 = !!(1));
}
static __attribute__((inline)) _Bool status_is_open(void){ return LATBbits.LATB3 && !LATBbits.LATB4; }
static __attribute__((inline)) _Bool status_is_close(void){ return LATBbits.LATB4 && !LATBbits.LATB3; }

static void start_open_cycle(void){
    g_pulse_count = 0;
    g_run_start_pos = g_position;
    g_run_dir = DIR_OPEN;
    g_counting_enabled = 1;
    g_running = 1;
    (LATBbits.LATB2 = !!(0));
    (LATBbits.LATB1 = !!(1));
    (LATBbits.LATB3 = !!(0));
...
static void end_cycle_open_done(void){
    stop_all_relays();
    g_counting_enabled = 0;
    g_running = 0;
    g_run_dir = DIR_IDLE;
    g_position = g_setpoint;
...
static void debounce_mode_input(void){
    uint8_t raw = (PORTAbits.RA0) ? 1U : 0U;
    if ((uint32_t)(g_ms - pd_last_sample_ms) < 5U) return;
    pd_last_sample_ms = g_ms;
    if (raw == pd_sample_last) {
        if (pd_streak < 255U) pd_streak++;
        if (pd_streak >= 5U) { pd_filtered = raw; }
    } else {
        pd_streak = 0U;
        pd_sample_last = raw;
    }
...
static void debounce_manual_trigger(void){
    uint8_t raw = (PORTAbits.RA2) ? 1U : 0U;
    if ((uint32_t)(g_ms - man_last_sample_ms) < 5U) return;
...
static void pd_boot_selftest(void){
    ...
    const uint32_t until = g_ms + 2000U;
    _Bool printed = 0;
...
static void oled_refresh(void){
    ...
    _Bool offline = g_offline_active;
    int8_t offdir = 0;
    if(offline){
...
static void process_cmd_line(char *line){
    ...
    if(strncmp(p,"DFLT",4)==0){
        ee_store_setpoint(15);
        uart_puts("OK PULSE="); uart_putu16(15); uart_putc('\n');
    ...
    if(p[0]=='P' && (p[1]>='0' && p[1]<='9')){
        _Bool ok=0; uint16_t v = parse_uint(p+1,&ok);
        if(ok && v>=1 && v<=250){
            ee_store_setpoint((uint8_t)v);
            uart_puts("OK PULSE="); uart_putu16(v); uart_putc('\n');
        } else {
...
int main(void){
    ...
    (void)RCONbits.POR; (void)RCONbits.BOR;
    RCONbits.POR = 0; RCONbits.BOR = 0;
    __asm("clrwdt");


    i2c_init(); __asm("clrwdt");
    uart_init(); __asm("clrwdt");
    tmr1_init(); __asm("clrwdt");
    int0_init(); __asm("clrwdt");
    ...
    for(uint8_t c=0;c<21;c++) oled_write_char(1,c*6,' ');
    __asm("clrwdt");
    ...
    pd_filtered = (PORTAbits.RA0)?1U:0U;
    pd_sample_last = pd_filtered;
    pd_streak = 0U;
    pd_last_sample_ms = g_ms;

    man_filtered = (PORTAbits.RA2)?1U:0U;
    ...
    for(;;){
        ...
        if((now - g_hb_last_ms) >= 500){
            g_hb_last_ms = now;
            (LATBbits.LATB5 ^= 1);
        }


        if(rx_line_ready || (rxlen>0 && (now - rx_last_ms) > 600)){
            char line_local[64];
        ...
        if(!g_running && g_offline_active){
            if((now - g_last_pulse_ms) > 600){
                int8_t dir_guess = 0;
                if(status_is_close() && !status_is_open()) dir_guess = +1;
                else if(status_is_open() && !status_is_close()) dir_guess = -1;
                else dir_guess = 0;
                ...
                ee_store_full(g_setpoint, 0U, g_position);
        ...
        if(g_mode == MODE_AUTO){
            ...
            uint8_t want_open = (PORTAbits.RA1)?1U:0U;
            ...
            _Bool locked = ((int32_t)(g_ms - man_cycle_lock_until) < 0);

            if(!g_running && !locked){
                switch(man_state){
                    case MS_WAIT_LOW:
                        if(man_filtered==0U && (g_ms - man_level_since_ms) >= 150){
                            man_state = MS_WAIT_HIGH;
                        }
                        break;

                    case MS_WAIT_HIGH:
                        if(man_filtered==1U && (g_ms - man_level_since_ms) >= 40){
                            if(status_is_open()){
                                start_close_cycle();
