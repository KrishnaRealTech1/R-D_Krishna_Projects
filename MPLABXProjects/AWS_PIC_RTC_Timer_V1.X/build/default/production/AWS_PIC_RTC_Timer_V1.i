...
# 12 "AWS_PIC_RTC_Timer_V1.c" 2

# 1 "C:\\Program Files\\Microchip\\xc8\\v2.31\\pic\\include\\c90\\stdint.h" 1 3
# 13 "AWS_PIC_RTC_Timer_V1.c" 2

# 1 "C:\\Program Files\\Microchip\\xc8\\v2.31\\pic\\include\\c90\\stdbool.h" 1 3
# 14 "AWS_PIC_RTC_Timer_V1.c" 2
# 122 "AWS_PIC_RTC_Timer_V1.c"
typedef struct
{
    uint8_t second;
    uint8_t minute;
...
static _Bool Is_Mode1_Hour(uint8_t hour);
static _Bool Is_Mode2_Hour(uint8_t hour);
static _Bool Is_Mode3_Hour(uint8_t hour);
static _Bool Is_Night_Mode_Relay_On_Time(const RTC_Time *now);
static _Bool Is_Scheduled_Restart_Time(Reset_Mode mode, const RTC_Time *now);
...
# 874 "AWS_PIC_RTC_Timer_V1.c"
static Reset_Mode Read_Selected_Mode(void)
{
    _Bool dip1_high;
    _Bool dip2_high;

    dip1_high = (GPIObits.GP4 == 1);
    dip2_high = (GPIObits.GP5 == 1);

    if ((dip1_high == 1) && (dip2_high == 0))
    {
        return RESET_MODE_1;
    }

    if ((dip1_high == 0) && (dip2_high == 1))
    {
        return RESET_MODE_2;
    }

    if ((dip1_high == 1) && (dip2_high == 1))
    {
        return RESET_MODE_3;
    }
