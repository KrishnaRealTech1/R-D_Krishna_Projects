...
// Test mode trigger interval.
// RESET_MODE_TEST now triggers every 10 minutes:
// HH:00, HH:10, HH:20, HH:30, HH:40, HH:50
#define TEST_TRIGGER_INTERVAL_MINUTES   10
...
static bool Is_Test_Mode_10_Minute(uint8_t minute);
...
static bool Is_Test_Mode_10_Minute(uint8_t minute)
{
    if ((minute % TEST_TRIGGER_INTERVAL_MINUTES) == 0)
    {
        return true;
    }
...
static bool Is_Scheduled_Restart_Time(Reset_Mode mode, const RTC_Time *now)
{
    ...
    switch (mode)
    {
        ...
        case RESET_MODE_TEST:
            // Test Mode:
            // Earlier: every hour at HH:00.
            // Now: every 10 minutes at HH:00, HH:10, HH:20, HH:30, HH:40, HH:50.
            return Is_Test_Mode_10_Minute(now->minute);
