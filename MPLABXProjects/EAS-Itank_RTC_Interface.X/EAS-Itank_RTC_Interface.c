...
// -----------------------------------------------------------------------------
// Schedule settings - NEW REQUIREMENT ONLY
// -----------------------------------------------------------------------------
// Relay triggers once at the start of each allowed RTC hour (HH:00).
// DIP1 ON  = Sunday enabled
// DIP1 OFF = Sunday disabled
// DIP2 ON  = operate only from 08:00 through 18:00 inclusive
...
// DIP Switch 2 - PIN 2 - GPIO5 - 08:00..18:00 window enable
...
static bool Is_Scheduled_Relay_Time(const RTC_Time *now)
{
    ...
    // PRODUCTION mode only: every hour at HH:00.
    return (now->minute == TRIGGER_MINUTE);
}
