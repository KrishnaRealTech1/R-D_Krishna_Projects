...
# 2 "<built-in>" 2
# 1 "LLS_Test_Bench_Auto.c" 2
# 38 "LLS_Test_Bench_Auto.c"
...
# 154 "LLS_Test_Bench_Auto.c"
volatile uint32_t g_millis = 0;


static uint8_t dayMask = 0x7F;
static uint32_t restDelayMs = 60000UL;


static uint32_t preMotorDelayMs = 30000UL;
static uint32_t postMotorRelOffMs = 30000UL;


static uint32_t drainSingleMs = (60UL*1000UL);


static volatile char rxBuf[64];
...
static ram_rec_t ramLog[16];
static uint8_t ramLogHead = 0, ramLogCount = 0;


static _Bool btVerbose = 0;
...
static _Bool drain_single_active = 0;
static uint8_t drain_single_relay = 0;
...
static _Bool uart_line_available(char *outLine, uint8_t maxLen);
...
static _Bool i2c_write(uint8_t b);
static uint8_t i2c_read(_Bool ack);
...
static _Bool rtc_shadow_load(uint16_t *yr, uint8_t *mo, uint8_t *dt, uint8_t *dow, uint8_t *hh, uint8_t *mm, uint8_t *ss);


static void relays_all_off(void);
static void relays_function_off(uint8_t fn);
static void motor_set(uint8_t motor, _Bool on);
static void relay_set(uint8_t relay, _Bool on);
static _Bool relay_get(uint8_t relay);
static _Bool float_triggered(uint8_t relay);


static _Bool float_active_stable(uint8_t idx);

static _Bool today_enabled(void);
static _Bool dow_enabled(uint8_t dow);
...
static void fsm_resume_apply(fsm_t *F, fsm_state_t st, uint8_t idx, _Bool running,
                             _Bool stopreq, uint8_t est_age_s);
...
static _Bool ee_log_read_slot(uint8_t slot, ee_log_t *rec);
...
static __attribute__((inline)) uint32_t millis(void);
static __attribute__((inline)) _Bool elapsed(uint32_t start, uint32_t dur);
...
static uint8_t lcd_backpack = 0x08;

static void lcd_send(uint8_t val, _Bool rs);
...
static __attribute__((inline)) _Bool elapsed(uint32_t start, uint32_t dur) { return (uint32_t)(millis() - start) >= dur; }
...
int main(void) {
    ...
    _Bool rtc_osf = (rtc_read_reg(0x0F) & 0x80u) != 0;
    rtc_sanitize();
    ...
    F1 = (fsm_t){ .state=ST_IDLE, .sequence={1,2,0,0}, .idx=0, .motor=1, .tMark=0, .running=0, .stop_requested=0, .loop=1 };
    F2 = (fsm_t){ .state=ST_IDLE, .sequence={3,4,5,0}, .idx=0, .motor=2, .tMark=0, .running=0, .stop_requested=0, .loop=1 };
    ...
    if (lastSlot >= 0) {
        ee_log_t rec; if (ee_log_read_slot((uint8_t)lastSlot, &rec)) {
            ...
            LATBbits.LATB0 = (rec.relBits & 0x01)?1:0; LATBbits.LATB1 = (rec.relBits & 0x02)?1:0; LATBbits.LATB2 = (rec.relBits & 0x04)?1:0;
            LATBbits.LATB3 = (rec.relBits & 0x08)?1:0; LATBbits.LATB4 = (rec.relBits & 0x10)?1:0;
            LATAbits.LATA0 = (rec.motorBits & 0x01)?1:0; LATAbits.LATA1 = (rec.motorBits & 0x02)?1:0;
            _Bool f1run=(rec.flags&0x01)!=0, f2run=(rec.flags&0x02)!=0; _Bool f1stop=(rec.flags&0x08)!=0, f2stop=(rec.flags&0x10)!=0;
            if (f1run) fsm_resume_apply(&F1, (fsm_state_t)rec.f1_state, rec.f1_idx, 1, f1stop, f1_age_now);
            if (f2run) fsm_resume_apply(&F2, (fsm_state_t)rec.f2_state, rec.f2_idx, 1, f2stop, f2_age_now);
            log_event("RESUME");
        }
    }
...
static void init_gpio(void) {
    TRISBbits.TRISB0=0; TRISBbits.TRISB1=0; TRISBbits.TRISB2=0; TRISBbits.TRISB3=0; TRISBbits.TRISB4=0; TRISAbits.TRISA0=0; TRISAbits.TRISA1=0;
    TRISCbits.TRISC0=1; TRISCbits.TRISC1=1; TRISCbits.TRISC2=1; TRISBbits.TRISB5=1; TRISAbits.TRISA2=1;
...
static void init_uart(void) {
    ...
    BAUDCON1bits.BRG16=1; TXSTA1bits.BRGH=1; SPBRGH1=0x01; SPBRG1=0xA0;
...
static void init_i2c(void) {
    ...
    SSP1STAT=0x00; SSP1STATbits.SMP=1;
    SSP1CON1=0b00101000; SSP1ADD=39; SSP1CON1bits.SSPEN=1;
}
...
static _Bool i2c_write(uint8_t b){ i2c_wait(); SSP1BUF=b; i2c_wait(); return !SSP1CON2bits.ACKSTAT; }
static uint8_t i2c_read(_Bool ack){ uint8_t v; i2c_wait(); SSP1CON2bits.RCEN=1; i2c_wait(); v=SSP1BUF; i2c_wait(); SSP1CON2bits.ACKDT=ack?0:1; SSP1CON2bits.ACKEN=1; return v; }


static void pcf8574_write(uint8_t data){
    i2c_start(); i2c_write((0x27<<1)|0); i2c_write(data); i2c_stop();
}
static void lcd_pulse(uint8_t d){ pcf8574_write((uint8_t)(d|0x04)); _delay((unsigned long)((1)*(16000000UL/4000000.0))); pcf8574_write((uint8_t)(d&~0x04)); _delay((unsigned long)((50)*(16000000UL/4000000.0))); }
static void lcd_send4(uint8_t nibble, _Bool rs){
    uint8_t d = (uint8_t)((uint8_t)((nibble & 0x0F) << 4) | lcd_backpack | (rs ? 0x01 : 0));
    lcd_pulse(d);
}
static void lcd_send(uint8_t v, _Bool rs){ lcd_send4((uint8_t)(v>>4),rs); lcd_send4((uint8_t)(v&0x0F),rs); }
...
static void lcd_init(void){
    ...
    lcd_send4(0x03,0); _delay((unsigned long)((5)*(16000000UL/4000.0)));
    lcd_send4(0x03,0); _delay((unsigned long)((150)*(16000000UL/4000000.0)));
    lcd_send4(0x03,0); lcd_send4(0x02,0);
    lcd_cmd(0x28);
    lcd_cmd(0x08);
    lcd_cmd(0x01); _delay((unsigned long)((2)*(16000000UL/4000.0)));
    lcd_cmd(0x06);
    lcd_cmd(0x0C);
}
static void lcd_clear(void){ lcd_cmd(0x01); _delay((unsigned long)((2)*(16000000UL/4000.0))); }
static void lcd_set_cursor(uint8_t row, uint8_t col){
...
static uint8_t rtc_read_reg(uint8_t reg){
    i2c_start(); i2c_write((0x68<<1)|0); i2c_write(reg);
    i2c_restart(); i2c_write((0x68<<1)|1);
    uint8_t v = i2c_read(0); i2c_stop(); return v;
}
static void rtc_write_reg(uint8_t reg, uint8_t val){
    i2c_start(); i2c_write((0x68<<1)|0); i2c_write(reg); i2c_write(val); i2c_stop();
}
static void rtc_sanitize(void){
    ...
    if (hr & (1u<<6)) {
        uint8_t hr12 = bcd2bin((uint8_t)(hr & 0x1F)); _Bool pm = (hr & 0x20) != 0;
        uint8_t hr24 = (uint8_t)((hr12%12) + (pm?12:0));
        rtc_write_reg(0x02, (uint8_t)(bin2bcd(hr24) & 0x3F));
    }
}
static void rtc_read(uint8_t *sec, uint8_t *min, uint8_t *hour, uint8_t *dow) {
    i2c_start(); i2c_write((0x68<<1)|0); i2c_write(0x00);
    i2c_restart(); i2c_write((0x68<<1)|1);
    uint8_t s=i2c_read(1), m=i2c_read(1), h=i2c_read(1), dw=i2c_read(0); i2c_stop();
...
static void rtc_read_full(uint8_t *sec, uint8_t *min, uint8_t *hour, uint8_t *dow,
    ...
    i2c_start(); i2c_write((0x68<<1)|0); i2c_write(0x00);
    i2c_restart(); i2c_write((0x68<<1)|1);
    uint8_t s=i2c_read(1), m=i2c_read(1), h=i2c_read(1), dw=i2c_read(1), dt=i2c_read(1), mo=i2c_read(1), yr=i2c_read(0); i2c_stop();
...
static void load_settings(void){
    if (eeprom_read_byte(0x00)!=0x5Au){
        dayMask=0x7F; restDelayMs=60000UL;
        preMotorDelayMs = 30000UL;
        postMotorRelOffMs = 30000UL;
        drainSingleMs = (60UL*1000UL);

        eeprom_write_byte(0x00,0x5Au);
        eeprom_write_byte(0x01,dayMask);

        uint32_t v=restDelayMs;
        eeprom_write_byte(0x02 +0,(uint8_t)v);
        eeprom_write_byte(0x02 +1,(uint8_t)(v>>8));
        eeprom_write_byte(0x02 +2,(uint8_t)(v>>16));
        eeprom_write_byte(0x02 +3,(uint8_t)(v>>24));

        v = preMotorDelayMs;
        eeprom_write_byte(0x06 +0,(uint8_t)v);
        eeprom_write_byte(0x06 +1,(uint8_t)(v>>8));
        eeprom_write_byte(0x06 +2,(uint8_t)(v>>16));
        eeprom_write_byte(0x06 +3,(uint8_t)(v>>24));

        v = postMotorRelOffMs;
        eeprom_write_byte(0x0A +0,(uint8_t)v);
        eeprom_write_byte(0x0A +1,(uint8_t)(v>>8));
        eeprom_write_byte(0x0A +2,(uint8_t)(v>>16));
        eeprom_write_byte(0x0A +3,(uint8_t)(v>>24));

        v = drainSingleMs;
        eeprom_write_byte(0x0E +0,(uint8_t)v);
        eeprom_write_byte(0x0E +1,(uint8_t)(v>>8));
        eeprom_write_byte(0x0E +2,(uint8_t)(v>>16));
        eeprom_write_byte(0x0E +3,(uint8_t)(v>>24));
    } else {
        dayMask=eeprom_read_byte(0x01);

        uint32_t v=0;
        v|=(uint32_t)eeprom_read_byte(0x02 +0);
        v|=((uint32_t)eeprom_read_byte(0x02 +1))<<8;
        v|=((uint32_t)eeprom_read_byte(0x02 +2))<<16;
        v|=((uint32_t)eeprom_read_byte(0x02 +3))<<24;
        if (v==0||v>(12UL*60UL*60UL*1000UL)) v=60000UL; restDelayMs=v;

        v=0;
        v|=(uint32_t)eeprom_read_byte(0x06 +0);
        v|=((uint32_t)eeprom_read_byte(0x06 +1))<<8;
        v|=((uint32_t)eeprom_read_byte(0x06 +2))<<16;
        v|=((uint32_t)eeprom_read_byte(0x06 +3))<<24;
        if (v==0||v>(10UL*60UL*1000UL)) v=30000UL; preMotorDelayMs=v;

        v=0;
        v|=(uint32_t)eeprom_read_byte(0x0A +0);
        v|=((uint32_t)eeprom_read_byte(0x0A +1))<<8;
        v|=((uint32_t)eeprom_read_byte(0x0A +2))<<16;
        v|=((uint32_t)eeprom_read_byte(0x0A +3))<<24;
        if (v==0||v>(10UL*60UL*1000UL)) v=30000UL; postMotorRelOffMs=v;

        v=0;
        v|=(uint32_t)eeprom_read_byte(0x0E +0);
        v|=((uint32_t)eeprom_read_byte(0x0E +1))<<8;
        v|=((uint32_t)eeprom_read_byte(0x0E +2))<<16;
        v|=((uint32_t)eeprom_read_byte(0x0E +3))<<24;
        if (v==0||v>(10UL*60UL*1000UL)) v=(60UL*1000UL); drainSingleMs=v;
    }
}
static void save_settings(void){
    eeprom_write_byte(0x01,dayMask);
...
static void rtc_shadow_save(uint16_t yr, uint8_t mo, uint8_t dt, uint8_t dow, uint8_t hh, uint8_t mm, uint8_t ss){
    uint8_t rec[9]; rec[0]=0xB1; rec[1]=(uint8_t)(yr-2000); rec[2]=mo; rec[3]=dt; rec[4]=dow; rec[5]=hh; rec[6]=mm; rec[7]=ss;
    uint16_t sum=0; for(uint8_t i=0;i<8;i++) sum+=rec[i]; rec[8]=(uint8_t)sum;
    eeprom_write_block(0x20, rec, 9);
}
static _Bool rtc_shadow_load(uint16_t *yr, uint8_t *mo, uint8_t *dt, uint8_t *dow, uint8_t *hh, uint8_t *mm, uint8_t *ss){
    uint8_t rec[9]; eeprom_read_block(0x20, rec, 9);
    if (rec[0]!=0xB1) return 0;
    uint16_t sum=0; for(uint8_t i=0;i<8;i++) sum+=rec[i];
    if (((uint8_t)sum)!=rec[8]) return 0;
    uint16_t Y=(uint16_t)(2000 + rec[1]); if (Y<2000 || Y>2099) return 0;
...
static void relays_all_off(void){ LATBbits.LATB0=LATBbits.LATB1=LATBbits.LATB2=LATBbits.LATB3=LATBbits.LATB4=0; LATAbits.LATA0=LATAbits.LATA1=0; }
static void relays_function_off(uint8_t fn){ if(fn==1){LATBbits.LATB0=0;LATBbits.LATB1=0;LATAbits.LATA0=0;} else {LATBbits.LATB2=0;LATBbits.LATB3=0;LATBbits.LATB4=0;LATAbits.LATA1=0;} }
static void motor_set(uint8_t motor,_Bool on){ if(motor==1) LATAbits.LATA0=on?1:0; else LATAbits.LATA1=on?1:0; }
static void relay_set(uint8_t relay,_Bool on){ switch(relay){case 1:LATBbits.LATB0=on;break;case 2:LATBbits.LATB1=on;break;case 3:LATBbits.LATB2=on;break;case 4:LATBbits.LATB3=on;break;case 5:LATBbits.LATB4=on;break;} }
static _Bool relay_get(uint8_t relay){ switch(relay){case 1:return LATBbits.LATB0;case 2:return LATBbits.LATB1;case 3:return LATBbits.LATB2;case 4:return LATBbits.LATB3;case 5:return LATBbits.LATB4;} return 0; }
static _Bool float_triggered(uint8_t relay){
    uint8_t pin=0xFF; switch(relay){case 1:pin=!PORTCbits.RC0;break;case 2:pin=!PORTCbits.RC1;break;case 3:pin=!PORTCbits.RC2;break;case 4:pin=!PORTBbits.RB5;break;case 5:pin=!PORTAbits.RA2;break;default:return 0;}
    static uint32_t tStart[6]={0}; static _Bool last[6]={0};
    _Bool active=(pin!=0);
    if (active!=last[relay]){ last[relay]=active; tStart[relay]=millis(); return 0; }
    if (active && elapsed(tStart[relay],120UL)) return 1;
    return 0;
}
...
static _Bool float_active_stable(uint8_t idx){
    ...
    _Bool raw=0;
    switch(idx){
        case 1: raw = (!PORTCbits.RC0)!=0; break;
        case 2: raw = (!PORTCbits.RC1)!=0; break;
        case 3: raw = (!PORTCbits.RC2)!=0; break;
        case 4: raw = (!PORTBbits.RB5)!=0; break;
        case 5: raw = (!PORTAbits.RA2)!=0; break;
    }
    static _Bool lastRaw[6]={0};
    static _Bool stable [6]={0};
    ...
    if (elapsed(changeT[idx], 120UL)){ stable[idx]=lastRaw[idx]; }
    return stable[idx];
}


static void compose_lcd_lines_busy(void){
    char tmp[96]={0}; _Bool any=0;
    if (relay_get(1)){strcat(tmp, any?" | ":"");strcat(tmp,"T1 Fill/T2 Drain");any=1;}
    if (relay_get(2)){strcat(tmp, any?" | ":"");strcat(tmp,"T2 Fill/T1 Drain");any=1;}
    if (relay_get(3)){strcat(tmp, any?" | ":"");strcat(tmp,"T3 Fill/T5 Drain");any=1;}
    if (relay_get(4)){strcat(tmp, any?" | ":"");strcat(tmp,"T4 Fill/T3 Drain");any=1;}
    if (relay_get(5)){strcat(tmp, any?" | ":"");strcat(tmp,"T5 Fill/T4 Drain");any=1;}
    if (!any) strcpy(tmp,"");
    strncpy(lcd_line1,tmp,sizeof(lcd_line1)-1);

    _Bool m1=LATAbits.LATA0!=0, m2=LATAbits.LATA1!=0;
...
static void lcd_task(void){
    ...
    if (drain_single_active) {
        ...
        switch (drain_single_relay) {
            case 1: tank = "Tank 2"; break;
            case 2: tank = "Tank 1"; break;
            case 3: tank = "Tank 5"; break;
            case 4: tank = "Tank 3"; break;
            case 5: tank = "Tank 4"; break;
            default: tank = ""; break;
        }
        strncpy(lcd_line2, tank, sizeof(lcd_line2)-1);
    } else {
        _Bool fBusy = (F1.running || F2.running);
        _Bool anyRelay = relay_get(1)||relay_get(2)||relay_get(3)||relay_get(4)||relay_get(5)||(LATAbits.LATA0||LATAbits.LATA1);
...
static void fsm_abort(fsm_t *F){
    ...
    F->state=ST_IDLE; F->running=0; F->stop_requested=0;
...
static void fsm_start(fsm_t *F){
    ...
    F->running=1; F->stop_requested=0; F->idx=0; F->state=ST_PRE_MOTOR_DELAY;
    uint8_t r = (F->idx < 4) ? F->sequence[F->idx] : 0; relay_set(r,1); F->tMark=millis();
    log_event("start-pre"); ee_log_write_snapshot();
}
static void fsm_request_stop(fsm_t *F){
    if (!F->running && F->state==ST_IDLE) return;
    if (F->stop_requested) return;
    F->stop_requested=1; log_event("stop-req"); ee_log_write_snapshot();
}
static void fsm_next_or_done(fsm_t *F){
    F->idx++;
    _Bool end = (F->idx >= 4) || (F->sequence[F->idx] == 0);
    if (end){
        if (F->loop && !F->stop_requested && today_enabled()){
            ...
            F->running=0; F->state=ST_IDLE; F->stop_requested=0;
...
static void fsm_tick(fsm_t *F){
    if (!F->running && F->state==ST_IDLE) return;
    uint8_t r = (F->idx < 4) ? F->sequence[F->idx] : 0;
    switch(F->state){
        case ST_PRE_MOTOR_DELAY:
            ...
            if (elapsed(F->tMark, preMotorDelayMs)){
                motor_set(F->motor,1); F->state=ST_MOTOR_ON_WAIT_FLOAT; F->tMark=millis();
                log_event("motor-on"); ee_log_write_snapshot();
            }
            break;
        case ST_MOTOR_ON_WAIT_FLOAT:
            if (F->stop_requested){
                motor_set(F->motor,0); F->state=ST_POST_MOTOR_OFF_DELAY; F->tMark=millis();
        ...
        case ST_REST_DELAY:
            if (elapsed(F->tMark, restDelayMs)){
                if (F->stop_requested){ F->running=0; F->state=ST_IDLE; F->stop_requested=0; log_event("stopped"); ee_log_write_snapshot(); }
                else if (today_enabled()){
                    r = (F->idx < 4) ? F->sequence[F->idx] : 0;
                    relay_set(r,1); F->state=ST_PRE_MOTOR_DELAY; F->tMark=millis(); log_event("next-pre"); ee_log_write_snapshot();
                }
                else { F->running=0; F->state=ST_IDLE; log_event("idle/dayoff"); ee_log_write_snapshot(); }
            }
            break;
        default: break;
    }
...
static void fsm_resume_apply(fsm_t *F, fsm_state_t st, uint8_t idx, _Bool running,_Bool stopreq,uint8_t est_age_s){
    if (idx >= 4 || F->sequence[idx] == 0) idx = 0;
    F->idx = idx; F->running=running; F->stop_requested=stopreq; F->state=st;
    uint8_t r = (F->idx < 4) ? F->sequence[F->idx] : 0;
    switch(st){
        case ST_PRE_MOTOR_DELAY:
            relay_set(r,1); motor_set(F->motor,0);
            if (est_age_s >= (preMotorDelayMs/1000)){ motor_set(F->motor,1); F->state=ST_MOTOR_ON_WAIT_FLOAT; F->tMark=millis(); }
            else { uint32_t rem=preMotorDelayMs-(uint32_t)est_age_s*1000UL; F->tMark=millis()-(preMotorDelayMs-rem); }
            break;
        case ST_MOTOR_ON_WAIT_FLOAT:
            relay_set(r,1); motor_set(F->motor,1); F->tMark=millis(); break;
        case ST_POST_MOTOR_OFF_DELAY:
            relay_set(r,1); motor_set(F->motor,0);
            if (est_age_s >= (postMotorRelOffMs/1000)){ relay_set(r,0); if (F->stop_requested){ F->state=ST_REST_DELAY; F->tMark=millis(); } else { fsm_next_or_done(F); } }
            else { uint32_t rem=postMotorRelOffMs-(uint32_t)est_age_s*1000UL; F->tMark=millis()-(postMotorRelOffMs-rem); }
            break;
        case ST_REST_DELAY:
            relay_set(r,0); motor_set(F->motor,0);
            if (est_age_s >= (restDelayMs/1000)){ if (F->stop_requested){ F->running=0; F->state=ST_IDLE; F->stop_requested=0; } else if (today_enabled()){ relay_set(r,1); F->state=ST_PRE_MOTOR_DELAY; F->tMark=millis(); } else { F->running=0; F->state=ST_IDLE; } }
            else { uint32_t rem=restDelayMs-(uint32_t)est_age_s*1000UL; F->tMark=millis()-(restDelayMs-rem); }
            break;
        case ST_IDLE: default: F->running=0; F->state=ST_IDLE; F->stop_requested=0; break;
    }
}
...
static void print_status_now(void){
    ...
    uart_puts("Relay 1 - "); uart_puts(LATBbits.LATB0?"ON\n":"OFF\n");
    uart_puts("Relay 2 - "); uart_puts(LATBbits.LATB1?"ON\n":"OFF\n");
    uart_puts("Relay 3 - "); uart_puts(LATBbits.LATB2?"ON\n":"OFF\n");
    uart_puts("Relay 4 - "); uart_puts(LATBbits.LATB3?"ON\n":"OFF\n");
    uart_puts("Relay 5 - "); uart_puts(LATBbits.LATB4?"ON\n":"OFF\n");

    uart_puts("Motor 1 - "); uart_puts(LATAbits.LATA0?"ON\n":"OFF\n");
    uart_puts("Motor 2 - "); uart_puts(LATAbits.LATA1?"ON\n":"OFF\n");
...
static _Bool uart_line_available(char *outLine, uint8_t maxLen){
    static uint8_t rd=0;
    while (rd != rxHead) {
        char c = rxBuf[rd]; rd = (rd+1)%64;
        static uint8_t w=0; static char line[64];
        if (c=='\r' || c=='\n') {
            ...
            return 1;
        } else { if (w<(64 -1)) line[w++]=c; }
    }
    return 0;
}

static _Bool parse_uint(const char *s, uint16_t *out, uint8_t minDigits, uint8_t maxDigits){
    uint16_t v=0; uint8_t n=0; while(*s && n<maxDigits && *s>='0'&&*s<='9'){ v=(uint16_t)(v*10+(*s-'0')); s++; n++; }
    if (n<minDigits) return 0; *out=v; return 1;
}
static _Bool parse_hms(const char *s, uint8_t *h, uint8_t *m, uint8_t *sc){
    uint16_t H,M,S=0; const char *p=s;
    if(!parse_uint(p,&H,2,2)||p[2]!=':')return 0; p+=3;
    if(!parse_uint(p,&M,2,2))return 0; p+=2;
    if(*p==':'){p++; if(!parse_uint(p,&S,2,2))return 0;}
    if(H>23||M>59||S>59)return 0; *h=(uint8_t)H;*m=(uint8_t)M;*sc=(uint8_t)S; return 1;
}
static _Bool parse_ymd(const char *s, uint16_t *y, uint8_t *m, uint8_t *d){
    uint16_t Y,M,D; const char *p=s;
    if(!parse_uint(p,&Y,4,4)||p[4]!='-')return 0; p+=5;
    if(!parse_uint(p,&M,2,2)||p[2]!='-')return 0; p+=3;
    if(!parse_uint(p,&D,2,2))return 0;
    if(M<1||M>12||D<1||D>31)return 0; *y=Y;*m=(uint8_t)M;*d=(uint8_t)D; return 1;
}
...
static _Bool is_all_digits(const char *p){
    if (!p || !*p) return 0;
...
static void start_single_drain(uint8_t relay){
    ...
    if (drain_single_active && drain_single_relay>=1 && drain_single_relay<=5) {
        relay_set(drain_single_relay,0);
    }


    relay_set(relay,1);
    drain_single_active = 1;
...
static void parse_command(const char *cmd){
    ...
    if (!strcmp(buf,"STARTF1")) { if(!today_enabled()){ send_today_off_msg(); return; } if(drain_single_active){ uart_puts("DRAIN ACTIVE - Start blocked\n"); return; } fsm_start(&F1); uart_puts(ok); }
    ...
    else if (!strcmp(buf,"QUIET")) { btVerbose=0; uart_puts(ok); }
    else if (!strcmp(buf,"VERBOSE")) { btVerbose=1; uart_puts(ok); }
        ...
        if (ms==0 || ms>(12UL*60UL*60UL*1000UL)) { uart_puts(err); return; }
        restDelayMs = ms;
        save_settings();
        ee_log_write_snapshot();
        uart_puts(ok);
        ...
        uint32_t ms = (uint32_t)mins * 60000UL;
        if (ms==0 || ms>(10UL*60UL*1000UL)) { uart_puts(err); return; }
        preMotorDelayMs = ms;
        ...
        if (L==7 || L==8){
            ...
            if (d1>='0'&&d1<='9'&&d2>='0'&&d2<='9'){
                uint8_t reqDay=(uint8_t)((d1-'0')*10 + (d2-'0'));
                const char* suf = buf+5; _Bool set_on=0, set_off=0;
                if (!strcmp(suf,"ON")) set_on=1;
                else if (!strcmp(suf,"OFF")) set_off=1;
        ...
        switch(idx){
            ...
            case 3: mapped=4; break;
            case 4: mapped=5; break;
            case 5: mapped=3; break;
        }
...
static void log_event(const char *msg){
    ...
    rec.relBits=0; if(LATBbits.LATB0)rec.relBits|=0x01; if(LATBbits.LATB1)rec.relBits|=0x02; if(LATBbits.LATB2)rec.relBits|=0x04; if(LATBbits.LATB3)rec.relBits|=0x08; if(LATBbits.LATB4)rec.relBits|=0x10;
    rec.flags=0; if(F1.running)rec.flags|=0x01; if(F2.running)rec.flags|=0x02; if(LATAbits.LATA0)rec.flags|=0x04; if(LATAbits.LATA1)rec.flags|=0x08; if(dow_enabled(dw))rec.flags|=0x10;

    if (ramLogCount > 0) {
        uint8_t prevIdx = (uint8_t)((ramLogHead + 16 - 1) % 16);
    ...
    ramLogHead=(uint8_t)((ramLogHead+1)%16);
    if(ramLogCount<16) ramLogCount++;
}
static void print_onoff(_Bool v){ uart_puts(v?"ON\n":"OFF\n"); }
static void print_yesno(_Bool v){ uart_puts(v?"YES\n":"NO\n"); }

static void ram_log_dump(void){
    if (ramLogCount==0){ uart_puts("EMPTY\n"); return; }
    uint8_t start=(uint8_t)((ramLogHead + 16 - ramLogCount)%16);
    for(uint8_t i=0;i<ramLogCount;i++){
        uint8_t idx=(uint8_t)((start+i)%16);
...
static void ram_log_print_latest(void){
    if (ramLogCount==0){ uart_puts("EMPTY\n"); return; }
    uint8_t idx=(uint8_t)((ramLogHead + 16 - 1)%16);
    ram_rec_t *r = &ramLog[idx];
    char line[48];
    snprintf(line,sizeof(line),"%04u-%02u-%02u %s %02u:%02u:%02u\n",
             2000u+r->yOff, r->mon, r->day, dow_str(r->dow), r->hour, r->min, r->sec);
...
static uint8_t compute_csum(const ee_log_t *r){
    ...
    uint16_t s=0; for(uint8_t i=0;i<20 -1;i++) s+=p[i]; return (uint8_t)s;
}
static void ee_log_init(void){
    uint16_t lastSeq=0; int8_t slot=ee_log_find_latest_slot(&lastSeq);
    if (slot<0){ ee_log_nextSeq=1; ee_log_nextSlot=0; } else { ee_log_nextSeq=(uint16_t)(lastSeq+1); ee_log_nextSlot=(uint8_t)((slot+1)%48); }
}
static void fill_snapshot(ee_log_t *rec){
    memset(rec,0,sizeof(*rec)); rec->magic=0xA5; rec->seq=ee_log_nextSeq;
    uint8_t s,m,h,dw,dt,mo; uint16_t yr; rtc_read_full(&s,&m,&h,&dw,&dt,&mo,&yr);
    rec->yOff=(uint8_t)(yr-2000); rec->mon=mo; rec->day=dt; rec->dow=dw; rec->hour=h; rec->min=m; rec->sec=s;
    uint8_t relBits=0; if(LATBbits.LATB0)relBits|=0x01; if(LATBbits.LATB1)relBits|=0x02; if(LATBbits.LATB2)relBits|=0x04; if(LATBbits.LATB3)relBits|=0x08; if(LATBbits.LATB4)relBits|=0x10; rec->relBits=relBits;
    uint8_t motors=0; if(LATAbits.LATA0)motors|=0x01; if(LATAbits.LATA1)motors|=0x02; rec->motorBits=motors;
...
static void ee_log_write_snapshot(void){
    ...
    uint16_t addr=0x40 + (uint16_t)ee_log_nextSlot*20;
    eeprom_write_block(addr,(const uint8_t*)&rec,20);
    ee_log_nextSeq++; ee_log_nextSlot=(uint8_t)((ee_log_nextSlot+1)%48);
}
static _Bool ee_log_read_slot(uint8_t slot, ee_log_t *rec){
    if (slot>=48) return 0;
    uint16_t addr=0x40 + (uint16_t)slot*20;
    eeprom_read_block(addr,(uint8_t*)rec,20);
    if (rec->magic!=0xA5) return 0;
    if (compute_csum(rec)!=rec->csum) return 0;
    return 1;
}
static int8_t ee_log_find_latest_slot(uint16_t *pSeq){
...
static void ee_log_dump_latest(uint8_t maxLines){
    int8_t last=ee_log_find_latest_slot(((void*)0));
    ...
    while(count<maxLines){
        ...
        slot=(uint8_t)((slot + 48 - 1) % 48);
    }
}


static _Bool is_leap(uint16_t y){ return ((y%4==0)&&((y%100)!=0)) || (y%400==0); }
