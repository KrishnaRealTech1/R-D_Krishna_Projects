...
static bool Is_Hour_Window_Enabled(uint8_t hour, uint8_t minute);
...
static bool Is_Hour_Window_Enabled(uint8_t hour, uint8_t minute)
{
    ...
    // DIP2 ON = operate from 08:00 up to and including 18:00 exactly.
    // This blocks 18:01 through 23:59, so the scheduled 18:40 pulse
    // can never occur in daytime-only mode.
    if (hour < DAY_WINDOW_START_HOUR)
    {
        return false;
    }

    if (hour < DAY_WINDOW_END_HOUR)
    {
        return true;
    }

    if ((hour == DAY_WINDOW_END_HOUR) && (minute == 0U))
    {
        return true;
    }
...
static bool Is_Allowed_Day_And_Hour(const RTC_Time *now)
{
    ...
    // DIP2 OFF = 24 hours; DIP2 ON = 08:00 through 18:00 exactly.
