...
volatile _Bool g_h1_state = 0;
volatile _Bool g_h2_state = 0;
volatile _Bool g_lever_state = 0;
...
volatile uint16_t g_handle_low_seconds = 0;
volatile _Bool g_brush_timeout_active = 0;
...
void debounce_inputs_10ms(void)
{
    ...
    h1_hist <<= 1;
    h1_hist |= (PORTAbits.RA0 ? 1 : 0);
    if (h1_hist == 0x00) g_h1_state = 0;
    else if (h1_hist == 0xFF) g_h1_state = 1;

    h2_hist <<= 1;
    h2_hist |= (PORTAbits.RA1 ? 1 : 0);
    if (h2_hist == 0x00) g_h2_state = 0;
    else if (h2_hist == 0xFF) g_h2_state = 1;

    lever_hist <<= 1;
    lever_hist |= (PORTAbits.RA2 ? 1 : 0);
    if (lever_hist == 0x00) g_lever_state = 0;
    else if (lever_hist == 0xFF) g_lever_state = 1;
}
...
void update_logic_1s(void)
{
    ...
    if (!g_h1_state || !g_h2_state)
    {
        if (g_handle_low_seconds < (5u * 60u))
            g_handle_low_seconds++;

        if (g_handle_low_seconds >= (5u * 60u))
        {
            g_brush_timeout_active = 1;
            g_brush_on = 0;
        }
    }
    else
...
_Bool i2c_write_byte(uint8_t data)
{
    ...
    {
        if (data & 0x80) TRISCbits.TRISC4 = 1;
...
void oled_show_boot_screen(void)
{
    ...
    const uint8_t top_page = 2;
    const uint8_t underline_page = 6;
...
void hourmeter_save_to_eeprom(void)
{
    ...
    eeprom_write_byte(0x0000u + 0, (uint8_t)(val & 0xFFu));
    eeprom_write_byte(0x0000u + 1, (uint8_t)((val >> 8) & 0xFFu));
    eeprom_write_byte(0x0000u + 2, (uint8_t)((val >> 16) & 0xFFu));
    eeprom_write_byte(0x0000u + 3, (uint8_t)((val >> 24) & 0xFFu));
...
void hourmeter_load_from_eeprom(void)
{
    ...
    uint32_t val;
    val = (uint32_t)eeprom_read_byte(0x0000u + 0);
    val |= (uint32_t)eeprom_read_byte(0x0000u + 1) << 8;
    val |= (uint32_t)eeprom_read_byte(0x0000u + 2) << 16;
    val |= (uint32_t)eeprom_read_byte(0x0000u + 3) << 24;
