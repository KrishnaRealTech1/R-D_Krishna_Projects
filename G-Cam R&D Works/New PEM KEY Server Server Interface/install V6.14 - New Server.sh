...
DEFAULT_SFTP_HOST="103.91.187.21"
DEFAULT_SFTP_PORT="2244"
...
echo "=================================================="
echo " G-Cam Full Clean Reinstall - V6.13 PV ONLY V8"
...
SFTP_AUTH_MODE_DEFAULT="password"
SFTP_PRIVATE_KEY_PATH_DEFAULT=""
SFTP_PRIVATE_KEY_PASSPHRASE_DEFAULT=""
SFTP_PRIVATE_KEY_SOURCE_PATH=""
SFTP_PRIVATE_KEY_TEMP_SOURCE=""
...
read -rp "Enter SFTP Auth Mode password/key [${SFTP_AUTH_MODE_DEFAULT}]: " SFTP_AUTH_MODE
SFTP_AUTH_MODE="${SFTP_AUTH_MODE:-$SFTP_AUTH_MODE_DEFAULT}"
SFTP_AUTH_MODE="$(echo "$SFTP_AUTH_MODE" | tr '[:upper:]' '[:lower:]' | xargs)"
if [[ "$SFTP_AUTH_MODE" != "password" && "$SFTP_AUTH_MODE" != "key" ]]; then
  echo "Invalid SFTP auth mode: $SFTP_AUTH_MODE. Use password or key."
  exit 1
fi

if [[ "$SFTP_AUTH_MODE" == "password" ]]; then
  ...
  SFTP_PASSWORD=""
  read -rp "Enter SFTP Private Key Path (.ppk or OpenSSH .pem) [${SFTP_PRIVATE_KEY_PATH_DEFAULT}]: " SFTP_PRIVATE_KEY_PATH
  SFTP_PRIVATE_KEY_PATH="${SFTP_PRIVATE_KEY_PATH:-$SFTP_PRIVATE_KEY_PATH_DEFAULT}"

  read -rsp "Enter SFTP Private Key Passphrase if any [empty]: " SFTP_PRIVATE_KEY_PASSPHRASE
  SFTP_PRIVATE_KEY_PASSPHRASE="${SFTP_PRIVATE_KEY_PASSPHRASE:-$SFTP_PRIVATE_KEY_PASSPHRASE_DEFAULT}"
  echo

  if [[ -z "$SFTP_PRIVATE_KEY_PATH" ]]; then
    echo "SFTP key auth selected, but no private key path was provided."
    exit 1
  fi

  SFTP_PRIVATE_KEY_SOURCE_PATH="$(expand_user_path "$SFTP_PRIVATE_KEY_PATH")"
  if [[ ! -f "$SFTP_PRIVATE_KEY_SOURCE_PATH" ]]; then
    echo "SFTP private key file not found: $SFTP_PRIVATE_KEY_SOURCE_PATH"
    exit 1
  fi

  SFTP_PRIVATE_KEY_TEMP_SOURCE="$(mktemp /tmp/gcam-sftp-key-source.XXXXXX)"
  cp "$SFTP_PRIVATE_KEY_SOURCE_PATH" "$SFTP_PRIVATE_KEY_TEMP_SOURCE"
  chmod 600 "$SFTP_PRIVATE_KEY_TEMP_SOURCE"
fi
...
sudo apt-get install -y \
  ...
  ffmpeg alsa-utils espeak-ng curl wget ca-certificates jq sudo putty-tools \
...
sudo dpkg -i "$CLOUDFLARED_DEB" || true
...
if [[ "$SFTP_AUTH_MODE" == "key" ]]; then
  echo "[8/17] Prepare SFTP private key..."
  SFTP_PRIVATE_KEY_FINAL="$APP_DIR/data/sftp_private_key.pem"

  if [[ "${SFTP_PRIVATE_KEY_SOURCE_PATH,,}" == *.ppk ]]; then
    puttygen "$SFTP_PRIVATE_KEY_TEMP_SOURCE" -O private-openssh -o "$SFTP_PRIVATE_KEY_FINAL"
  else
    cp "$SFTP_PRIVATE_KEY_TEMP_SOURCE" "$SFTP_PRIVATE_KEY_FINAL"
  fi

  sudo chown "$PI_USER:$PI_USER" "$SFTP_PRIVATE_KEY_FINAL"
  sudo chmod 600 "$SFTP_PRIVATE_KEY_FINAL"
  rm -f "$SFTP_PRIVATE_KEY_TEMP_SOURCE" 2>/dev/null || true
  SFTP_PRIVATE_KEY_PATH="$SFTP_PRIVATE_KEY_FINAL"
fi
...
cat > "$APP_DIR/data/config.json" <<JSON
{
  ...
  "software_version": "Version 6.13",
  "software_features": "Tiruppur Requirement Only + PEM Key Method Server Upload",
...
FULL_RES_IMAGE_JPEG_QUALITY = 98
EVENT_VIDEO_H264_CRF = 24
...
SOFTWARE_VERSION = str(CONFIG.get("software_version", "Version 6.13")).strip()
SOFTWARE_FEATURES = str(
    CONFIG.get(
        "software_features",
                "Tiruppur Requirement Only + PEM Key Method Server Upload"
    )
).strip()
...
SFTP_PASSWORD = CONFIG["sftp"].get("password", "")
SFTP_AUTH_MODE = str(CONFIG["sftp"].get("auth_mode", "password")).strip().lower()
SFTP_PRIVATE_KEY_PATH = str(CONFIG["sftp"].get("private_key_path", "")).strip()
SFTP_PRIVATE_KEY_PASSPHRASE = CONFIG["sftp"].get("private_key_passphrase") or None
...
def read_direct_main_frame_for_event(
    attempts: int = 18,
    warmup_frames: int = 3,
    retry_delay_sec: float = 0.08
) -> Tuple[Optional[np.ndarray], str]:
    """
    Synchronously pull a fresh frame from the MAIN RTSP stream for event upload.

    This is a safety net for the upload bug: if the background MAIN-stream
    cache is temporarily stale, the app must not save the SUB-stream detection
    frame to SFTP. It opens MAIN RTSP directly, skips a few warm-up frames, and
    returns the freshest MAIN frame available.
    """
    global latest_main_frame, latest_main_frame_ts
    ...
    try:
        ...
        if cap is None or not cap.isOpened():
            return None, "main_direct_open_failed"

        total_attempts = max(int(attempts), int(warmup_frames) + 1, 1)
        ...
        for idx in range(total_attempts):
            ...
            if ok and frame is not None and frame.size > 0:
                ...
                if idx >= warmup_frames:
                    with frame_lock:
                        latest_main_frame = best_frame.copy()
                        latest_main_frame_ts = time.time()
                    return best_frame, "main_direct"

            time.sleep(max(float(retry_delay_sec), 0.01))

        if best_frame is not None and best_frame.size > 0:
            with frame_lock:
                latest_main_frame = best_frame.copy()
                latest_main_frame_ts = time.time()
            return best_frame, "main_direct_last_good"

        return None, "main_direct_read_failed"

    except Exception as exc:
        return None, f"main_direct_error: {exc}"
...
def get_event_main_frame(
    wait_sec: float = 5.0,
    max_age_sec: float = 2.0
) -> Tuple[Optional[np.ndarray], str]:
    """
    Return a MAIN-stream frame for event image uploads.

    Order:
      1. Fresh background MAIN-frame cache.
      2. Direct MAIN RTSP read.
      3. None. Caller must skip upload instead of using SUB stream.
    """
    deadline = time.time() + max(float(wait_sec), 0.5)
    last_source = "main_cache_missing"

    while time.time() < deadline and not stop_event.is_set():
        main_frame, _main_ts = get_main_frame_copy_with_ts(max_age_sec=max_age_sec)
        if main_frame is not None and main_frame.size > 0:
            return main_frame, "main_cache"

        time.sleep(0.05)

    direct_frame, direct_source = read_direct_main_frame_for_event()
    if direct_frame is not None and direct_frame.size > 0:
        return direct_frame, direct_source

    last_source = direct_source or last_source
...
def build_high_quality_event_image(
    ...
    """
    Build SFTP event image from MAIN stream only.

    AI detection still runs on the SUB stream. But uploaded JPGs must come from
    the MAIN stream. This function no longer silently falls back to the SUB
    frame, because that created low-resolution SFTP images.
    """
    main_frame, main_source = get_event_main_frame(wait_sec=5.0, max_age_sec=2.0)

    if main_frame is None or main_frame.size == 0:
        raise RuntimeError(
            "MAIN RTSP frame unavailable for high-quality SFTP upload; "
            f"source={main_source}. Skipping event image instead of uploading SUB stream."
        )
    ...
    if callable(refine_fn):
        ...
        # For strict person mode, still keep the uploaded image MAIN-stream.
        # If redetect fails on full-res main, scale the confirmed sub-stream boxes
        # instead of falling back to the sub-stream image.
        scaled = scale_detections_to_frame(detections, detection_frame, main_frame)
        if scaled:
            return draw_fn(main_frame, scaled), scaled, f"{main_source}_scaled_after_redetect_none"
...
def load_sftp_private_key() -> paramiko.PKey:
    if not SFTP_PRIVATE_KEY_PATH:
        raise RuntimeError("SFTP key authentication selected, but private_key_path is empty")

    key_path = Path(SFTP_PRIVATE_KEY_PATH).expanduser()
    if not key_path.exists():
        raise FileNotFoundError(f"SFTP private key file not found: {key_path}")
    ...
    for key_loader in key_loaders:
        try:
            return key_loader.from_private_key_file(
                str(key_path),
    ...
    raise RuntimeError(f"Unable to load SFTP private key {key_path}: {last_error}")
...
def sftp_connect() -> Tuple[paramiko.SSHClient, paramiko.SFTPClient]:
    ...
    if SFTP_AUTH_MODE == "key":
        connect_kwargs["pkey"] = load_sftp_private_key()
    else:
                ...
                if persons:
                    ...
                    if should_fire_event:
                        ...
                        try:
                            ...
                            image_source_stream = "main_unavailable_skip"
                            error_text = str(image_exc).strip() or repr(image_exc)
                            add_event("person_event_skipped_main_stream_unavailable", {
                                "reason": error_text,
                            ...
                            update_state(lambda s: s.update({
                                "last_person": {
                                    ...
                                    "image": s["last_person"].get("image"),
                                    "image_file": s["last_person"].get("image_file"),
                                    "image_status": f"skipped_main_stream_unavailable: {error_text}",
                                    "image_uploaded_path": s["last_person"].get("image_uploaded_path"),
                                    "count": len(persons),
                                    "best_confidence": best_conf,
                                    "stable_inside_sec": round(person_stable_inside_sec, 1),
                            ...
                            print(f"[PERSON EVENT] Skipped HQ upload: {error_text}", flush=True)
                            continue
            ...
            if vehicle_detection_enabled:
                ...
                def fire_vehicle_event(vehicle_event_type: str) -> None:
                    ...
                    try:
                        ...
                        image_source_stream = "main_unavailable_skip"
                        error_text = str(image_exc).strip() or repr(image_exc)
                        add_event("vehicle_event_skipped_main_stream_unavailable", {
                            "reason": error_text,
                        ...
                        print(f"[VEHICLE EVENT] Skipped HQ upload: {error_text}", flush=True)
                        return
...
def handle_command(payload: Dict[str, Any]) -> None:
    ...
    if cmd in {"close the software", "open the software", "camera permanent url", "camera temp url"}:
        mqtt_publish(MQTT_RESPONSE_TOPIC, {
            "device_id": DEVICE_ID,
...
echo "[INFO] MAIN stream ONLY is used for high-quality image/video SFTP uploads: ${RTSP_URL}"
