...
echo "=================================================="
echo " G-Cam Full Clean Reinstall - V6 FIXED"
...
cat > "$APP_DIR/data/config.json" <<JSON
{
  ...
  "software_version": "Version 6.1",
  "software_features": "Person Detection + Vehicle Detection + Garbage Detection + Audio DVM + Warning System + Live Stream + Temp Cam Access URL + V/A Interface + Device Monitoring + Image & Video Upload",
...
{"command":"Pi Volume 75%","token":"***"}
  ...
  <div style="margin-bottom:14px">
    <strong>Draw garbage detection zone</strong> — click and drag on the image below.
    The highlighted box is the only area where garbage will be detected.
  </div>
  ...
  <div style="display:flex;gap:14px;align-items:flex-start;flex-wrap:wrap">
    ...
    <div style="display:grid;gap:12px;min-width:200px">
      <div class="info-box">
        <h4>Current Zone (ratios)</h4>
        <div id="geoRoiDisplay" style="font-family:monospace;font-size:13px">—</div>
      </div>
      <div class="info-box">
        <h4>Status</h4>
        <div id="geoStatus">—</div>
      </div>
      <button id="geoBtnSave" onclick="geoSave()"
      ...
      <button id="geoBtnReset" onclick="geoReset()"
      ...
      <div style="color:var(--muted);font-size:12px;line-height:1.6">
        Drag a rectangle on the image.<br>
        Only garbage inside the box is detected.<br>
        Saved immediately — no restart needed.
      </div>
    </div>
  </div>
</div>
...
<div id="tab-person_geofence" class="panel">
  <div style="margin-bottom:14px">
    <strong>Draw person detection zone</strong> — click and drag on the image below.
    Only persons whose center/foot falls inside this box will trigger detection.
  </div>
  ...
  <div style="display:flex;gap:14px;align-items:flex-start;flex-wrap:wrap">
    ...
    <div style="display:grid;gap:12px;min-width:200px">
      <div class="info-box">
        <h4>Current Zone (ratios)</h4>
        <div id="personGeoRoiDisplay" style="font-family:monospace;font-size:13px">—</div>
      </div>
      <div class="info-box">
        <h4>Status</h4>
      ...
      <div style="color:var(--muted);font-size:12px;line-height:1.6">
        Drag a rectangle on the image.<br>
        Only persons inside the box are detected.<br>
        Saved immediately — no restart needed.
      </div>
    </div>
  </div>
...
let geoRect = null;      // {x1,y1,x2,y2} in canvas pixels
let geoDrawing = false;
let geoStart = null;
let geoCurrentRoi = null; // saved ratios from server

function geoCanvasPos(e) {
  const r = geoCanvas.getBoundingClientRect();
  const scaleX = geoCanvas.width / r.width;
  const scaleY = geoCanvas.height / r.height;
...
function geoRedraw() {
  geoCtx.drawImage(geoImg, 0, 0, geoCanvas.width, geoCanvas.height);

  // Darken outside zone
  if (geoRect) {
    const {x1,y1,x2,y2} = geoRect;
    geoCtx.fillStyle = 'rgba(0,0,0,0.45)';
    geoCtx.fillRect(0, 0, geoCanvas.width, y1);
    geoCtx.fillRect(0, y2, geoCanvas.width, geoCanvas.height - y2);
    geoCtx.fillRect(0, y1, x1, y2 - y1);
    geoCtx.fillRect(x2, y1, geoCanvas.width - x2, y2 - y1);

    // Draw box
    geoCtx.strokeStyle = '#39c6ff';
    geoCtx.lineWidth = 2.5;
    geoCtx.setLineDash([6, 3]);
    geoCtx.strokeRect(x1, y1, x2 - x1, y2 - y1);
    geoCtx.setLineDash([]);

    // Label
    geoCtx.fillStyle = '#39c6ff';
    geoCtx.font = 'bold 13px Arial';
    geoCtx.fillText('Garbage Zone', x1 + 6, y1 + 18);

    // Update ratio display
    const rX1 = (x1 / geoCanvas.width).toFixed(4);
    const rY1 = (y1 / geoCanvas.height).toFixed(4);
    const rX2 = (x2 / geoCanvas.width).toFixed(4);
    const rY2 = (y2 / geoCanvas.height).toFixed(4);
    document.getElementById('geoRoiDisplay').textContent =
      `x1=${rX1}, y1=${rY1}\nx2=${rX2}, y2=${rY2}`;
  }
}
...
geoCanvas.addEventListener('mousemove', e => {
  ...
  const pos = geoCanvasPos(e);
  geoRect = {
    x1: Math.min(geoStart.x, pos.x),
    y1: Math.min(geoStart.y, pos.y),
    x2: Math.max(geoStart.x, pos.x),
    y2: Math.max(geoStart.y, pos.y)
  };
  geoRedraw();
});
...
geoCanvas.addEventListener('mouseup', () => { geoDrawing = false; });
geoCanvas.addEventListener('touchend', () => { geoDrawing = false; });

function geoApplyServerRoi(roi) {
  // roi = [x1r, y1r, x2r, y2r] as ratios
  geoCurrentRoi = roi;
  geoRect = {
    x1: roi[0] * geoCanvas.width,
    y1: roi[1] * geoCanvas.height,
    x2: roi[2] * geoCanvas.width,
    y2: roi[3] * geoCanvas.height
  };
  geoRedraw();
}

async function geoRefreshSnapshot() {
  document.getElementById('geoMsg').textContent = 'Loading snapshot...';
  geoImg = new Image();
  geoImg.onload = async () => {
    geoCanvas.width = geoImg.naturalWidth;
    geoCanvas.height = geoImg.naturalHeight;
    geoCtx.drawImage(geoImg, 0, 0);
    document.getElementById('geoMsg').textContent = 'Drag to draw zone';

    // Load current saved ROI
    try {
      const r = await fetch('/api/geofence');
      const d = await r.json();
      geoApplyServerRoi(d.garbage_roi);
    } catch(e) {}
  };
  geoImg.onerror = () => {
    document.getElementById('geoMsg').textContent = 'No frame available — camera connecting?';
  };
  geoImg.src = '/api/snapshot?t=' + Date.now();
}

async function geoSave() {
  if (!geoRect) {
    document.getElementById('geoStatus').textContent = '⚠️ Draw a zone first';
    return;
  }
  const x1r = geoRect.x1 / geoCanvas.width;
  const y1r = geoRect.y1 / geoCanvas.height;
  const x2r = geoRect.x2 / geoCanvas.width;
  const y2r = geoRect.y2 / geoCanvas.height;

  if ((x2r - x1r) < 0.04 || (y2r - y1r) < 0.04) {
    document.getElementById('geoStatus').textContent = '⚠️ Zone too small, draw bigger';
    return;
  }

  document.getElementById('geoStatus').textContent = 'Saving...';
  try {
    const resp = await fetch('/api/geofence', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({garbage_roi: [x1r, y1r, x2r, y2r]})
    });
    const data = await resp.json();
    if (data.ok) {
      document.getElementById('geoStatus').textContent = '✅ Saved! Applied immediately.';
      geoCurrentRoi = data.garbage_roi;
    } else {
      document.getElementById('geoStatus').textContent = '❌ Error: ' + data.error;
    }
  } catch(e) {
    document.getElementById('geoStatus').textContent = '❌ Network error';
  }
}

async function geoReset() {
  document.getElementById('geoStatus').textContent = 'Resetting...';
  try {
    const resp = await fetch('/api/geofence/reset', {method: 'POST'});
    const data = await resp.json();
    if (data.ok) {
      document.getElementById('geoStatus').textContent = '✅ Reset to default.';
      geoApplyServerRoi(data.garbage_roi);
    }
  } catch(e) {
    document.getElementById('geoStatus').textContent = '❌ Network error';
  }
...
// ============ PERSON GEOFENCE EDITOR ============
const personGeoCanvas = document.getElementById('personGeoCanvas');
const personGeoCtx = personGeoCanvas.getContext('2d');
let personGeoImg = new Image();
let personGeoRect = null;
let personGeoDrawing = false;
let personGeoStart = null;

function personGeoCanvasPos(e) {
  const r = personGeoCanvas.getBoundingClientRect();
  const scaleX = personGeoCanvas.width / r.width;
  const scaleY = personGeoCanvas.height / r.height;
  const clientX = e.touches ? e.touches[0].clientX : e.clientX;
  const clientY = e.touches ? e.touches[0].clientY : e.clientY;
  return {
    x: (clientX - r.left) * scaleX,
...
function personGeoRedraw() {
  personGeoCtx.drawImage(personGeoImg, 0, 0, personGeoCanvas.width, personGeoCanvas.height);

  if (personGeoRect) {
    const {x1,y1,x2,y2} = personGeoRect;
    personGeoCtx.fillStyle = 'rgba(0,0,0,0.45)';
    personGeoCtx.fillRect(0, 0, personGeoCanvas.width, y1);
    personGeoCtx.fillRect(0, y2, personGeoCanvas.width, personGeoCanvas.height - y2);
    personGeoCtx.fillRect(0, y1, x1, y2 - y1);
    personGeoCtx.fillRect(x2, y1, personGeoCanvas.width - x2, y2 - y1);

    personGeoCtx.strokeStyle = '#22c55e';
    personGeoCtx.lineWidth = 2.5;
    personGeoCtx.setLineDash([6, 3]);
    personGeoCtx.strokeRect(x1, y1, x2 - x1, y2 - y1);
    personGeoCtx.setLineDash([]);

    personGeoCtx.fillStyle = '#22c55e';
    personGeoCtx.font = 'bold 13px Arial';
    personGeoCtx.fillText('Person Zone', x1 + 6, y1 + 18);

    const rX1 = (x1 / personGeoCanvas.width).toFixed(4);
    const rY1 = (y1 / personGeoCanvas.height).toFixed(4);
    const rX2 = (x2 / personGeoCanvas.width).toFixed(4);
    const rY2 = (y2 / personGeoCanvas.height).toFixed(4);
    document.getElementById('personGeoRoiDisplay').textContent =
      `x1=${rX1}, y1=${rY1}\nx2=${rX2}, y2=${rY2}`;
  }
}

personGeoCanvas.addEventListener('mousedown', e => {
  personGeoDrawing = true;
  personGeoStart = personGeoCanvasPos(e);
  personGeoRect = null;
});
personGeoCanvas.addEventListener('touchstart', e => {
  e.preventDefault();
  personGeoDrawing = true;
  personGeoStart = personGeoCanvasPos(e);
  personGeoRect = null;
...
personGeoCanvas.addEventListener('mousemove', e => {
  if (!personGeoDrawing) return;
  const pos = personGeoCanvasPos(e);
  personGeoRect = {
    x1: Math.min(personGeoStart.x, pos.x),
    y1: Math.min(personGeoStart.y, pos.y),
    x2: Math.max(personGeoStart.x, pos.x),
    y2: Math.max(personGeoStart.y, pos.y)
  };
  personGeoRedraw();
});
personGeoCanvas.addEventListener('touchmove', e => {
  e.preventDefault();
  if (!personGeoDrawing) return;
  const pos = personGeoCanvasPos(e);
  personGeoRect = {
...
personGeoCanvas.addEventListener('mouseup', () => { personGeoDrawing = false; });
personGeoCanvas.addEventListener('touchend', () => { personGeoDrawing = false; });

async function personGeoRefreshSnapshot() {
  document.getElementById('personGeoMsg').textContent = 'Loading snapshot...';
  personGeoImg = new Image();
  personGeoImg.onload = async () => {
    personGeoCanvas.width = personGeoImg.naturalWidth;
    personGeoCanvas.height = personGeoImg.naturalHeight;
    personGeoCtx.drawImage(personGeoImg, 0, 0);
    document.getElementById('personGeoMsg').textContent = 'Drag to draw zone';

    try {
      const r = await fetch('/api/person_geofence');
      const d = await r.json();
      const roi = d.person_roi;
      personGeoRect = {
        x1: roi[0] * personGeoCanvas.width,
        y1: roi[1] * personGeoCanvas.height,
        x2: roi[2] * personGeoCanvas.width,
        y2: roi[3] * personGeoCanvas.height
      };
      personGeoRedraw();
    } catch(e) {}
  };
  personGeoImg.onerror = () => {
    document.getElementById('personGeoMsg').textContent = 'No frame available — camera connecting?';
  };
  personGeoImg.src = '/api/snapshot?t=' + Date.now();
}

async function personGeoSave() {
  if (!personGeoRect) {
    document.getElementById('personGeoStatus').textContent = '⚠️ Draw a zone first';
    return;
  }
  const x1r = personGeoRect.x1 / personGeoCanvas.width;
  const y1r = personGeoRect.y1 / personGeoCanvas.height;
  const x2r = personGeoRect.x2 / personGeoCanvas.width;
  const y2r = personGeoRect.y2 / personGeoCanvas.height;

  if ((x2r - x1r) < 0.04 || (y2r - y1r) < 0.04) {
    document.getElementById('personGeoStatus').textContent = '⚠️ Zone too small, draw bigger';
    return;
  }

  document.getElementById('personGeoStatus').textContent = 'Saving...';
  try {
    const resp = await fetch('/api/person_geofence', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({person_roi: [x1r, y1r, x2r, y2r]})
    });
    const data = await resp.json();
    if (data.ok) {
      document.getElementById('personGeoStatus').textContent = '✅ Saved! Applied immediately.';
    } else {
      document.getElementById('personGeoStatus').textContent = '❌ Error: ' + data.error;
    }
  } catch(e) {
    document.getElementById('personGeoStatus').textContent = '❌ Network error';
  }
}

async function personGeoReset() {
  document.getElementById('personGeoStatus').textContent = 'Resetting...';
  try {
    const resp = await fetch('/api/person_geofence/reset', {method: 'POST'});
    const data = await resp.json();
    if (data.ok) {
      document.getElementById('personGeoStatus').textContent = '✅ Reset to default.';
      const roi = data.person_roi;
      personGeoRect = {
        x1: roi[0] * personGeoCanvas.width,
        y1: roi[1] * personGeoCanvas.height,
        x2: roi[2] * personGeoCanvas.width,
...
async function refreshState(){
  try{
    ...
    document.getElementById('garbageMode').textContent = s.system.garbage_mode || 'Unknown';
...
EVENT_PRE_RECORD_SEC = 10
EVENT_POST_RECORD_SEC = 10
EVENT_CLIP_DURATION_SEC = EVENT_PRE_RECORD_SEC + EVENT_POST_RECORD_SEC
EVENT_ROLLING_BUFFER_SEC = 30
EVENT_BUFFER_JPEG_QUALITY = 82
EVENT_MIN_FPS = 6.0
EVENT_MAX_FPS = 20.0
EVENT_DEFAULT_FPS = 10.0
...
PERSON_UPSCALE_FACTOR = 1.18
PERSON_NMS_THRESHOLD = 0.26
PERSON_LOST_TIMEOUT_SEC = 1.2
PERSON_MIN_AREA = 1800
PERSON_TILE_STEP_RATIO = 0.34
PERSON_TILE_MIN_CONF = 0.52
PERSON_HOG_ENABLE = False
PERSON_ALLOWED_ROI = (0.01, 0.03, 0.99, 0.995)  # kept as hard boundary fallback
DEFAULT_PERSON_ROI = (0.01, 0.03, 0.99, 0.995)
PERSON_MIN_BOX_HEIGHT_RATIO = 0.12
PERSON_MIN_ASPECT_RATIO = 1.20
PERSON_MAX_BOX_WIDTH_RATIO = 0.40

VEHICLE_CLASSES = {"car", "bus", "motorbike", "train"}
...
VEHICLE_MAX_BOX_WIDTH_RATIO = 0.92
VEHICLE_MAX_BOX_HEIGHT_RATIO = 0.92
VEHICLE_COOLDOWN_SEC = 12
VEHICLE_CONFIRMATION_FRAMES = 3
VEHICLE_LOST_TIMEOUT_SEC = 1.5
...
# Pi 5 1GB-safe settings
GARBAGE_OBJ_INPUT_SIZE = 320
GARBAGE_OBJ_CONF       = 0.18
GARBAGE_OBJ_NMS        = 0.45
GARBAGE_MIN_BOX_AREA   = 12 * 12
GARBAGE_MAX_BOX_AREA_RATIO = 0.55

# Only use labels that really exist in COCO for this ONNX
...
# Map COCO labels to your preferred display names
GARBAGE_LABEL_ALIAS = {
    "bottle": "Bottle",
    "cup": "Cup",
    "bowl": "Container",
...
SOFTWARE_VERSION = str(CONFIG.get("software_version", "Version 6.1")).strip()
SOFTWARE_FEATURES = str(
    CONFIG.get(
        "software_features",
        "Person Detection + Vehicle Detection + Garbage Detection + Audio DVM + Warning System + Live Stream + Temp Cam Access URL + V/A Interface + Device Monitoring + Image & Video Upload"
    )
).strip()
...
def load_state() -> Dict[str, Any]:
    ...
    with state_lock:
        ...
        state_cache["system"]["garbage_mode"] = "reference_plus_object_roi"
...
def detect_garbage_objects(roi: np.ndarray) -> List[Dict[str, Any]]:
    ...
    for row in data:
        ...
        # YOLOv8 ONNX: [cx, cy, w, h, class_scores...]
        # YOLOv5-style: [cx, cy, w, h, obj_conf, class_scores...]
        if row.shape[0] == 84:
            class_scores = row[4:]
        ...
        # If model output is normalized, scale to ROI pixels
        ...
        box_area = w * h
        if box_area < GARBAGE_MIN_BOX_AREA:
            continue
    ...
    # COCO 80 labels expected by this ONNX model
    coco_labels = [
    ...
    return dedupe_garbage_detections(detections)
...
def person_box_allowed(frame: np.ndarray, box: Tuple[int, int, int, int], confidence: float) -> bool:
    ...
    # reject short/wide clutter near the ground
    if aspect_ratio < 1.45 and confidence < 0.72:
        return False

    # Use dynamic person ROI from config
    rx1, ry1, rx2, ry2 = get_person_roi_pixels(frame)
...
def detect_persons(frame: np.ndarray) -> List[Dict[str, Any]]:
    ...
    for d in merged:
        ...
        if aspect_ratio < 1.45 and conf < 0.72:
            continue
...
def vehicle_box_allowed(frame: np.ndarray, box: Tuple[int, int, int, int], confidence: float) -> bool:
    h, w = frame.shape[:2]
    x1, y1, x2, y2 = box
    ...
    # reject obvious tiny clutter in top sky/background area
    cy = int((y1 + y2) / 2)
    if cy < int(h * 0.10):
        return False
...
def detect_vehicles(frame: np.ndarray) -> List[Dict[str, Any]]:
    ...
    for i in range(detections.shape[2]):
        ...
        if confidence < max(0.35, VEHICLE_CONF - 0.10):
            continue
        ...
        if not vehicle_box_allowed(frame, (x1, y1, x2, y2), confidence):
            continue
...
def draw_vehicle_annotations(frame: np.ndarray, vehicles: List[Dict[str, Any]]) -> np.ndarray:
    ...
    for idx, d in enumerate(vehicles, start=1):
        x1, y1, x2, y2 = d["box"]
        label = f'{d["display_label"]} {idx} {d["confidence"]:.2f}'
...
def maybe_publish_vehicle_combined_response(event_key: str) -> None:
    with vehicle_event_lock:
        ...
        payload = {
            ...
            "image_path": image_path,
            "image_status": "success" if item.get("image_ok") else "failed",
            "image_message": item.get("image_message"),

            "video_file": item.get("video_file"),
            "video_path": video_path,
            "video_status": "success" if item.get("video_ok") else "failed",
...
def async_send(local_path: Path, remote_dir: str, remote_name: str, mqtt_topic: Optional[str], mqtt_payload: Optional[Dict[str, Any]]) -> None:
    def worker():
        try:
            sftp_upload(local_path, remote_dir, remote_name)
            update_state(lambda s: s["system"].update({"sftp_last_ok": now_str(), "sftp_last_error": None}))
        except Exception as exc:
            update_state(lambda s: s["system"].update({"sftp_last_error": str(exc)}))

        if mqtt_topic and mqtt_payload:
            try:
                time.sleep(15)  # wait 15 second after SFTP upload before MQTT response
                mqtt_publish(mqtt_topic, mqtt_payload)
                update_state(lambda s: s["system"].update({"mqtt_last_ok": now_str(), "mqtt_last_error": None}))
            except Exception as exc:
                update_state(lambda s: s["system"].update({"mqtt_last_error": str(exc)}))
...
def draw_garbage_boxes(
    ...
    for idx, box in enumerate(change_boxes or [], start=1):
        x1, y1, x2, y2 = box
        gx1 = x1 + x1_roi
        gy1 = y1 + y1_roi
        gx2 = x2 + x1_roi
    ...
    cv2.putText(
        out,
        f"Severity: {severity}  Count: {len(detections) + len(change_boxes or [])}",
        (20, 30),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.8,
        color,
...
def analyze_garbage_once(frame: np.ndarray) -> Dict[str, Any]:
    ...
    if reference_frame is None:
        marked = frame.copy()
        x1_roi, y1_roi, x2_roi, y2_roi = roi_box
        cv2.rectangle(marked, (x1_roi, y1_roi), (x2_roi, y2_roi), (0, 255, 255), 2)
        cv2.putText(
            marked,
    ...
    abs_diff = cv2.absdiff(current_gray, ref_gray)
    _, diff_mask = cv2.threshold(abs_diff, 32, 255, cv2.THRESH_BINARY)
    ...
    for cnt in contours:
        ...
        if area > roi_area * 0.65:
            continue
        change_boxes.append((x, y, x + w, y + h))
        change_area_sum += area
    ...
    diff_ratio = round(float(np.count_nonzero(diff_mask)) / roi_area, 4)
...
def detector_loop() -> None:
    ...
    while not stop_event.is_set():
        ...
        try:
            if PERSON_DETECT_EVERY_N_FRAMES <= 1 or counter % PERSON_DETECT_EVERY_N_FRAMES == 0:
                persons = detect_persons(frame)
                print(f"[PERSON DEBUG] detected={len(persons)}", flush=True)

            if VEHICLE_DETECT_EVERY_N_FRAMES <= 1 or counter % VEHICLE_DETECT_EVERY_N_FRAMES == 0:
                vehicles = detect_vehicles(frame)
                print(f"[VEHICLE DEBUG] detected={len(vehicles)}", flush=True)
            ...
            if persons:
                ...
                is_new_person = not person_present_latch
            ...
            if vehicles:
                ...
                best_vehicle_conf = round(max(v["confidence"] for v in vehicles), 3)
                vehicle_labels = sorted({v["display_label"] for v in vehicles})
                ...
                confirmed_vehicle = vehicle_confirm_hits >= VEHICLE_CONFIRMATION_FRAMES
                ...
                if should_fire_vehicle_event:
                    now = now_dt()
                    event_ts = time.time()
                    marked_vehicle = draw_vehicle_annotations(frame, vehicles)
                    ...
                    threading.Thread(
                        ...
                        args=(now, event_ts, len(vehicles), best_vehicle_conf, vehicle_labels),
                        daemon=True
                    ).start()
                ...
                if (time.time() - last_seen_vehicle_ts) >= VEHICLE_LOST_TIMEOUT_SEC:
                    vehicle_present_latch = False
                    vehicle_confirm_hits = 0
                    update_state(lambda s: s.update({
                        "last_vehicle": {
...
def handle_command(payload: Dict[str, Any]) -> None:
    if not validate_command(payload):
        mqtt_publish(MQTT_RESPONSE_TOPIC, {"device_id": DEVICE_ID, "event": "command_rejected", "message": "Invalid token"})
        return
...
def api_geofence_get():
    """Return current garbage ROI as ratios."""
    cfg = load_config()
    roi = cfg.get("garbage_roi", list(DEFAULT_GARBAGE_ROI))
    return jsonify({"garbage_roi": roi, "source": "config"})


@app.route("/api/geofence", methods=["POST"])
def api_geofence_set():
    """Save new garbage ROI ratios to config.json (hot-reload, no restart)."""
    data = request.get_json(force=True, silent=True) or {}
    roi = data.get("garbage_roi")

    if not roi or len(roi) != 4:
        return jsonify({"ok": False, "error": "garbage_roi must be [x1,y1,x2,y2] ratios"}), 400

    x1, y1, x2, y2 = roi
    if not (0.0 <= x1 < x2 <= 1.0 and 0.0 <= y1 < y2 <= 1.0):
        return jsonify({"ok": False, "error": "Values must be 0.0–1.0, x1<x2, y1<y2"}), 400

    # Read config, update, write back
    cfg = load_config()
    cfg["garbage_roi"] = [round(v, 4) for v in [x1, y1, x2, y2]]
    safe_write_json(CONFIG_FILE, cfg)

    return jsonify({"ok": True, "garbage_roi": cfg["garbage_roi"]})


@app.route("/api/geofence/reset", methods=["POST"])
def api_geofence_reset():
    """Reset garbage ROI to full-frame default."""
    cfg = load_config()
    cfg["garbage_roi"] = list(DEFAULT_GARBAGE_ROI)
    safe_write_json(CONFIG_FILE, cfg)
    return jsonify({"ok": True, "garbage_roi": cfg["garbage_roi"]})
	
@app.route("/api/person_geofence", methods=["GET"])
def api_person_geofence_get():
    """Return current person ROI as ratios."""
    cfg = load_config()
    roi = cfg.get("person_roi", list(DEFAULT_PERSON_ROI))
    return jsonify({"person_roi": roi, "source": "config"})


@app.route("/api/person_geofence", methods=["POST"])
def api_person_geofence_set():
    """Save new person ROI ratios to config.json (hot-reload, no restart)."""
    data = request.get_json(force=True, silent=True) or {}
    roi = data.get("person_roi")

    if not roi or len(roi) != 4:
        return jsonify({"ok": False, "error": "person_roi must be [x1,y1,x2,y2] ratios"}), 400
    ...
    cfg = load_config()
    cfg["person_roi"] = [round(v, 4) for v in [x1, y1, x2, y2]]
    safe_write_json(CONFIG_FILE, cfg)

    return jsonify({"ok": True, "person_roi": cfg["person_roi"]})


@app.route("/api/person_geofence/reset", methods=["POST"])
def api_person_geofence_reset():
    """Reset person ROI to full-frame default."""
    cfg = load_config()
    cfg["person_roi"] = list(DEFAULT_PERSON_ROI)
    safe_write_json(CONFIG_FILE, cfg)
    return jsonify({"ok": True, "person_roi": cfg["person_roi"]})
