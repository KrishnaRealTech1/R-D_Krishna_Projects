...
echo "=================================================="
echo " G-Cam Full Clean Reinstall - V6.9 PV ONLY V5"
...
SFTP_HOST_DEFAULT="${DEFAULT_SFTP_HOST:-3.111.78.82}"
SFTP_PORT_DEFAULT="${DEFAULT_SFTP_PORT:-22}"
...
cat > "$APP_DIR/data/config.json" <<JSON
{
  ...
  "software_version": "Version 6.9",
  "software_features": "Tiruppur Requirement Only",
...
<div class="wrap">
  ...
  <div id="tab-live" class="panel active">
    <div class="panel-grid">
      ...
      <div class="info">
        ...
        <div class="info-box"><h4>Camera Config Temporary Link</h4><div class="link-box" id="cameraConfigTempUrl">{{ state.camera_config_temp_url or state.live_link.camera_url or "Inactive" }}</div></div>
      </div>
    </div>
  </div>
  ...
  <div id="tab-controls" class="panel">
    <div style="margin-bottom:14px">
      ...
      warning audio, garbage actions, audio queue, power saving, device restart
  ...
  <div id="tab-links" class="panel">
    <div class="panel-grid">
      <div class="info">
        ...
        <div class="info-box"><h4>Camera Config Temporary URL</h4><div class="link-box" id="cameraConfigTempUrl2">{{ state.camera_config_temp_url or state.live_link.camera_url or "Inactive" }}</div></div>
...
{"command":"Pi Volume 75%","token":"***"}
  ...
  <div class="footer-note">
    ...
    Camera config: <strong>{{ state.camera_config_url or "Unavailable" }}</strong><br>
    Temporary camera access: <strong>{{ state.camera_config_temp_url or state.live_link.camera_url or "Inactive" }}</strong>
  </div>
</div>
...
function personStatusText(person) {
  ...
  if (person.detected) return 'Detected now';
  if ((person.count || 0) > 0) return 'Detected, confirming...';
  if (person.image) return 'Last detection saved';
  return 'No recent detection';
}

function vehicleStatusText(vehicle) {
  if (!vehicle) return 'No recent detection';
  if (vehicle.detected) return 'Detected now';
  if ((vehicle.count || 0) > 0) {
    if ((vehicle.stable_required_sec || 0) > 0) {
      return 'Vehicle seen, waiting stable time...';
    }
    return 'Vehicle seen, confirming...';
  }
  if (vehicle.image) return 'Last detection saved';
  return 'No recent detection';
}
...
async function refreshState(){
  try{
    ...
    uiText('cameraConfigTempUrl', s.camera_config_temp_url || liveLink.camera_url || 'Inactive');
    ...
    if (lastPerson.image) {
      uiSrc('personImg', '/media/' + lastPerson.image + '?t=' + Date.now());
    }
    ...
    uiText('cameraConfigTempUrl2', s.camera_config_temp_url || liveLink.camera_url || 'Inactive');
...
from flask import Flask, Response, jsonify, render_template, send_from_directory, request, stream_with_context
...
CAMERA_TEMPORARY_URL_COMMAND = "camera temporary url"
...
SOFTWARE_VERSION = str(CONFIG.get("software_version", "Version 6.9")).strip()
SOFTWARE_FEATURES = str(
    CONFIG.get(
        "software_features",
                "Tiruppur Requirement Only"
    )
).strip()
...
def load_state() -> Dict[str, Any]:
    ...
    with state_lock:
        ...
        state_cache.setdefault("camera_config_url", PERMANENT_CAMERA_CONFIG_URL)
        state_cache.setdefault("camera_config_temp_url", None)
        state_cache.setdefault("camera_config_target_url", CAMERA_CONFIG_TARGET_URL)
        state_cache.setdefault("live_link", default_state()["live_link"])
        state_cache["live_link"].setdefault("camera_url", None)
...
def set_power_saving_enabled(
    ...
    def mutate(state):
        ...
        if enabled:
            ...
            state["camera_config_temp_url"] = None
            state["last_audio"] = {
                "file": None,
                "source_path": None,
                "time": changed_at,
...
def refresh_runtime_links_in_state() -> None:
    def mutate(state):
        ...
        if not state.get("camera_config_temp_url") and not state.get("live_link", {}).get("camera_url"):
            state["camera_config_temp_url"] = None
    update_state(mutate)
...
def build_high_quality_event_image(
    ...
    """
    Draw event annotations on the latest MAIN-stream frame for SFTP upload.

    AI detection runs on SUB stream, but uploaded image must come from MAIN stream.
    On an event, wait briefly for a fresh MAIN frame before falling back.
    This keeps dashboard/detection responsive and preserves high-quality SFTP output.
    """
    deadline = time.time() + 2.0

    while time.time() < deadline and not stop_event.is_set():
        main_frame, _main_ts = get_main_frame_copy_with_ts(max_age_sec=3.0)

        if main_frame is not None and main_frame.size > 0:
            scaled = scale_detections_to_frame(detections, detection_frame, main_frame)
            return draw_fn(main_frame, scaled), scaled, "main"

        time.sleep(0.05)

    fallback = detection_frame.copy()
    return draw_fn(fallback, detections), detections, "sub_fallback"
...
def set_temporary_cloudflare_link_state(base_url: Optional[str], expires_at: Optional[str]) -> None:
    base_url = str(base_url or "").strip().rstrip("/")
    live_url = f"{base_url}/live" if base_url else None
    camera_url = f"{base_url}/camera" if base_url else None

    def mutate(state):
        state["live_link"].update({
            "active": bool(base_url),
            "url": live_url,
            "camera_url": camera_url,
            "expires_at": expires_at
        })
        state["camera_config_temp_url"] = camera_url
...
def stop_live_tunnel() -> None:
    ...
    set_temporary_cloudflare_link_state(None, None)
...
def start_live_tunnel(duration_sec: int = LIVE_LINK_DURATION_SEC) -> Optional[str]:
    ...
    with cloudflare_lock:
        ...
        if cloudflare_is_active() and cloudflare_url:
            ...
            set_temporary_cloudflare_link_state(cloudflare_url, expires_at)
        ...
        expires_at = dt.datetime.fromtimestamp(cloudflare_expiry_ts).strftime("%Y-%m-%d %H:%M:%S")
        set_temporary_cloudflare_link_state(found_url, expires_at)
        return found_url
...
def build_live_links_payload(force_new: bool = False) -> Dict[str, Any]:
    ...
    camera_temp_url = f"{live_base}/camera" if live_base else None
    return {
...
def build_common_links_payload() -> Dict[str, Any]:
    active_live_base = cloudflare_url.rstrip("/") if cloudflare_is_active() and cloudflare_url else None
    camera_temp_url = f"{active_live_base}/camera" if active_live_base else None
    return {
...
def collect_device_status() -> Dict[str, Any]:
    ...
    return {
        ...
        **garbage_detection_mode_payload(),
        ...
        "camera_config_url": PERMANENT_CAMERA_CONFIG_URL,
        "camera_config_permanent_url": PERMANENT_CAMERA_CONFIG_URL,
        "camera_config_temp_url": f"{cloudflare_url.rstrip('/')}/camera" if cloudflare_is_active() and cloudflare_url else None,
        "camera_access_url_temp": f"{cloudflare_url.rstrip('/')}/camera" if cloudflare_is_active() and cloudflare_url else None,
        "camera_config_target_url": CAMERA_CONFIG_TARGET_URL,
        "rtsp_url": RTSP_URL,
        "cloudflare_live_rtsp_url": CLOUDFLARE_LIVE_RTSP_URL,
...
def refresh_device_metrics_in_state() -> None:
    ...
    def mutate(state):
        ...
        state["camera_config_temp_url"] = status.get("camera_config_temp_url")
        state["camera_config_target_url"] = CAMERA_CONFIG_TARGET_URL
        state["system"].update({
...
def normalize_command(raw: str) -> str:
    ...
    mapping = {
        ...
        "camera temporary url": CAMERA_TEMPORARY_URL_COMMAND,
        "temporary camera url": CAMERA_TEMPORARY_URL_COMMAND,
        "camera temporary access url": CAMERA_TEMPORARY_URL_COMMAND,
        "temporary camera access url": CAMERA_TEMPORARY_URL_COMMAND,
        "camera access url": CAMERA_TEMPORARY_URL_COMMAND,
        "cloudflare camera url": CAMERA_TEMPORARY_URL_COMMAND,
        "cloudflare temporary camera url": CAMERA_TEMPORARY_URL_COMMAND,
        "cloudflare temporary camera access url": CAMERA_TEMPORARY_URL_COMMAND,
...
def handle_command(payload: Dict[str, Any]) -> None:
    ...
    if cmd == CAMERA_TEMPORARY_URL_COMMAND:
        live_payload = build_live_links_payload(force_new=True)
        camera_temp_url = live_payload.get("camera_config_url_temp")
        event_name = "camera_temporary_url_created" if camera_temp_url else "camera_temporary_url_failed"
        response_payload = {
            "device_id": DEVICE_ID,
            "event": event_name,
            "camera_name": CAMERA_NAME,
    ...
    if cmd in {"close the software", "open the software", "camera permanent url"}:
        mqtt_publish(MQTT_RESPONSE_TOPIC, {
            "device_id": DEVICE_ID,
...
def api_state():
    ...
    current_camera_temp_url = f"{cloudflare_url.rstrip('/')}/camera" if cloudflare_is_active() and cloudflare_url else payload.get("camera_config_temp_url")
    payload["camera_config_url"] = PERMANENT_CAMERA_CONFIG_URL
    payload["camera_config_permanent_url"] = PERMANENT_CAMERA_CONFIG_URL
    payload["camera_config_temp_url"] = current_camera_temp_url
    payload["camera_access_url_temp"] = current_camera_temp_url
    payload["camera_config_target_url"] = CAMERA_CONFIG_TARGET_URL
    ...
    if PERMANENT_LIVE_URL:
        payload["live_link"] = {
            ...
            "camera_url": current_camera_temp_url,
            "expires_at": "Never"
        }
...
def handle_command(payload: Dict[str, Any]) -> None:
    ...
    if cmd == "close the software":
        handle_close_software(); return
    if cmd == "open the software":
        handle_open_software(); return
    if cmd == "camera permanent url":
        handle_camera_permanent_url(); return
